...
- `.specify/memory/constitution.md` is a compatibility adapter directing SpecKit to `AGENTS.md`, not an independently versioned policy artifact. The active SpecKit plan is feature context, not governance.
...
- Do not create per-agent policy docs unless the tool loads them and the policy cannot live in `AGENTS.md`. For tools that do not load it, pass `AGENTS.md` as read-only context; SpecKit reads the same governance through its compatibility adapter.
- `speckit-constitution` is unsupported in this repository. Treat constitution requests as `AGENTS.md` governance changes and never replace the SpecKit adapter with a second policy document.
...
- Prefer deletion, existing NT surfaces, and compact contracts. Do not expand verifier ecosystems for test-local literals, mock venue universes, or documentation stacks that do not reduce live-trading risk.
...
- NautilusTrader owns adapter protocols, market data, execution, order lifecycle, cache, portfolio/account/order/fill state, reconciliation, and venue translation. Bolt owns TOML schema, SSM resolution, registrations, strategy decision policy, shared admission, and Bolt-derived audit evidence; do not rebuild NT-owned surfaces.
- Core registration, execution, and admission remain venue-, market-family-, and strategy-agnostic. Concrete providers and bindings are selected through TOML-backed registries.
- Backtesting, research, and analytics use NT backtest engines, catalog types, reports, events, and snapshots before Bolt-owned machinery. Research and backtest configuration uses TOML or NT-native run config with direct field mapping and lineage. Research notebooks remain read-only research surfaces; dashboard trading truth comes from those NT surfaces or explicitly labeled exploratory data. Dashboards and analytics cannot submit or cancel orders, transfer funds, mutate credentials, or define independent trading truth; production strategy behavior must graduate to typed Rust and TOML contracts.
- External data and analytics providers require source, license, fidelity, and cost evidence before selection and must respect the active spec's approved cap unless the user explicitly waives it. Do not rebuild commodity data infrastructure when an acceptable provider exists.
...
- Live arming belongs to one manifest-bound content-addressed immutable executable running its own finite `ops launch` pre-arm phase. Live submit also requires the production entrypoint, canary and admission gates, mandatory decision and no-submit readiness evidence, configured caps, real SSM and venue artifacts, exact-head proof, and explicit operator approval. Mock-only evidence cannot authorize live operation. Only complete success constructs the opaque, non-serializable, non-cloneable, one-use Rust `LiveReadinessPermit` that the sole Start entrypoint consumes by value. Installation and audit receipts are inert; every systemd start or restart obtains a fresh in-process permit.
