...
- External review follows resolved local findings. Until the live required-status cutover is complete and the rules are re-queried, the current exact-head required statuses remain authoritative. After that cutover, CI is non-authoritative evidence; review requires the evidence applicable to the changed risk plus the native human controls below, not a green advisory result.
...
- Applicable exact-head Rust evidence remains proof for Rust claims but cannot authorize or veto merge. Rust Probe is debugging—use it only when cheap local checks cannot answer the question and never present it as broader proof.
