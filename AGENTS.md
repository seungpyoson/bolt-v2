...
- Keep PR bodies stable: do not use them as rolling evidence ledgers or embed the current/moving PR-head SHA, transient CI/check status, or head-specific internal/external review receipts. Put exact-head SHAs and verification receipts in review requests, review comments/records, and check runs instead, and do not repeatedly rewrite the body as heads move. PR bodies may state timeless merge requirements and stable behavioral/scope disclosures; this PR-body status/evidence rule does not restrict immutable release artifacts or spec anchors.
