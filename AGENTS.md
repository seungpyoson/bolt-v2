...
- Enforcement boundary: repo tooling gates cooperative paths through `just`, `scripts/rust_verification.py`, and `.no-mistakes.yaml`; standard PATH `cargo ...` is guarded by the machine-level cargo shim versioned in [seungpyoson/claude-system-config#80](https://github.com/seungpyoson/claude-system-config/pull/80), which reads this repo's `ci/rust-verification.toml` `[local_compile_policy]`.
- Residual local bypasses remain outside the accidental-use guard: absolute-path cargo invocation, `rustup run <toolchain> cargo ...`, processes that never load shell startup files, and direct `rustc` execution.
