...
5. **PURE RUST LIVE RUNTIME** — the *live trading* binary has no Python: it is a standalone Rust `LiveNode` using NT's Rust API directly, with no PyO3/maturin/pip in its build or runtime. Python **is** permitted for the research / backtest-for-research / data lane — a separate workspace with its own venv, never imported by or shipped in the live binary. The constraint is on the live-trading runtime, not the repository. See `specs/023-nt-research-analytics-platform/python-rust-division.md`.
