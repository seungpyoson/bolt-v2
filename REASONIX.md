...
- **Language** — Rust 2024 edition, pinned to 1.96.0 (`rust-toolchain.toml`).
...
| `just live` | Generate runtime config from the tracked profile `config/prod-btc-5m.toml` → `config/live.toml`, then run. |
| `just live-verify` | Prove a deployed runtime config regenerates from the tracked profile and still loads against this binary. |
...
- **`config/live.toml` is a generated, gitignored runtime artifact** — generate it from the tracked production profile `config/prod-btc-5m.toml` via `just live` / `bolt-v2 ops generate-live-config`, and never hand-edit it (#768). The legacy gitignored `config/live.local.toml` is no longer a source of truth.
