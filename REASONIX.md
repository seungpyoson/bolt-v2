...
- **Remote-first Rust verification** — agent sessions use the workflow in `AGENTS.md`; standard PATH `cargo ...` is guarded by the machine-level cargo shim, and `ci/rust-verification.toml` remains the policy source.
- **Release builds require Zig** — `cargo-zigbuild` + Zig 0.15.2 are needed for cross-compilation to `aarch64-unknown-linux-gnu`.
- **Python verification layer** — several lint/check commands go through `rust_verification.py` which wraps cargo; absolute-path cargo, cross-repo `--manifest-path` / `-C` invocations, already-running daemon environments, and toolchain-manager bypasses remain outside the accidental-use guard.
