...
| `just live` | Compose the tracked overlay `config/profiles/prod-btc-5m.overlay.toml` onto its base `config/root.toml` → `config/live.toml`, then run. |
...
- **`config/live.toml` is a generated, gitignored runtime artifact** — compose it from the tracked production overlay `config/profiles/prod-btc-5m.overlay.toml` over the base template `config/root.toml` via `just live` / `bolt-v2 ops generate-live-config`, and never hand-edit it (#768). The legacy gitignored `config/live.local.toml` is no longer a source of truth.
