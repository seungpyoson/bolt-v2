...
pub const TRANSFORM_IDENTITY: &str = "bybit-public-archive-spot-tick-trades-to-canonical-trades.v1";

/// Native trade prints only; aggregated prints must never satisfy this table.
pub const TRADE_SOURCE_TYPE_NATIVE: &str = "native";

/// Expected raw Bybit spot tick-trades header, in order.
pub const BYBIT_SPOT_TICK_TRADES_HEADER: [&str; 6] =
...
impl TradeAggressorSide {
    ...
    fn parse_bybit(raw: &str) -> Result<Self> {
...
/// Normalize an accepted Bybit public-archive spot tick-trades CSV into the
...
pub fn normalize_bybit_spot_tick_trades(
...
mod tests {
    ...
    fn accepted_dataset() -> AcceptedDataset {
        ...
        let object = IngestManifestObjectRecord {
            ...
            schema_columns: BYBIT_SPOT_TICK_TRADES_HEADER
                .iter()
