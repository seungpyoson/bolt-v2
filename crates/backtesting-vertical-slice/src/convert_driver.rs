...
/// such as the bybit mark-price batch, or a per-binding listing.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ConvertReport {
    ...
    /// [`run_objects`]) and for binding-level units (a failed listing, the bybit
    /// batch) the caller records directly.
...
mod tests {
    ...
    fn run_objects_isolates_a_bad_object_and_still_attempts_the_rest() {
        // The RUN2 failure mode: convert() `?`-aborted on the first bad object,
        // silently dropping every later object. Here the middle object errors
        // and the third must still be attempted and converted.
        let keys = vec![
            "good-a".to_string(),
            "bad".to_string(),
            "good-c".to_string(),
        ...
        assert!(failures.contains("okx/candlesticks"), "{failures}");
