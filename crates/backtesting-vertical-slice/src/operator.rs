...
impl RunSpec {
    ...
    pub fn validate_artifact_store_publish_config(
        ...
        let backtests_prefix = format!(
            ...
            self.manifest.artifact_store.storage_options.is_empty(),
            "run spec manifest.artifact_store.storage_options is retired for durable publication; use rust_storage_options only"
        );
        let manifest_region = self
            .manifest
            .artifact_store
        ...
        let manifest_conditional_put = self
            .manifest
            .artifact_store
            .rust_storage_options
            .get("conditional_put")
            ...
            manifest_conditional_put == expected_conditional_put,
            "run spec artifact-store conditional_put mismatch: manifest.artifact_store.rust_storage_options {:?} != artifact_store.s3.conditional_put {:?}",
            manifest_conditional_put,
...
///
/// The durable artifact-store path has different crash and strict-publication
/// semantics and is admitted only by [`DurableRunDispatcher`].
...
/// Exact-version locator returned only by a freshly acknowledged remote
/// terminal create. Existing terminals are never read into this authority.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
/// cross-artifact contract check performed before the sole fresh terminal
/// create.
...
/// authority. A separate read-only API may inspect an exact-current remote
/// terminal, but fresh execution never accepts it as success.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
impl TransientCatalogRootLease {
    ...
    fn finish_retained(self, work_budget: &OperatorWorkBudgetGuard) -> Result<()> {
        ...
        // Stable projection roots are retained audit evidence only; fresh
        // execution never reconciles or reuses them. Unique hydration roots
        // share the lifecycle of their owning output attempt.
        work_budget.check_deadline(OperatorWorkBudgetStage::Publish)
    }
}
...
async fn persist_durable_contract_artifact(
    ...
    let created = writer
        .put_create_strict_guarded(&path, payload, work_budget)
        .await
...
async fn assert_current_durable_completion_absent_guarded(
    store: &dyn ObjectStore,
    artifact_root: &ResolvedArtifactRoot,
    spec: &RunSpec,
    work_budget: &OperatorWorkBudgetGuard,
...
fn run_from_run_spec_inner(
    ...
    let spec = local_spec.get();
    let prepared = prepare_local_run_from_run_spec_inner(
        local_spec,
        object_bytes,
        output_dir,
...
fn prepare_local_run_from_run_spec_inner(
    ...
    source_binding_registry: &VerifiedSourceBindingRegistry,
    work_budget: &OperatorWorkBudgetGuard,
) -> Result<PreparedTradeRunArtifacts> {
...
fn prepare_run_from_run_spec_inner(
    ...
    let transient_catalog_root_lease = Some(lease);
...
impl DurableRunDispatcher {
    ...
    /// Require the deterministic current terminal to be absent without
    /// consuming source bytes or entering any publication/BacktestNode path.
    pub(crate) async fn assert_current_completion_absent_guarded(
        &self,
        spec: &RunSpec,
        registry: &VerifiedSourceBindingRegistry,
        work_budget: &OperatorWorkBudgetGuard,
...
pub(crate) async fn assert_current_durable_completion_absent_with_artifact_store_guarded(
    spec: &RunSpec,
    store: &dyn ObjectStore,
    versioning_enabled: &BucketVersioningEnabled,
    registry: &VerifiedSourceBindingRegistry,
    ...
    let (_, artifact_root, _) = guarded_operation_outcome(
        work_budget,
        OperatorWorkBudgetStage::ObjectVerification,
        || durable_run_validation_context(spec, versioning_enabled, registry),
    )??;
    assert_current_durable_completion_absent_guarded(store, &artifact_root, spec, work_budget).await
}
...
async fn run_from_run_spec_with_verified_registry_guarded(
    ...
    let created_completion = writer
        .create_terminal_strict(prepared_completion, permit)
        .await?;
    let completion = DurableCompletionLocator {
        object: DurableObjectVersionIdentity {
            ...
            version_id: created_completion.version_id,
            e_tag: created_completion.e_tag,
        },
    };
