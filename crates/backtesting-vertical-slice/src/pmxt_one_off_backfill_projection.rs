...
use crate::{
    catalog_projection::{logical_catalog_hash, normalize_clear_order_precision},
    conversion_boundary::{
...
mod tests {
    ...
    /// the price bounds populated by the official Polymarket parser.
    ...
    #[test]
    fn parser_shaped_binary_option_preserves_price_bounds_on_round_trip() {
        ...
        assert_eq!(read_back.max_price, parsed.max_price);
        assert_eq!(read_back.min_price, parsed.min_price);
        assert_eq!(read_back.outcome, parsed.outcome);
        assert_eq!(read_back.description, parsed.description);
    }
}
