...
const BYBIT_OPERATOR_INPUTS_PREFIX: &str =
    "specs/023-nt-research-analytics-platform/reference/source-universe-operator-inputs/bybit-";
const BYBIT_OPERATOR_INPUTS_SUFFIX: &str = "/operator-inputs/source-universe-operator-inputs.json";
...
/// `true` iff `path` is a generated Bybit source-universe operator-inputs artifact.
pub fn is_bybit_source_universe_operator_inputs_path(path: &str) -> bool {
    let Some(scope) = path.strip_prefix(BYBIT_OPERATOR_INPUTS_PREFIX) else {
        return false;
    };
    let Some(scope) = scope.strip_suffix(BYBIT_OPERATOR_INPUTS_SUFFIX) else {
        return false;
    };
    !scope.is_empty() && !scope.contains('/')
}
...
mod tests {
    ...
    #[test]
    fn bybit_operator_inputs_scope_accepts_only_family_shape() {
        let accepted = format!(
        ...
        let rejected_other_json =
            format!("{BYBIT_OPERATOR_INPUTS_PREFIX}example/operator-inputs/metadata.json");
        let rejected_toml =
            format!("{BYBIT_OPERATOR_INPUTS_PREFIX}example/source-universe-operator-inputs.toml");
