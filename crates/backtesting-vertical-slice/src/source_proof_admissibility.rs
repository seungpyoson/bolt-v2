...
impl SourceProofAdmissibilityReport {
    ...
    pub fn content_hash(&self) -> Result<String, SourceProofAdmissibilityReportError> {
        canonical_json_sha256(self).map_err(|error| match error {
            ReferenceArtifactError::Serialize(error) => {
                SourceProofAdmissibilityReportError::Serialize(error)
            }
            ReferenceArtifactError::ReadExisting { .. }
            | ReferenceArtifactError::ExistingArtifactMismatch { .. }
            | ReferenceArtifactError::Write { .. } => unreachable!(),
        })
    }
}
...
pub fn write_source_proof_admissibility_report(
    ...
    let written = write_reference_artifact_with_len_mapped(
        &path,
        ...
        ReferenceArtifactErrorMappers {
            ...
            write_error: |path, error| SourceProofAdmissibilityWriteError::Write { path, error },
        },
    )?;
