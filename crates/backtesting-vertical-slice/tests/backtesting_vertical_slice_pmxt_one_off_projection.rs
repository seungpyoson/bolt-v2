...
#[test]
fn pmxt_one_off_projection_honors_the_required_missing_side_policy() {
    let dropped = one_sided_quote_projection(true);
    assert!(
        dropped.quote_ticks.is_empty(),
        "drop_quotes_missing_side=true must reject a one-sided quote"
    );

    let synthesized = one_sided_quote_projection(false);
    assert_eq!(synthesized.quote_ticks.len(), 1);
    let quote = synthesized
        .quote_ticks
        .first()
        .expect("drop_quotes_missing_side=false must retain the one-sided quote");
    assert!(quote.bid_price < quote.ask_price);
    assert!(quote.bid_size.is_zero());
}
...
fn one_sided_quote_projection(drop_quotes_missing_side: bool) -> PmxtOneOffNtProjection {
    project_pmxt_one_off_rows_to_nt(PmxtOneOffProjectionRequest {
        ...
        selected_token_id: "token-a".to_string(),
        gamma_markets: gamma_markets(),
        rows: vec![PmxtOneOffSelectedRow::PriceChange(PmxtPriceChangeRow {
            market: "0xcondition".to_string(),
            asset_id: "token-a".to_string(),
            price: "0.50".to_string(),
    ...
    .expect("project one-sided PMXT quote")
}
