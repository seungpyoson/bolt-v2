...
#[test]
fn binance_operator_inputs_index_entries_match_declared_exact_scope() {
    let repo_root = repo_root_from_manifest_dir();
    let index = EvictedFixtureIndex::load(&repo_root).expect("load evicted-fixtures index");
    let indexed: BTreeSet<String> = index
        .entries
        .iter()
        .map(|entry| entry.path.clone())
        .filter(|path| is_binance_source_universe_operator_inputs_path(path))
        .collect();
    let declared: BTreeSet<String> = BINANCE_OPERATOR_INPUTS_EVICTED_REFERENCE_PATHS
        .iter()
        .map(|path| (*path).to_string())
        .collect();
    ...
    assert_eq!(
        indexed,
        declared,
        "Binance operator-inputs index entries must exactly match the declared generated-reference eviction scope; \
         indexed but undeclared: {:?}; declared but unindexed: {:?}",
        indexed.difference(&declared).collect::<Vec<_>>(),
        declared.difference(&indexed).collect::<Vec<_>>(),
    );
    ...
    for path in &declared {
        assert!(
            !repo_root.join(path).exists(),
            "Binance operator-inputs artifact {path:?} must be absent from the working tree"
        );
    }
}

#[test]
fn binance_operator_inputs_scope_has_no_regrown_working_tree_artifacts() {
    let repo_root = repo_root_from_manifest_dir();
    ...
    for path in files_under(&operator_inputs_root) {
        let repo_relative = repo_relative_path(&repo_root, &path);
        assert!(
            !is_binance_source_universe_operator_inputs_path(&repo_relative),
            "Binance operator-inputs artifact {repo_relative:?} must remain evicted from the working tree"
        );
    }
}

#[test]
fn binance_operator_inputs_gitignore_pattern_matches_eviction_predicate() {
    let repo_root = repo_root_from_manifest_dir();
    for &path in BINANCE_OPERATOR_INPUTS_EVICTED_REFERENCE_PATHS {
        assert!(
            is_binance_source_universe_operator_inputs_path(path),
            "Binance operator-inputs path {path:?} must be in the eviction predicate"
        );
        assert!(
            git_check_ignore(&repo_root, path),
            "Binance operator-inputs path {path:?} must be ignored by .gitignore"
        );
    }

    let hypothetical = "specs/023-nt-research-analytics-platform/reference/source-universe-operator-inputs/binance-hypothetical/operator-inputs/source-universe-operator-inputs.json";
    ...
    let nested = "specs/023-nt-research-analytics-platform/reference/source-universe-operator-inputs/binance-hypothetical/nested/operator-inputs/source-universe-operator-inputs.json";
    assert!(!is_binance_source_universe_operator_inputs_path(nested));
