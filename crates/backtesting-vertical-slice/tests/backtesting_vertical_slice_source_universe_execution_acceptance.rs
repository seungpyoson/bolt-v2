...
use crate::backtesting_vertical_slice_test_support::{
    BINANCE_SOURCE_UNIVERSE_OPERATOR_INPUTS_PATH, BYBIT_SOURCE_UNIVERSE_OPERATOR_INPUTS_PATH,
    PMXT_SOURCE_UNIVERSE_OBJECT_MANIFEST_PATH, generate_evicted_binance_operator_inputs,
    generate_evicted_bybit_operator_inputs, materialize_evicted_pmxt_object_manifests,
...
fn committed_source_universe_execution_acceptance_ledger_round_trips_through_evaluator() {
    ...
    let binance_operator_inputs_sha256 = evicted_index
        .sha256_for(BINANCE_SOURCE_UNIVERSE_OPERATOR_INPUTS_PATH)
        .unwrap_or_else(|| {
            panic!(
                "evicted fixture index does not contain {BINANCE_SOURCE_UNIVERSE_OPERATOR_INPUTS_PATH}"
            )
        });
    let (bybit_operator_inputs_path, bybit_run_plan_path) =
    ...
    let bybit_operator_inputs_sha256 = evicted_index
        .sha256_for(BYBIT_SOURCE_UNIVERSE_OPERATOR_INPUTS_PATH)
        .unwrap_or_else(|| {
            panic!(
                "evicted fixture index does not contain {BYBIT_SOURCE_UNIVERSE_OPERATOR_INPUTS_PATH}"
            )
        });
    ...
    let evaluated_binance_operator_inputs_sha256 = artifact_ref_sha256(
        &evaluated,
        "backfill-source-universe-binance-data-vision-trades-2026-03-01-all-instruments",
    ...
    let evaluated_bybit_operator_inputs_sha256 = artifact_ref_sha256(
        &evaluated,
        "backfill-source-universe-bybit-public-archive-tick-trades-2025-06-01-2026-06-01",
        "source_universe_operator_inputs",
    );
    ...
    assert_eq!(
        evaluated_binance_operator_inputs_sha256, binance_operator_inputs_sha256,
        "evaluator must hash the freshly regenerated Binance operator inputs before ledger normalization"
    );
    assert_eq!(
        evaluated_bybit_run_plan_sha256, bybit_run_plan_sha256,
        "evaluator must hash the freshly regenerated Bybit run-plan artifact before ledger normalization"
    );
    assert_eq!(
        evaluated_bybit_operator_inputs_sha256, bybit_operator_inputs_sha256,
        "evaluator must hash the freshly regenerated Bybit operator inputs before ledger normalization"
    );
    assert_eq!(
    ...
    normalize_artifact_ref_path(
        ...
        "source_universe_operator_inputs",
        BINANCE_SOURCE_UNIVERSE_OPERATOR_INPUTS_PATH,
        &binance_operator_inputs_sha256,
    );
    normalize_artifact_ref_path(
        &mut evaluated,
        "backfill-source-universe-bybit-public-archive-tick-trades-2025-06-01-2026-06-01",
