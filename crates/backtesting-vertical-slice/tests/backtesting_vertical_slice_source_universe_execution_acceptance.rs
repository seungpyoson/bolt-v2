...
fn committed_source_universe_execution_acceptance_ledger_round_trips_through_evaluator() {
    ...
    // Re-evaluate from the committed spec, using its directory as base_dir so
    // repo-relative input paths resolve the same way the writer did.
    ...
    // The evaluator must reproduce the committed ledger exactly.
    assert_eq!(
