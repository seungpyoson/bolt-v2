...
fn committed_source_universe_execution_acceptance_ledger_round_trips_through_evaluator() {
    ...
    // The historical evicted Bybit run-plan blob embeds absolute worktree
    // paths, so tempdir regeneration cannot byte-match the indexed fixture.
    // Keep this live hash check before normalizing the committed path/hash.
    assert_eq!(
