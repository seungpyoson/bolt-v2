...
If `entry_no_action_reason` is `missing_reference_quote`, `stale_reference_quote`, `fee_rate_unavailable`, `fair_probability_unavailable`, `insufficient_edge`, `market_cooling_down`, `recovery_mode`, `one_position_invariant`, `thin_book`, or `position_limit_reached`, `updown_market_mechanical_outcome` must be `accepted` and `updown_market_mechanical_rejection_reason` must be null.
