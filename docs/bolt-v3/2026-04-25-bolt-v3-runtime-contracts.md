...
The gate reads at most the configured `max_no_submit_readiness_report_bytes` from `no_submit_readiness_report_path` and requires a JSON object with non-empty string linkage fields `approval_id_hash`, `executable_identity`, and `config_bundle_checksum`, plus a non-empty `stages` array. The linkage fields must match the current loaded `[live_canary].approval_id`, current executable bytes, and loaded root/strategy TOML bundle. Each stage must expose `stage` and `status = "satisfied"` exactly. Missing, mismatched, unreadable, oversized, unparsable, non-array, empty, or unsatisfied reports reject the run before NT's runner loop is entered.
