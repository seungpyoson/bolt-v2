...
- runtime-bearing NautilusTrader risk-engine defaults must be explicit in TOML and mapped into `LiveRiskEngineConfig`
- selected NautilusTrader live exec-engine defaults are explicit in TOML and mapped into `LiveExecEngineConfig`; the complete exec-engine default audit remains a separate production-readiness gate
