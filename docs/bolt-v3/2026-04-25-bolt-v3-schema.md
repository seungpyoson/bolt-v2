...
For Polymarket market forced exits, use `time_in_force = "ioc"` and `is_reduce_only = false`; pinned NautilusTrader's Polymarket adapter rejects market `gtc` and reduce-only orders before submit.
