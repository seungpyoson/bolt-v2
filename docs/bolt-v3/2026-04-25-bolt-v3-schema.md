...
The additive optional `realized_volatility_gate_result` and `realized_volatility_receive_watermark_ms` fields remain within schema 15. For legacy records that omit these keys, the receive watermark deserializes as `None`; the custom deserializers infer an `Accepted` gate result only when complete, valid legacy realized-volatility provenance supports that inference, and otherwise leave it as `None`. Older serde readers accept and ignore these unknown keys. No evidence migration or schema-version bump is required for this additive extension.
