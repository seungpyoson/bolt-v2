...
- `qsize` must equal the pinned NT `LiveDataEngineConfig::default().qsize` value, verified as `100000` at pinned NT rev `01818c1dbd577a73f5ac0d0272ca9c9befb4f03c`
...
| `qsize` | must equal the pinned NT `LiveDataEngineConfig::default().qsize` value, verified as `100000` at pinned NT rev `01818c1dbd577a73f5ac0d0272ca9c9befb4f03c` | `LiveDataEngineConfig.qsize` |
...
- `snapshot_positions = false`
- `purge_from_database = false`
- `qsize` must equal the pinned NT `LiveExecEngineConfig::default().qsize` value, verified as `100000` at pinned NT rev `01818c1dbd577a73f5ac0d0272ca9c9befb4f03c`

Every Bolt live configuration must give NT complete startup and continuous reconciliation authority;
...
represented in NT and declares all required reconciliation report capabilities
complete. Strict reconciliation failure fences risk-increasing execution
commands before client routing. After NT reaches `Running`, Bolt projects the canonical NT
...
| `qsize` | must equal the pinned NT `LiveExecEngineConfig::default().qsize` value, verified as `100000` at pinned NT rev `01818c1dbd577a73f5ac0d0272ca9c9befb4f03c` | `LiveExecEngineConfig.qsize` |
...
- `authorized_operator_ids` scopes manual reset evidence; `account_ids` and `instrument_ids` scope loss-protection position-event PnL when enabled, and all three arrays must be non-empty at enable time; the shipped `instrument_ids = []` is a disabled-template placeholder
...
- must equal the pinned NT `LiveRiskEngineConfig::default().qsize` value, verified as `100000` at pinned NT rev `01818c1dbd577a73f5ac0d0272ca9c9befb4f03c`
