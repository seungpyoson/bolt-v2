...
| R30 | Provider-specific adapter, secret, and client-registration leak coverage is physically verifier-backed for the current provider-boundary class. | `src/bolt_v3_adapters.rs`, `src/bolt_v3_secrets.rs`, `src/bolt_v3_client_registration.rs`, `scripts/verify_bolt_v3_provider_leaks.py` | Bolt-v3 maintainers | `scripts/verify_bolt_v3_provider_leaks.py` fails closed provider-adapter/secret/registration shapes outside approved provider-owned modules and is wired through `just source-fence`; broader provider-boundary refactors remain tracked by R3-R9. | Implemented verifier |
...
| V16 | R3-R9, R30 | Core/shared Bolt-v3 code contains no closed provider adapter enum, closed resolved-secret enum, concrete NT provider factory imports, adapter-core `MarketSlugFilter` construction, or core match on concrete provider keys outside approved provider-owned modules | Source scan with positive failing fixtures and path-scope coverage | `scripts/verify_bolt_v3_provider_leaks.py`; `scripts/test_verify_bolt_v3_provider_leaks.py`; `just source-fence` |
