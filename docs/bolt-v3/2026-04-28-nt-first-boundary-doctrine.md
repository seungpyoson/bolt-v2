...
| R30 | Provider-specific adapter, secret, and client-registration leaks now have a physical source verifier, but the provider-boundary refactor still remains. | `src/bolt_v3_adapters.rs`, `src/bolt_v3_secrets.rs`, `src/bolt_v3_client_registration.rs`, `scripts/verify_bolt_v3_provider_leaks.py` | Bolt-v3 maintainers | Provider-boundary slices must keep `scripts/verify_bolt_v3_provider_leaks.py` passing while retiring R3-R9. | Verifier implemented |
