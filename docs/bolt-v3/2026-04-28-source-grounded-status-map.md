...
| 2 | TOML-owned runtime configuration | Partial | `src/bolt_v3_config.rs` carries root, venue, strategy, timeout, persistence, risk, complete explicit NT data-engine fields, complete explicit NT exec-engine fields, and complete explicit NT risk-engine fields; `src/bolt_v3_live_node.rs` explicitly fixes the remaining top-level `LiveNodeConfig` residuals to disabled/empty current behavior | `tests/config_parsing.rs`; `src/bolt_v3_live_node.rs::live_node_config_maps_explicit_nt_runtime_defaults_from_v3_root`; Rust struct literal exhaustiveness for `LiveNodeConfig` | Full runtime hardcode audit is still missing, including logger sub-config defaults and non-v3 runtime paths. |
...
| 5 | Runtime values come from TOML | Partial | `src/bolt_v3_config.rs` carries timeout, persistence, risk, venue, strategy, complete explicit NT data-engine fields, complete explicit NT exec-engine fields, and complete explicit NT risk-engine fields; top-level `LiveNodeConfig` residuals are explicit disabled/empty settings instead of inherited defaults | Config parsing tests cover selected parser/validator cases; `src/bolt_v3_live_node.rs::live_node_config_maps_explicit_nt_runtime_defaults_from_v3_root` asserts the NT mapping | Need a verifier that enumerates string/numeric literals in `src/bolt_v3_*.rs` and maps runtime-bearing ones to TOML or accepted disabled defaults. Single-value enums (`RuntimeMode::Live`, `OmsType::Netting`, `CatalogFsProtocol::File`, `RotationKind::None`) are scope constraints that must be widened or explicitly accepted before dry-run/shadow modes. `RuntimeMode::Live`-only blocks rows 40 and 42 at the config level. |
