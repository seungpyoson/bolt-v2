...
5. `docs/bolt-v3/2026-05-18-production-readiness-contract.md`: Issue #369
   readiness-level contract separating tiny-canary, staged-live, and production
   live claims.
...
| 3 | No Python runtime layer | Implemented for current runtime boundary | `Cargo.toml`, `Cargo.lock`, and production `src/**/*.rs` carry no PyO3, maturin, cpython, or Python export bridge for the Rust binary | `scripts/verify_bolt_v3_pure_rust_runtime.py` | Python verifier scripts remain CI tooling only; this row proves the current Rust runtime metadata/source boundary, not absence of all Python automation in the repository. |
...
| 5 | Runtime values come from TOML | Partial | `src/bolt_v3_config.rs` carries timeout, persistence, risk, venue, strategy, complete explicit NT data-engine fields, complete explicit NT exec-engine fields, and complete explicit NT risk-engine fields; `LoggerConfig` and top-level `LiveNodeConfig` residuals are explicit accepted default or disabled/empty settings instead of inherited defaults; `docs/bolt-v3/research/runtime-literals/bolt-v3-runtime-literal-audit.toml` classifies current candidate runtime literals in production root `src/bolt_v3_*.rs` files and files under `src/bolt_v3_*` module directories | Config parsing tests cover selected parser/validator cases; `src/bolt_v3_live_node.rs::live_node_config_maps_explicit_nt_runtime_defaults_from_v3_root` asserts the NT mapping; `src/bolt_v3_live_node.rs::live_node_config_maps_explicit_logger_residuals_in_builder_path` asserts logger residuals; `scripts/verify_bolt_v3_runtime_literals.py` runs through `just verify-bolt-v3-runtime-literals` and `just fmt-check` | Literal verifier exists for production root `src/bolt_v3_*.rs` files and `src/bolt_v3_*` module directories; status remains partial because non-v3 runtime paths and accepted provider-boundary residual literals are still tracked separately. Single-value enums (`RuntimeMode::Live`, `OmsType::Netting`, `CatalogFsProtocol::File`, `RotationKind::None`) are scope constraints that must be widened or explicitly accepted before dry-run/shadow modes. `RuntimeMode::Live`-only blocks rows 40 and 42 at the config level. |
...
| 18 | LiveNode assembly without running trading | Partial | `src/bolt_v3_live_node.rs` builds `LiveNode`, exposes the gated `run_bolt_v3_live_node` runner wrapper, and delegates the fail-closed pre-run contract to `src/bolt_v3_live_canary_gate.rs` | `tests/live_node_run.rs::builds_bolt_v3_livenode_without_running_event_loop`, `tests/bolt_v3_client_registration.rs`, `tests/bolt_v3_live_canary_gate.rs`, `tests/bolt_v3_controlled_connect.rs::live_node_module_only_runs_nt_after_live_canary_gate` | `LiveNodeBuilder::build` is not purely passive: it parses Polymarket private key material into an NT signer and performs internal NT engine/message-bus subscriptions. It opens no network socket and does not run the event loop. The approved runner wrapper enters the NT event loop only after the live canary gate accepts the loaded config and no-submit readiness report. |
...
| 48 | Production live trading | Missing | `docs/bolt-v3/2026-05-18-production-readiness-contract.md` defines the claim levels and evidence gates; no production deployment path is accepted for Bolt-v3 | No production verification found | Not in scope until tiny-canary, staged-live runbooks, order lifecycle, reconciliation, monitoring, deploy provenance, panic gate, CLOB V2 readiness, and explicit operator approval are proven or waived. |
| 49 | Provider-leak verifier for adapter/secrets/registration | Implemented for current provider-boundary leak class | `scripts/verify_bolt_v3_provider_leaks.py` scans Bolt-v3 adapter, secret, client-registration, validation, and provider-binding paths for closed provider/family dispatch and concrete NT provider leaks outside approved binding modules | `scripts/test_verify_bolt_v3_provider_leaks.py`; `just source-fence` | Verifier exists and is wired into CI source-fence. The provider-boundary refactor itself remains tracked by rows 14-17; this row only proves the source-scan gate is present. |
