...
| 43 | CI compile/test gating | Partial | `.github/workflows/ci.yml` runs `just fmt-check`, `just deny`, `just clippy`, `just test`, and conditional `just build`; `justfile` defines those recipes, and `fmt-check` includes `verify-bolt-v3-nucleus-admission` in report-only mode | Existing checks do not exercise live trading, readiness, or production sockets | Track what CI proves separately from roadmap completeness. |
