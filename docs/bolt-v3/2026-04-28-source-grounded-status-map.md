...
| 2 | TOML-owned runtime configuration | Partial | `src/bolt_v3_config.rs` carries root, venue, strategy, timeout, persistence, risk, complete explicit NT exec-engine fields, and complete explicit NT risk-engine fields | `tests/config_parsing.rs`; `src/bolt_v3_live_node.rs::live_node_config_maps_explicit_nt_runtime_defaults_from_v3_root` | Full runtime hardcode audit is still missing. |
