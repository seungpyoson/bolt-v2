...
- Upstream NautilusTrader commit
  `56a438216442f079edf322a39cdc0d9e655ba6d8` is reported to migrate the
  Polymarket adapter to CLOB V2 (2026-04-28 per upstream history; not verified
  inside this repo). Bolt is pinned to
  `48d1c126335b82812ba691c5661aeb2e912cde24`, so the new commit is a
  pin-change audit input, not current Bolt behavior.
...
| 2 | TOML-owned runtime configuration | Partial | `src/bolt_v3_config.rs` carries root, venue, strategy, timeout, persistence, and risk fields | `tests/config_parsing.rs` | Full runtime hardcode audit is still missing. |
...
| 5 | Runtime values come from TOML | Partial | `src/bolt_v3_config.rs` carries timeout, persistence, risk, venue, and strategy fields | Config parsing tests cover selected parser/validator cases; no full runtime literal-to-TOML audit exists | Need a verifier that enumerates string/numeric literals in `src/bolt_v3_*.rs` and maps runtime-bearing ones to TOML. Single-value enums (`RuntimeMode::Live`, `OmsType::Netting`, `CatalogFsProtocol::File`, `RotationKind::None`) are scope constraints that must be widened or explicitly accepted before dry-run/shadow modes. `RuntimeMode::Live`-only blocks rows 40 and 42 at the config level. |
...
| 46 | Polymarket CLOB V2 readiness gate | Missing blocker for current provider | Contract ledger marks CLOB V2 readiness as blocker; external reference cites upstream NT commit `56a438216442f079edf322a39cdc0d9e655ba6d8` as the Polymarket CLOB V2 migration; this commit is not present in Bolt's pinned NT checkout (`48d1c126...`) | No Bolt pin-change audit or Bolt-side CLOB V2 verification evidence found in this status pass | Upstream support appears available after Bolt's current NT pin. Required next step is a dedicated NT pin-change audit: bump candidate pin, verify Rust API drift, CLOB V2 signing/order/collateral behavior, and all Bolt adapter/secret/registration/readiness impacts. |
...
- The next implementation work should be another narrow hardcode-breaking slice,
  not a new trading feature.
