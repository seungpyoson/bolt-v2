...
| 2 | TOML-owned runtime configuration | Partial | `src/bolt_v3_config.rs` carries root, venue, strategy, timeout, persistence, risk, and selected explicit NT live runtime fields | `tests/config_parsing.rs`; `src/bolt_v3_live_node.rs::live_node_config_maps_explicit_nt_runtime_defaults_from_v3_root` | Full runtime hardcode audit is still missing. |
...
| 5 | Runtime values come from TOML | Partial | `src/bolt_v3_config.rs` carries timeout, persistence, risk, venue, strategy, and selected explicit NT exec/risk runtime fields | Config parsing tests cover selected parser/validator cases; `src/bolt_v3_live_node.rs::live_node_config_maps_explicit_nt_runtime_defaults_from_v3_root` asserts the NT mapping | Need a verifier that enumerates string/numeric literals in `src/bolt_v3_*.rs` and maps runtime-bearing ones to TOML. Complete NT exec/risk-engine default ownership remains a follow-up. Single-value enums (`RuntimeMode::Live`, `OmsType::Netting`, `CatalogFsProtocol::File`, `RotationKind::None`) are scope constraints that must be widened or explicitly accepted before dry-run/shadow modes. `RuntimeMode::Live`-only blocks rows 40 and 42 at the config level. |
