...
This proves the market-selection/order-boundary event subset and direct catalog handoff. It does not prove evaluation events or order-submit gating. The next F7 implementation slice must add `entry_evaluation` and `exit_evaluation` only after their reference/risk facts are accepted, then wire the v3 order-intent path so order submit is blocked when event construction, registration, encoding, or handoff fails.
