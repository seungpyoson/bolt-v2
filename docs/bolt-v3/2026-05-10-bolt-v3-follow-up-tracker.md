...
This now proves the market-selection, evaluation, order-submission, pre-submit-rejection event subset, direct catalog handoff, and a gate helper that refuses to call entry/exit submit closures when order-submission event handoff fails. It does not prove actual strategy submit wiring. The next F7 implementation slice must wire the v3 order-intent path through this gate before NT submit.
