...
This now proves the market-selection, evaluation, order-submission, pre-submit-rejection event subset, direct catalog handoff, a gate helper that refuses to call entry/exit submit closures when order-submission event handoff fails, and common event-field construction from v3 TOML plus supplied release identity. It does not prove actual strategy submit wiring. The next F7 implementation slice must wire the v3 order-intent path through this context and gate before NT submit.
