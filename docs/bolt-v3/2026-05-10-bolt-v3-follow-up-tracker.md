...
This now proves the market-selection, evaluation, order-submission, pre-submit-rejection event subset and direct catalog handoff. It does not prove order-submit gating. The next F7 implementation slice must wire the v3 order-intent path so order submit is blocked when event construction, registration, encoding, or handoff fails.
