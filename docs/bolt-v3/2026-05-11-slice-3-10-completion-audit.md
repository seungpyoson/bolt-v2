...
Date: 2026-05-11
Branch context: `codex/bolt-v3-market-selection-failure-event`
Base chain head: `origin/codex/bolt-v3-market-selection-event`
...
| --- | --- | --- |
| Use fresh branches/worktrees from origin | Current branch `codex/bolt-v3-market-selection-failure-event` was created in `.worktrees/bolt-v3-market-selection-failure-event` from pushed origin ref `origin/codex/bolt-v3-market-selection-event` | met |
| Commit and push each slice | Prior slices remain pushed separately; current failed market-selection event slice is scoped to this branch; remote SHA verification must be checked after push | met-local |
...
| F7 decision-event persistence | Follow-up TDD probes prove registered custom data with `event_facts: Params` preserves explicit JSON null through NT local catalog; `tests/bolt_v3_decision_event_handoff.rs` proves market-selection, entry/exit evaluation, entry/exit order submission, and entry/exit pre-submit rejection events write through the canonical NT catalog handoff; `tests/bolt_v3_order_intent_gate.rs` proves entry/exit submit closures are blocked when order-submission event handoff fails; `tests/bolt_v3_decision_event_context.rs` proves common fields derive from loaded v3 TOML plus supplied release identity; `tests/bolt_v3_release_identity.rs` proves release identity loads from configured manifest path with config-hash and NT-pin verification; `src/platform/polymarket_catalog.rs` unit coverage proves Gamma candidate translation carries selected-market slug, question ID, selected-market observed timestamp, end timestamp, price-to-beat value, price source, and price observed timestamp into `CandidateMarket`; `tests/eth_chainlink_taker_runtime.rs` proves existing-strategy accepted market-selection, failed no-selected-market market-selection, accepted entry-evaluation, and entry order-submission events are written without unintended NT submit, proves insufficient-edge no-action entry-evaluation emits without NT submit, proves forced-flat exit-evaluation emits before exit order-submission with the same decision trace, proves actual entry pre-submit rejection for `instrument_missing_from_cache` emits without NT submit, proves actual exit pre-submit rejection for `exit_price_missing` emits without NT submit, proves exit order-submission event wiring, and proves failed decision-evidence write blocks entry submit | selected-market fact carriage, accepted market-selection event emission, failed no-selected-market market-selection event emission, accepted entry-evaluation, insufficient-edge no-action entry-evaluation, forced-flat exit-evaluation, one actual entry pre-submit rejection, one actual exit pre-submit rejection, and order-submission wiring verified; other failed market-selection reasons, other no-action entry reasons, other entry pre-submit rejection reasons, other exit-evaluation reasons, and other exit pre-submit rejection reasons remain incomplete |
...
- F7: nullable-field encoding unblocked by NT `Params` proof; `market_selection_result`, `entry_evaluation`, `exit_evaluation`, entry/exit order submission, entry/exit pre-submit rejection, direct catalog handoff, order-submission gate helper, common event-field context, release-identity manifest loading, selected-market fact carriage, actual existing-strategy accepted market-selection event emission, failed no-selected-market market-selection event emission, accepted entry-evaluation plus entry order-submission wiring, insufficient-edge no-action entry-evaluation without NT submit, forced-flat exit-evaluation before exit order-submission with the same decision trace, actual entry pre-submit rejection for `instrument_missing_from_cache` without NT submit, actual exit pre-submit rejection for `exit_price_missing` without NT submit, and exit order-submission wiring are verified locally. Other failed market-selection reasons, other no-action entry reasons, other entry pre-submit rejection reasons, other exit-evaluation reasons, and other exit pre-submit rejection reasons remain unverified.
...
The next implementation decision should not start F9 or F10. With selected-market fact carriage, accepted market-selection event emission, failed no-selected-market market-selection event emission, one actual entry pre-submit rejection, and one actual exit pre-submit rejection now covered, the next credible F7 slice is broader failed market-selection coverage, broader entry rejection coverage, broader exit rejection coverage, or other exit-evaluation reasons through the same decision-evidence context. Still no production start/run wrapper and no live orders.
