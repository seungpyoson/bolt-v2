...
The next implementation decision should not start F9 or F10. With one actual entry pre-submit rejection and one actual exit pre-submit rejection now covered, the next credible F7 slice is market-selection emission, broader entry rejection coverage, broader exit rejection coverage, or other exit-evaluation reasons through the same decision-evidence context. Still no production start/run wrapper and no live orders.
