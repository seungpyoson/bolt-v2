...
| Commit and push each slice | Current reference-contract slice is committed on `codex/bolt-v3-reference-contract` and pushed to origin; prior slices remain pushed separately | met |
...
| Verification before claims | Focused reference-contract tests and `cargo test --tests --no-run` passed locally before push; whitespace and marker scans were clean | met locally |
