...
The next implementation decision should not start F9 or F10. The next credible slice is still upstream of live order lifecycle: finish F7 evaluation events only after reference/risk facts are accepted, then add v3 order-intent submit-blocking integration.
