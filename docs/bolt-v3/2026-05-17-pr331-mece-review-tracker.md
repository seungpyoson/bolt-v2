...
| P1-E (Claude P1-4, Kimi P1-004) | Claude NB, Kimi FYI | NON_BLOCKING | Stale path references in `docs/bolt-v3/research/nt-pin-change/**` and `docs/bolt-v3/2026-05-15-price-data-adapter-bridge-plan.md`. Historical research/planning docs. | open | Deferred to follow-up docs-hygiene PR. Listed in `023d1214` commit message as "not addressed in this commit (intentional)". |
...
| P3-FYI-1 | Claude F5 (NB-debatable), Claude F6 (FYI), DeepSeek FYI-padding | FYI — out-of-scope | (a) Polymarket wallet swap requires editing both `[venues.polymarket_main.secrets]` (SSM paths) and `[venues.polymarket_main.execution].funder_address` (public routing identifier) — group-by-change interpretation; defense: funder_address is a public value, not a credential. (b) `normalize_api_secret_padding` over-pads already-padded base64 input. Data-correctness, not P3 secret-source scope. | **noted** | Both observations recorded for future operator-docs / config-strictness work; neither violates the P3 secret-source invariant set. |
