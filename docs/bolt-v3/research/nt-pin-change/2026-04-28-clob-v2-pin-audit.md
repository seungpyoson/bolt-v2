...
- `BinanceDataClientConfig` gained `transport_backend`; `BinanceExecClientConfig` gained `transport_backend` and `use_trade_lite` defaulting to `false`.
...
Branch/tag containment is audit-time evidence; re-run containment checks before any probe because upstream refs can move.
...
The `/tmp` clone above was present during audit review; `git -C /tmp/nt-pin-change-audit-nautilus-full-20260428 cat-file -t 56a438216442f079edf322a39cdc0d9e655ba6d8` resolved to `commit`, confirming the candidate was reachable from that clone.
...
| --- | --- | --- |
| CLOB V2 signing | Signing `DOMAIN_VERSION` changes to `"2"` while `CLOB_AUTH_DOMAIN_VERSION` remains `"1"`; CTF exchange addresses change to V2 addresses; EIP-712 `Order` drops `taker`, `expiration`, `nonce`, and `feeRateBps`; adds `timestamp`, `metadata`, and `builder`. | This is the direct CLOB V2 blocker reducer. Bolt does not build Polymarket signed orders itself, so the signing fix is inside NT once pinned. |
...
| Collateral currency | NT adds `Currency::pUSD()` and Polymarket adapter internals move from `USDC` to `PUSD`. Global `Currency::USDC()` remains available at the candidate; only the Polymarket adapter's internal string constant changes. Balances and fills now use pUSD. | Bolt docs and risk fields currently describe gross USDC entry-cost terms. Runtime semantics must decide whether Bolt-facing names stay "USDC terms" or become pUSD collateral terms. |
...
| CLOB URLs | Default HTTP CLOB URL changes from `https://clob.polymarket.com` to `https://clob-v2.polymarket.com`; upstream `urls.rs` comment says `https://clob.polymarket.com` also serves V2 and NT plans to flip the default back in a follow-up, making the V2-specific hostname temporary. | Bolt currently requires explicit URLs and docs/fixtures use `https://clob.polymarket.com`. Because Bolt overrides defaults, it will not inherit NT's temporary default CLOB V2 preview host unless config/docs change, but the canonical host may remain valid if upstream flips back. |
...
| Live runtime configs | `LiveNodeConfig::validate_runtime_support()` adds Rust-live-runtime gates; `LiveDataEngineConfig`, `LiveRiskEngineConfig`, and `LiveExecEngineConfig` expand materially. `LiveRiskEngineConfig` now includes safety-relevant `bypass` and `max_notional_per_order` fields. `LiveExecEngineConfig` defaults change `max_single_order_queries_per_cycle` from `5` to `10` and `position_check_threshold_ms` from `60_000` to `5_000`. `InstrumentProviderConfig.load_ids` becomes `Option<Vec<String>>`; `filters` becomes `HashMap<String, serde_json::Value>`. | Bolt mostly uses `..Default::default()` here, so this is more behavior/config drift than direct compile drift. The probe must verify inherited runtime defaults and validation gates, not just Polymarket adapter fields. Specifically verify `LiveRiskEngineConfig.bypass = false`, `max_notional_per_order` default behavior, and `LiveExecEngineConfig.reconciliation_startup_delay_secs` passing the new non-negative-finite validation gate. Bolt does not construct `InstrumentProviderConfig` directly at audit time. |

Symbol breadcrumbs for review: `eip712.rs::DOMAIN_VERSION`, `eip712.rs::CLOB_AUTH_DOMAIN_VERSION`, `eip712.rs::Order`, `http/models.rs::PolymarketOrder`, `consts.rs::PUSD`, `urls.rs::CLOB_HTTP_URL`, `currencies.rs::Currency::USDC`, `currencies.rs::Currency::pUSD`, `execution/parse.rs::compute_commission`, `execution/submitter.rs::MarketBuyFeeContext`, `http/parse.rs::fee_schedule`, `http/parse.rs::game_id`, `reports/fill.rs::FillReport::avg_px`, `live/config.rs::LiveNodeConfig::validate_runtime_support`, `live/config.rs::LiveExecEngineConfig::max_single_order_queries_per_cycle`, `live/config.rs::LiveExecEngineConfig::position_check_threshold_ms`, and `live/config.rs::InstrumentProviderConfig`.
...
| `src/bolt_v3_client_registration.rs` | Polymarket factory names still exist, but test direct data config literal breaks. | Main path passes cloned configs into `LiveNodeBuilder::add_*`; that API remains. Inline test literal must add new Polymarket data config fields. |
| `src/bolt_v3_live_node.rs` | Likely compiles for builder/logging at candidate because the builder setter methods Bolt calls still exist and logging uses `..Default::default()`. Behavior and validation must be rechecked. | `LiveNodeBuilder::build` now calls `LiveNodeConfig::validate_runtime_support()`, which rejects unsupported `msgbus`, `streaming`, `emulator`, `loop_debug`, `LoggerConfig.file_config`, logging clear-file behavior, unsupported engine `qsize` overrides, unsupported exec snapshots/purge flags, invalid risk rate limits, invalid time-bar origins, and hostile reconciliation values. Additional sub-gates: `LiveExecEngineConfig.reconciliation_startup_delay_secs` must be non-negative finite, and `LiveRiskEngineConfig.max_order_submit_rate` / `max_order_modify_rate` must parse as rate-limit strings. Current Bolt defaults appear to stay on the accepted path but the probe must verify this. |
...
- `nautilus_system::factories` import sites: `src/clients/mod.rs`, `src/live_node_setup.rs`, `src/clients/chainlink.rs`, `tests/support/mod.rs`, `tests/reference_pipeline.rs`, `tests/platform_runtime.rs`, and `src/platform/runtime.rs`. Candidate only exposes these traits from `nautilus_common::factories`.
- Direct `BinanceDataClientConfig { ... }` literal sites: `src/clients/binance.rs` and `src/bolt_v3_adapters.rs`. Both must add `transport_backend`.
- `src/clients/polymarket.rs` constructs `PolymarketDataClientConfig` with `..Default::default()`. Under the candidate this silently inherits `auto_load_missing_instruments = true` unless explicitly overridden. Treat this as legacy/shared behavior risk if that path remains callable during a probe.
...
| Legacy/shared Polymarket path | `src/clients/polymarket.rs` uses `..Default::default()` for `PolymarketDataClientConfig`. | If this path remains callable in the probe, explicitly set `auto_load_missing_instruments = false`; otherwise document that the probe is Bolt-v3-only and does not validate this legacy/shared path. |
| Live runtime defaults/gates | Candidate expands live engine configs and changes inherited exec reconciliation defaults. | Verify the probe still maps Bolt TOML into supported Rust-live-runtime config. Record any accepted inherited defaults, especially risk-engine `bypass = false`, `max_notional_per_order`, exec reconciliation timing, and unsupported config gates. |
| Exec engine reconciliation cadence | Candidate flips defaults: `max_single_order_queries_per_cycle` from `5` to `10`, and `position_check_threshold_ms` from `60_000` to `5_000`. | Decide whether Bolt's `..Default::default()` should inherit the more aggressive V2 cadence or explicitly pin prior values to retain V1 reconciliation behavior. |
| Binance/reference-data drift | Candidate changes Binance adapter config and carries broader Binance adapter diffs; current Bolt has `src/clients/binance.rs`. | Treat the first probe as compile/config compatibility only. It does not validate Binance runtime behavior unless separately scoped. |
...
Also recheck any signing/replay verifier or fixture that assumes V1 signed-order shape: V2 keeps `expiration` in the wire body but excludes it from the EIP-712 signed hash. Audit-time grep found no current Bolt V1 EIP-712 hash fixture, so treat this as a forward-looking invariant for future replay/verifier work.
...
The probe must not be based on `main`; `main` is missing the slice-8-1 / market-identity / source-grounded-status-map work represented by this branch. Local evidence at audit time: `git log --oneline main..feat/236-bolt-v3-slice-8-1-baseline` listed the baseline commits, including `0342346 docs(bolt-v3): add source-grounded status map`, while `git log --oneline feat/236-bolt-v3-slice-8-1-baseline..main` was empty.
...
- Update only mechanical compile fallout:
  ...
  - new Binance `transport_backend` fields where existing direct literals break; `BinanceExecClientConfig` also adds `use_trade_lite`, so any future exec literal must set or consciously inherit the V2 baseline `false`
  ...
  - Live runtime config compatibility checks for `validate_runtime_support()` and inherited live engine defaults
