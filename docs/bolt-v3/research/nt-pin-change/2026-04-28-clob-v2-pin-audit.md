...
The `/tmp` clone above was present during audit review and contained the candidate commit.
...
| Live runtime configs | `LiveNodeConfig::validate_runtime_support()` adds Rust-live-runtime gates; `LiveDataEngineConfig`, `LiveRiskEngineConfig`, and `LiveExecEngineConfig` expand materially. `LiveExecEngineConfig` defaults change `max_single_order_queries_per_cycle` from `5` to `10` and `position_check_threshold_ms` from `60_000` to `5_000`. `InstrumentProviderConfig.load_ids` becomes `Option<Vec<String>>`; `filters` becomes `HashMap<String, serde_json::Value>`. | Bolt mostly uses `..Default::default()` here, so this is more behavior/config drift than direct compile drift. The probe must verify inherited runtime defaults and validation gates, not just Polymarket adapter fields. |

Symbol breadcrumbs for review: `eip712.rs::DOMAIN_VERSION`, `eip712.rs::CLOB_AUTH_DOMAIN_VERSION`, `eip712.rs::Order`, `http/models.rs::PolymarketOrder`, `consts.rs::PUSD`, `urls.rs::CLOB_HTTP_URL`, `currencies.rs::Currency::pUSD`, `execution/parse.rs::compute_commission`, `execution/submitter.rs::MarketBuyFeeContext`, `http/parse.rs::fee_schedule`, `http/parse.rs::game_id`, `reports/fill.rs::FillReport::avg_px`, `live/config.rs::LiveNodeConfig::validate_runtime_support`, and `live/config.rs::InstrumentProviderConfig`.
