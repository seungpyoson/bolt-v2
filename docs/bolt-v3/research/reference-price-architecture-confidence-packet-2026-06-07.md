...
- `[reference_price]` is strategy-scoped. Prompt defaults for omitted `min_valid_sources`, `enabled`, and `required` are applied through a custom TOML wire-deserializer; no `#[serde(default)]` legacy defaults are used.
...
- Strategy handling keeps Chainlink resolution strike isolated to `price_to_beat`; reference-price custom data updates separate reference-price state and source health.
...
|---|---|---|
| 1 | Current config parse path for strategy config and root/shared provider config | Root config owns `clients` and `gate_providers`; strategy config owns `reference_data`, `signal_data`, and `resolution_data` (`src/bolt_v3_config.rs:70-71`, `src/bolt_v3_config.rs:372-379`). `ClientBlock` already separates `data`, `execution`, and `secrets` (`src/bolt_v3_config.rs:270-277`). `load_bolt_v3_config` reads the root TOML, strategy files, then validates root and strategies (`src/bolt_v3_config.rs:452-506`). New `reference_price` belongs in the strategy config; provider endpoints and SSM paths stay in root client blocks. |
...
| 3 | Current strategy subscription/update path | `on_start` subscribes reference and signal quotes, and `on_stop` unsubscribes them (`src/strategies/binary_oracle_edge_taker/mod.rs:4108-4175`). Reference quotes use `subscribe_quotes`; resolution strike uses `subscribe_index_prices` with `window_open_unix_seconds` params (`src/strategies/binary_oracle_edge_taker/mod.rs:1190-1290`). `on_quote` routes quote ticks to `observe_reference_quote`; `on_index_price` routes Chainlink strike `IndexPriceUpdate` to `observe_resolution_strike` (`src/strategies/binary_oracle_edge_taker/mod.rs:4137-4168`). New reference-price updates must use custom data, not the existing strike index-price path. |
...
| 8 | Current handling of `reference_data` and why preserved/non-overlap | `reference_data` is an existing quote subscription mapping with at most one role for this archetype (`src/bolt_v3_archetypes/binary_oracle_edge_taker.rs:1217-1248`, `src/bolt_v3_archetypes/binary_oracle_edge_taker.rs:1347-1355`). Tests confirm it maps to `reference_venue`/`reference_instrument_id` and is independent of `decision_reference` (`tests/bolt_v3_strategy_registration.rs:1236-1325`). New `[reference_price]` must remain separate from existing `[reference_data]`; `reference_data` stays the quote/fair-value input. |
...
| 10 | Exact strategy-local state/update method for `reference_price` | Pre-code audit found no production `reference_price` strategy state name. Current implementation adds explicit reference-price state and custom-data handling, while preserving existing `reference_data` quote/fair-value state and `price_to_beat` resolution-strike state as separate paths. |
...
- `reference_price` separation is proven by current absence of a strategy state name and by the existing `reference_data` quote path.
