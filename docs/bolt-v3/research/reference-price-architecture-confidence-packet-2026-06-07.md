...
- `apiKey` appears only in PolyResearch legacy-query rejection guards/tests; `/bolt/polyresearch/api-key` appears only as an SSM parameter-name fixture and is intentionally not a testnet path. Secret-source search hits otherwise remain existing guard/tests/protocol strings, not new fallback paths.
