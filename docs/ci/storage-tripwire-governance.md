...
That TOML owns the operator storage cap, cap source, fixed thresholds, owner, escalation text, issue labels, marker format, issue-matching limits, issue listing page size, workflow contract, and update cadence for the scheduled storage tripwire. Do not duplicate those values in this document.
...
The issue-writing workflow is schedule-only. It must not expose `workflow_dispatch`, because manual dispatch can load branch-selected workflow YAML before a job-level guard runs.
...
The workflow verifier enforces the storage tripwire workflow as a closed contract: the configured trigger set, single schedule entry, top-level keys, top-level permissions, concurrency, schedule-only job guard, runner, single configured job, allowed job keys, and exact checkout/run step shape must all match TOML-owned policy.
