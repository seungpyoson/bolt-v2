...
- Treat ready-PR full CI as a high-cost proof run for the intended merge candidate, not a debug loop. Use draft deferred runs or `just rust-probe` (max two, per the [Rust Probe Policy](../../AGENTS.md#rust-probe-policy)) for mid-iteration feedback.
