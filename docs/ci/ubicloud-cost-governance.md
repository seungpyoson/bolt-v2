...
Deferred draft CI keeps the required `gate` / `backtester-gate` check names. A draft PR push skips the root full-CI merge-proof lanes (build, test, check-aarch64, source-fence, nextest fingerprint/reuse, test-archive — gated on `full_ci_required`, with runner tiers that vary) but still runs the always-on lanes (clippy on `managed_heavy`, deny on `managed_light`) for feedback. The root gate may report success only when it verifies prior same-SHA provenance against the current base; otherwise it fails closed. The backtester gate does not carry proof forward: it recomputes the crate lanes when the crate changed, or verifies those lanes skipped when the crate did not change. Manual `workflow_dispatch` feedback runs keep non-required names (`gate-dispatch` / `gate-iteration`) and cannot satisfy merge readiness. Use `just rust-probe suggest` for debugging, then run `just verify-remote` for full feedback or mark the PR ready for merge proof.
