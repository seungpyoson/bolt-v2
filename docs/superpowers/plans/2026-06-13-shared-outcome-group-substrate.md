...
- [ ] Add a metadata-fingerprint exclusion test: two groups with identical venue identity/grouping metadata but different order constraints, freshness windows, notional caps, or submit modes must have the same `metadata_fingerprint`; changing a covered venue identity or grouping field must change it.
