...
- Builds use `--locked --no-default-features --features s3 --release` and native `aarch64-unknown-linux-musl` or `x86_64-unknown-linux-musl` targets.
...
def test_verify_release_record_requires_exact_immutable_assets(self) -> None:
    ...
    with self.assertRaisesRegex(ValueError, "release is not immutable"):
        verify_release_record(expected, release, attestations={})
```
...
REQUIRED_CANDIDATE_KEYS = {
    "schema_version", "repository", "run_id", "head_sha", "architecture",
    "target", "replica", "source_commit", "source_sha256", "patch_sha256",
    "container", "rustc_release", "rustc_commit", "binary_name",
    "binary_sha256", "binary_size",
}
```

`validate_publish_context()` must require `event_name == "workflow_dispatch"`, `requested_sha == event_sha == remote_main_sha`, `event_ref == "refs/heads/main"`, immutable releases enabled, and live environment JSON for `strict-sccache-publisher` whose deployment policy admits protected branches. A missing environment is failure; the workflow must not rely on GitHub implicitly creating it. `verify_release_record()` must require the exact tag target, `draft == false`, `immutable == true`, the exact three assets (ARM64 binary, X64 binary, provenance JSON), matching `sha256:` API digests, and at least one release attestation record per binary digest.
...
```bash
cargo test --locked --no-default-features --features s3 strict_ -- --nocapture
```
...
- [ ] **Step 7: Add stdin and output-producing equivalence tests**

Run both official and patched binaries against commands equivalent to:
...
Assert equal exit status, stdout, stderr, and output bytes; assert the strict server records `CannotCache("-")` or `NotCompilation` before client execution.
...
- [ ] **Step 9: Add a small real-process Cargo integration test**

Add `tests/strict_system.rs` with an in-process, test-only S3 HTTP stub bound to a random loopback port. The stub implements the exact HEAD/GET/PUT responses exercised by OpenDAL, records method/key/count, and can inject read, write, timeout, and corrupt-object responses. Launch the patched server with the stub endpoint and required strict timeout environment, then run a temporary Cargo crate through the patched wrapper. Prove one genuine miss and completed write, one hit, one link or probe classification, one stdin-fed classification, read-only zero PUTs, server death before a second compile causing failure, and stats remaining available from the original server. The test-only stub is not compiled into the release binary and is not an alternate runtime backend.
...
```bash
cargo fmt --all -- --check
cargo test --locked --no-default-features --features s3 --lib --tests
cargo build --locked --no-default-features --features s3 --release
```
...
- Uses `actions/checkout@de0fac2e4500dabe0009e67214ff5f5447ce83dd`, `actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a`, and `actions/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c`.

- [ ] **Step 1: Add failing workflow-context and manifest tests**

Test publish refusal for PR events, non-main refs, stale requested SHA, cross-run artifacts, duplicate replicas, wrong architecture, wrong container/toolchain, pre-existing release namespace, disabled immutability, absent environment marker, and mismatched API digest.
...
For PRs, preflight binds builders to `github.sha` and forces `publish=false`. For dispatch, `publish=true` requires a 40-character lowercase SHA equal to `github.sha`, `refs/heads/main`, and the SHA returned by `GET /repos/{repository}/branches/{default_branch}` using API version `2026-03-10`. Preflight also reads `GET /repos/{repository}/environments/strict-sccache-publisher` and `GET /repos/{repository}/immutable-releases`; it emits `publisher_ready=true` only after `validate-publish-context` accepts both live documents. The publish job's `if` condition requires that output, preventing a missing environment from being implicitly created by the honest workflow.
...
5. starts the pinned container once with network to run `cargo fetch --locked --target <triple>` into a fresh mounted `CARGO_HOME`;
6. starts the same container with `--network none`, read-only `CARGO_HOME`, fresh target directory, `CARGO_NET_OFFLINE=true`, `CARGO_INCREMENTAL=0`, empty `RUSTC_WRAPPER`, fixed locale/timezone/umask, fixed mount paths, and `SOURCE_DATE_EPOCH=1781869188`;
7. runs the strict upstream tests and release build;
8. verifies ELF machine, static musl linkage, and `sccache 0.16.0` on the host outside the container, then proves `--start-server` fails without governed S3 configuration instead of selecting a local cache;
...
It downloads only artifacts from its own `needs` graph, recomputes all four binary digests, validates manifests, and refuses an existing tag/ref/release/draft. Immediately before creation it rechecks current main and `GET /repos/{repository}/immutable-releases`. It uses `curl --fail-with-body` with `Authorization: Bearer $GITHUB_TOKEN`, `Accept: application/vnd.github+json`, and `X-GitHub-Api-Version: 2026-03-10` only in the release commands. It creates a draft, uploads the two agreed binaries and canonical provenance manifest, publishes, then reads back the release and user attestation endpoints and calls `verify-release-record`.
...
Before the first publication dispatch, the repository owner must enable immutable releases and create the `strict-sccache-publisher` environment restricted to `main`. Publication fails while either control is absent.
```
