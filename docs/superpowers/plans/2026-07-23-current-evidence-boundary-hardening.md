...
- [x] **Step 1: Write failing storage-state tests**
...
- [x] **Step 3: Implement the minimal sink state**
...
- [x] **Step 1: Write failing observation startup tests**
...
- [x] **Step 3: Generalize validation**
...
- [x] **Step 4: Construct pre-poisoned observation state**
...
- [x] **Step 3: Implement one component walk**
...
- [x] **Step 4: Delete the competing pathname authority**
...
- Modify: the 26 existing identity files in `tests/fixtures/bolt_v3/current_evidence/positive/`, preserving their current filenames and identity mapping.
...
- [x] **Step 1: Write a failing coverage test**
...
- [x] **Step 3: Make frozen enum coverage exhaustive**
...
- [x] **Step 4: Build linear typed cases and committed bytes**
...
- [x] **Step 1: Run the three known tests and confirm RED**
...
- [x] **Step 2: Replace whole-stream assumptions with phase checkpoints**
...
- [x] **Step 3: Assert typed/stable failure semantics**
...
- [x] **Step 1: Extend existing advisory jobs**
...
- name: test (backtesting vertical slice)
  if: ${{ !cancelled() }}
  working-directory: crates/backtesting-vertical-slice
  run: mold -run cargo nextest run --locked --no-fail-fast
...
Split root and nested formatting into separate steps in the existing job, with the nested step
running whenever the job is not cancelled. Use the same non-cancelled guard for nested clippy,
tests, and build so a root failure cannot hide backtesting evidence. Do not create a second
workflow or verification manifest.
...
- [x] **Step 1: Update active operational documentation**
...
- [x] **Step 2: Run cheap local verification**
...
- [x] **Step 4: Commit documentation and deterministic outputs**
