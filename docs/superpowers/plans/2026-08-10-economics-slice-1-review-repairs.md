...
- [ ] Add an exhaustive generation-checked exit-attempt reducer: arm `ExitAttempting` with `LocalExitAuthority` and its opaque lease before routing; every non-submitted outcome returns the same generation to `Managed` and drops that lease; only `Submitted` commits `ExitPending(LocallySubmitted)` and carries it forward; a synchronous callback that advances state transfers the authority and cannot be overwritten or de-authorized by the route return. The shared recovered-exit constructor is the sole `ExitPending(Recovered)` constructor for startup and running fill-void causes and otherwise selects `ExitAuthorityRecoveryHold`. Replace the current shadow latch test with `PolicySkipped` evidence, zero sink/capacity mutation, `Managed` exposure, no retained feed key, and a later eligible exit attempt.
