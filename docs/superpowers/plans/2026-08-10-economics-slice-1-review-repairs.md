...
- Test: edge-taker order/admission/evidence/adverse-path suites, maker dispatch/runtime suites, kill-switch/live-node suites, position-authority feed tests, and evidence codec/round-trip suites
...
- Replaces `Result<BoltV3SubmitRoutingOutcome>` with one opaque private-field `BoltV3SubmitAttemptOutcome` and exhaustive public `BoltV3SubmitAttemptKind` discriminant that classify route validation, intent-evidence rejection, admission rejection, policy skip, pre-sink rejection, sink rejection, and submission at their source. Only shared order execution constructs outcomes/linkage. Direct taker, resting-submit, maker command/runtime, and kill-switch/live-node adapters preserve the outcome end to end; there is no compatibility result path, submit-shaped `Result<()>` above the raw `BoltV3NtVenueMutationSink` leaf, unconditional caller-side `Submitted`, or string/downcast classification.
...
- Produces one bounded `BoltV3PositionAuthorityFeed` over pinned NT's raw `PositionStatusReport` topic. LiveNode owns its single RAII subscription, the strategy context carries an opaque capability, and shared order execution owns non-cloneable active-key leases begun when an exit attempt is armed before any live sink, plus the complete-fill/report-and-cache release predicate; the edge-taker owns only the resulting local exposure-state transition.
...
- [ ] Add the shared position-authority feed at the LiveNode composition boundary before strategy contexts are built. Subscribe once to `MessagingSwitchboard::reconciliation_raw_position_status_report_topic()` under an RAII guard and derive exact execution-client/account/venue report keys from loaded bindings, failing ambiguous composition. Shared execution begins an opaque non-cloneable lease when `ExitAttempting` is armed before any live sink; non-submitted outcomes drop it, `Submitted` carries it into `ExitPending`, and a synchronous callback transfers it without letting the stale route return destroy it. Reports without an active key are discarded, and dropping the last lease deletes that key's snapshot and health. For active keys retain exact report identity/account/instrument/optional venue-position/signed quantity/side/timestamps plus a checked local generation. Implement one exhaustive lease-state reducer: `Awaiting`, `Coherent`, and monotonic `Conflicted`. The first post-lease report admits; identical ID/body dedupes without increment; same ID/different body conflicts; lower `ts_last` exposes typed stale health without changing authority; equal timestamp with conflicting signed state conflicts; distinct coherent equal-or-newer state admits with one checked increment; checked-generation overflow conflicts. A conflicted lease never releases. Reuse the registered `PolymarketNtExecutionReconciliation` provider-boundary row; do not add a provider parser or strategy-owned report store.
- [ ] Implement a typed `PositionReductionFence` in `TerminalFillAwaitingPosition` carrying account/instrument/position and order identity, original signed quantity/side, compiled and cumulatively filled quantities, the complete set of target-order fill trade/event identities, terminal timestamp, the already-active position-authority lease, and its current coherent generation captured as the terminal floor. Checked signed arithmetic derives the maximum same-side residual. One shared reducer releases only when either (a) the canonical NT target position contains every target-order fill trade ID and the lease is not conflicted, or (b) the non-conflicted lease has observed an exact-key coherent raw position report at a generation strictly newer than the terminal floor, with `ts_last >= terminal_fill.ts_event`, that exactly matches the canonical cache. The report arm requires the target venue-position ID in hedging mode; a netting report without one requires exactly one cached account/instrument position and that position must be the fenced target. Both arms require same-side-or-flat quantity within the maximum residual; a cached closed position must satisfy the same proof. Cache absence, one terminal trade ID, a generic timestamp, a reconciliation flag, a smaller quantity alone, ambiguous aggregation, report generation at or before the terminal floor, stale/conflicted report state, or report/cache disagreement is insufficient.
- [ ] Run terminal-fill sequences with position-before-fill and fill-before-position ordering. Add the pinned-NT case where fill A is projected without position application and terminal fill B is applied normally: the position contains B but remains above the signed residual bound, so callbacks and timers stay awaiting. Prove an unrelated reconciliation fill, ambiguous netting aggregate, stale report, duplicate pre-terminal report, same-ID/body conflict, equal-time signed-state conflict, checked-generation overflow, report/cache disagreement, and side flip also stay awaiting and route nothing. Then publish a strictly post-terminal exact-key `PositionStatusReport`, converge the cache to its signed quantity without a strategy callback, and prove the next timer remanages exactly five and the next evaluation routes only five. Add a direct positive case where the position contains every target-order fill ID, and make duplicate reports/events idempotent. Separately prove reports without an active lease are discarded, the last lease drop deletes key state, and LiveNode stop/restart leaves exactly one handler with no retained stale authority.
...
- [ ] Close the submit caller graph. Change `MakerOrderCommandSink::submit_maker_order` to return `BoltV3SubmitAttemptOutcome`; nest it in a submit-specific `MakerOrderDispatchOutcome`; rotate maker leg identities only for the inner `Submitted`; and replace submit use of maker runtime's `routing_error: String` with typed outcome transport. Keep pre-route build and cancel/modify failures in a separate stable typed command-failure phase whose diagnostic text is never parsed. Change kill-switch fan-out/executor results to a typed per-command outcome and aggregate report; `risk_admission_loss` exhaustively accepts only `Submitted` from each flatten route, preserves every other variant through the aggregate, and converts it to operator diagnostics only after the typed decision rather than `?` plus `Ok(())`.
- [ ] Add maker shadow-policy evidence proving `PolicySkipped`, zero submitted leg identity, zero sink/capacity mutation, and a later eligible attempt. Inject every non-submitted kill-switch outcome at the live-node caller and prove none returns successful submission or mutates the sink; `Submitted` remains the only success case.
...
- [ ] Add an exhaustive generation-checked exit-attempt reducer: arm `ExitAttempting` with its opaque position-authority lease before routing; every non-submitted outcome returns the same generation to `Managed` and drops that lease; only `Submitted` commits `ExitPending` and carries it forward; a synchronous callback that advances state transfers the lease and cannot be overwritten or de-authorized by the route return. Replace the current shadow latch test with `PolicySkipped` evidence, zero sink/capacity mutation, `Managed` exposure, no retained feed key, and a later eligible exit attempt.
