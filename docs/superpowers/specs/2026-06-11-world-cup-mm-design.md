...
For static sports events, `fair_probability_up` is not derived from strike/spot pricing. This slice preserves `fair_probability_source = "reference_current_price"` as the only supported static-event fair-probability source token, but the source table, provider runtime, and selected current-price plumbing are owned by PR 730. This does not add a standalone World Cup maker strategy and does not fake rotating-market pricing inputs for static event markets.
...
No World Cup-specific quote lifecycle, order-intent, order-compile, or dispatch implementation is added in this PR. Those shared maker mechanics remain outside this World Cup static-event selection slice.
...
After static event selection lands, PR 730 must provide the referenced current-price fair-probability source and PR 716 must provide the shared maker quote/order pipeline. The later World Cup runtime shell must connect those pieces without creating a second standalone World Cup maker strategy, duplicate maker math, or a parallel NT order-construction path.
