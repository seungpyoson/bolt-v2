...
5. For #1016, first open the draft precursor to reserve its exact PR number, make it review-ready and green, and reserve a draft activation PR prepared to the exact allowed scope as far as possible. Complete the control-plane/live canaries and review the pre-precursor abort/restore procedure. Disposable rehearsal must prove this repository/API's GitHub Cloud `exempt` support and Mergify injection behavior plus exact-number admission, injection-disabled behavior, hidden routes, self-change reset, preexisting proof invalidation, mixed batches, merge-time Freeze re-evaluation, exclusions, dequeue/no-running-batch state, wrong publisher, native/direct blocking, Freeze under exempt, identity, latency, bounded retry behavior, a successful-ceremony target and pre-precursor abort threshold, and API/quorum/audit. The owner approves the rehearsal-derived target, retry count/time budgets, and abort threshold and separately accepts post-precursor no recovery plus the resulting unbounded terminal-tail ordinary-merge outage risk before the lock lands; no numeric bound is invented by this plan.
   Operators then land one separately reviewed temporary `.mergify.yml` admission lock under current legacy authority. It has exactly one queue matching the precursor number, batch size one, one parallel check, branch-protection injection disabled, four explicit legacy checks, mandatory native review, and no alternate route. The ordinary/unrelated-merge outage begins at that lock merge; only the precursor and activation ceremony PRs may merge until cleanup. That merge makes the precursor's pre-lock green evidence stale, so operators refresh it onto post-lock `main` and regenerate exact-head evidence before queueing; the rehearsal-derived outage budget includes this refresh, and only fresh exact proof-head checks are merge-authoritative. Operators establish an indefinite Freeze, atomically put ruleset 14763242 in final gate-only-replacement state, change Mergify 10562 from `always` to GitHub Cloud bypass mode `exempt` subject to disposable live proof, add the reserved activation as an inert second exclusion, and terminally re-query all state. Because `exempt` removes GitHub-side bypass audit entries, compensating evidence consists of retained digest-bound control-plane records for every App emission and a signed terminal re-query receipt sufficient for incident reconstruction. The precursor stages the dormant replacement and manifest and atomically replaces the admission lock with final hotfix/default mappings. The temporary lock, legacy checks, and native review judge it; trusted emits nothing. After merge, no enforcement mutation occurs: promotion, tombstone, and an internal, non-publishing closed canary run against final state. Enumerated infrastructure/observation noise may retry with a fresh nonce only while every protected, configuration, staged, activation, constituent, and tombstone identity is unchanged and within the owner-approved budget. Failed, intermediate, infrastructure-noise, canary, reused, converted, or partial results cannot publish merge-visible authority. A fresh activation retry under the same immutable activation/base/configuration tuple may emit a new result only after independently completing terminal validation for its exact regenerated proof head; no prior result is reused or converted. Merits/identity failure or budget exhaustion is terminal. Success permits the reserved activation to queue alone as the literal first subsequent covered change; trusted remains absent until terminal tuple validation succeeds for its exact proof head. After exact protected-main proof, Freeze and temporary Merge Protections reporting/binding are removed and the successful-path outage ends. Thereafter trusted is the steady-state authority for each exact Mergify proof head judged by the active protected-base verifier, remains absent on ordinary PR heads, and Vn judges Vn+1 staging. Final Mergify `exempt` remains unless separately redesigned. Failure after precursor has no recovery PR: Freeze remains, the ordinary-merge outage may be unbounded, and recovery requires a new program plus explicit owner/external operational decision. If the pre-precursor abort threshold is exceeded, the reviewed abort/restore procedure applies. This remains pending approval and is not a universal ceremony for later domains. The primary bypass-mode reference is [GitHub Cloud repository-rules REST API](https://docs.github.com/en/rest/repos/rules); [Mergify ruleset behavior](https://docs.mergify.com/merge-queue/github-rulesets/) remains separately subject to live proof.
...
Program B is blocked on issue-body/atomic-ruling reconciliation; owner/external approval of the proposed hinge and corrected dormant-base/two-context design; separate App/control-plane and temporary Merge Protections/Freeze authorization and budget; disposable live proof of exact-number admission, this repository/API's `exempt` support, Mergify injection behavior, hidden routes, identity, exclusions, self-change reset, proof invalidation, mixed batches, merge-time re-evaluation, dequeue/no-running-batch state, native/direct blocking, retry classification/exhaustion, successful-ceremony timing, latency, and API/quorum/audit; owner-approved rehearsal-derived successful-ceremony target, retry count/time budgets, and pre-precursor abort threshold; separate owner acceptance of the unbounded terminal-tail ordinary-merge outage risk; exact-SHA regeneration; precursor and activation drafts prepared before the admission-lock PR; reviewed pre-precursor abort/restore; pre-precursor terminal final state; the single precursor merge under legacy checks and native review; promotion and irreversible bootstrap disablement without later enforcement mutation; successful closed canary and exact freeze evidence; and atomic cutover as the first subsequent covered Git/enforcement-surface change. The ordinary/unrelated-merge outage begins at admission-lock merge, permits only the precursor and activation ceremony PRs, and on success ends only after terminal activation proof and temporary-control removal. If the pre-precursor abort threshold is exceeded, the separately reviewed abort/restore procedure applies. After precursor merge or merits/identity canary/activation failure, Freeze stays active and there is no recovery PR; the ordinary-merge outage may be unbounded, and any recovery is a new separately authorized program. Later subsystem rows are unauthorized planning nodes, not queued implementations.
