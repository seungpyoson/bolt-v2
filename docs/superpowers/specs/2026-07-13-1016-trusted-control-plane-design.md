...
3. **Authority service and publisher runtime:** a small external GitHub App service. It accepts events, observes GitHub/Mergify state, drives the launcher, validates policy-agnostic protocol structure, appends state, and publishes checks. It has no repository-rule definitions, parsers, corpus selection, applicability logic, or success exceptions. The deployed binary/runtime and its App signing boundary are one catastrophic TCB: compromise can publish directly to GitHub outside the ledger. The ledger detects but cannot cryptographically prevent that direct credential misuse.
4. **Sandboxed launcher:** fetches exact Git objects and the attested protected-base bundle, supplies candidate documents as inert inputs, enforces resource and output limits, and returns the engine's canonical protocol document plus execution metadata. Only an attested invocation of the active promoted engine/launcher artifact may request one purpose-bound engine-result signature for its exact invocation/result pair. Candidate bytes and a general launcher process cannot export the signing credential, choose an arbitrary payload, or invoke it outside that bounded path. The launcher cannot authenticate to the Checks API or mutate control-plane state.
...
6. **Append-only state:** the honesty root for ordered attempts, proof consumption, protected-state epochs, repository-governance evidence, key events, terminal re-query receipts, publication receipts, and the bootstrap tombstone. It exposes conditional append and rollback detection, not arbitrary record update/delete.
7. **Keys:** the GitHub App installation credential, an audit-record signing key, a purpose-bound engine-result machine signing credential and verification root, and builder/attestation verification roots.
...
## Principals, roles, permissions, and repository governance

The actual human identities and service accounts for each populated role, plus the recovery disposition for each credential or role, must be recorded before live proof; a recovery disposition may explicitly be `none`. This inventory does not require a backup human or create another approval gate.
...
| Authority runtime | Use installation credential; append records; request sandbox runs; publish after state-machine authorization | Cannot alter repository policy, bypass repository governance, or authorize its own key changes |
| Launcher | Read exact Git objects and governed artifacts; write one bounded result to the authority service | No GitHub/Mergify write credentials, state-store credential, App key, or network except allowlisted object/artifact reads |
| Builder | Produce source-to-artifact attestation for reviewed source | Cannot promote, publish, or mutate GitHub/Mergify |
| Repository operator | Execute reviewed ruleset/Mergify/Freeze operations and artifact promotion | Cannot bypass native code-owner review, protected-branch rules, exact-head CI, merge queue, or terminal re-query |
| Audit reader/incident owner | Read and reconstruct retained records | Cannot append authority or publish checks |

Control-plane source, configuration, artifact, epoch, bootstrap, tombstone, App/key, and ceremony mutations use the repository's existing governance only: required native code-owner review, protected-branch enforcement with stale-review dismissal, last-push approval and thread resolution, exact-head CI, and `just merge-queue`. The control plane adds no owner set, quorum, hardware-key requirement, emergency credential, or separate human approval gate. Each applied mutation is digest-bound to the reviewed repository change and recorded with its exact GitHub actor, repository head, review/check evidence, merge-queue evidence, observed control-plane payload, and terminal re-query. Steady-state proof evaluation and publication remain automated.

This governance reduces accidental or unauthorized repository changes; it cannot prevent direct use of a stolen App credential or compromise of the authorized publisher runtime. That remains a catastrophic residual TCB risk, with least privilege and durable audit evidence providing exposure reduction and detection rather than a fictitious additional authorization boundary.

## Canonical authority record

Records use UTF-8 JSON canonicalized with RFC 8785. Unknown, duplicate, missing, null-substituted, or non-canonical fields fail. The service signs final authority-record bytes with a purpose-bound audit key. The invocation digest is `SHA-256(canonical_invocation_tuple)`; the final record separately commits that invocation, the engine-signed result, and the terminal control-plane evidence. Compact references never replace the explicit records in retained audit evidence.

Before execution, the service constructs and signs a canonical invocation tuple containing:
...
- protected `.mergify.yml` blob SHA/digest, recorded Mergify configuration epoch, integration identity, bypass mode, queue/routing digest, Freeze identity/state/exclusions, and Merge Protections state;
...
- control-plane epoch, invocation nonce, attempt number and lineage root, prior ledger sequence/hash, tombstone generation/value, engine-result verification-root digest and key version/purpose, and other relevant key versions.

The protected-base semantic engine produces one canonical result containing its terminal classification, ordered finding digest, and required boolean `authority_surface_change`, then the bounded signing interface signs `(invocation_digest, canonical_result_digest, engine_result_key_version, purpose)`. That flag covers only enumerated repository-visible enforcement surfaces inspected by the semantic engine: `true` requires the governed Vn+1 staging/activation path, while `false` permits ordinary steady-state advancement only if every structural check also succeeds. The publisher carries the signed result, flag, signature, and result digest into the final authority record but must never derive, override, or reinterpret them.

The final signed authority record commits the complete invocation tuple, engine-result digest, `authority_surface_change` value, engine-result signature, verification-root digest, key version and purpose, terminal re-query receipt digest, completion time from the control plane's trusted clock, and record sequence. The engine-result root and key version must match the active artifact/control-plane epoch in both invocation and terminal re-query. External ruleset, Mergify, App-installation, key, and control-plane-epoch changes are not semantic flag inputs; the tuple and terminal live re-query independently detect them as structural invalidation.
...
3. It constructs the canonical invocation tuple, allocates a fresh nonce by conditional append, and rejects duplicate delivery, proof-use, active attempt, purpose mismatch, ordinary PR head, unauthorized constituent, or moved state.
4. The launcher receives only the immutable bundle, exact Git objects, invocation digest, and nonce. Candidate code is never executed as the semantic engine. Network, time, memory, process, filesystem, output count, and protocol channel are bounded.
5. The service validates the engine's one signed canonical JSON result using a closed, policy-agnostic protocol: exact invocation/nonce/bundle/manifest commitment, supported schema, terminal enum, required `authority_surface_change` boolean, canonical field and finding representation, protocol-defined uniqueness and ordering, the exact purpose-bound signature payload, verification root and key version admitted by the active artifact/control-plane epoch, and internal digest consistency. The protected-base engine alone owns rule IDs, rule ownership, applicability, semantic completeness, the repository-visible enforcement-surface classification, and whether findings are omitted or present. The publisher never enumerates rule IDs, recomputes expected findings, derives or changes the flag, or reads repository policy to reinterpret the result. Crash, timeout, signal, extra output, malformed data, unknown field/classification, neutral, skipped, or structurally incomplete output is non-authorizing.
...
`UNINSTALLED -> INSTALLED_NONAUTHORITATIVE -> EPOCH_RECORDED -> BOOTSTRAP_ISSUED -> PROMOTED -> TOMBSTONED -> CANARY_TERMINAL -> ACTIVATION_TERMINAL -> STEADY_STATE`
...
- **Epoch:** Every protected control/artifact/key set has a monotonically increasing generation and digest. Movement creates a new proposed epoch and invalidates attempts; it cannot silently amend an epoch.
...
- **Tombstone:** Promotion completion requires a repository-governed append of one repository/bootstrap tombstone with a strictly higher monotonic generation. Once observed, all service versions and state restorations must reject bootstrap issuance and acceptance. The state primitive must detect a restored snapshot or stale replica whose signed checkpoint/sequence predates the externally durable tombstone anchor. Ambiguity or unavailable state fails closed.
...
Steady-state main advancement is a separate monotonic transition, not an authority-epoch change. An ordinary proof may advance `main` from base B to expected descendant R under the same Vn engine/manifest/corpus identities only when the protected-base semantic engine emits `authority_surface_change: false` for the enumerated repository-visible enforcement surfaces; candidate metadata and the publisher cannot make that classification. After exact merge observation, R becomes the next admissible policy-base repository SHA while the semantic authority version remains Vn; every other old-B publication domain becomes non-publishable, and the live Mergify proof must show that old-B proof heads cannot merge after advancement. `authority_surface_change: true` requires the separately governed Vn+1 staging/activation path for repository-visible changes to engine, manifest, corpus, semantic registry, authority protocol, or launcher trust boundary. External ruleset, Mergify, App-installation, key, or control-plane-epoch movement is independently terminal through tuple comparison and terminal live re-query, regardless of the flag. Out-of-band, unauthorized, non-descendant, ambiguous, or unexpected protected-ref movement is terminal; it never becomes a normal base roll-forward.
...
- The engine-result machine signing credential is purpose-bound to the exact payload `(invocation_digest, canonical_result_digest, engine_result_key_version, purpose)` and available only through workload identity and admission for the attested active promoted engine/launcher artifact. Candidate execution, general launcher code, builders, operators, and the publisher cannot export it or request arbitrary signatures. Its verification-root digest and active key version are bound to the artifact/control-plane epoch, invocation tuple, terminal re-query, and final authority record. Least-privilege policy and non-exportable custody are required where the selected platform supports them; if hosting cannot prove equivalent isolation, payload restriction, workload admission, access logging, and revocation semantics before implementation, hosting is an implementation blocker rather than grounds to weaken the boundary.
- Builder attestations bind source commit, clean source tree, build recipe/image digest, dependency lock digests, builder identity, artifact digest, and output manifest. Promotion requires repository-governed review of that exact attestation and independent digest verification.
...
- Rotation is a repository-governed monotonic epoch transition: outstanding attempts are invalidated, and only one App installation/publisher and engine-result signing epoch may authorize publication. Verification overlap for historical audit does not create dual publication authority.
- Revocation is append-only and immediately blocks new append/publication under that key. Engine-result signing-key loss, compromise, ambiguous custody, wrong-purpose use, workload-admission failure, or failed revocation propagation fails closed. The existing repository or GitHub App owner may use the existing platform's revoke/disable control immediately to stop publication; this narrow emergency action cannot publish or approve a result, restore bootstrap, bypass Freeze, create alternate authority, rotate a key, or activate an epoch. Its exact actor, API/control payload, and observed before/after state are appended or reconciled into the audit ledger afterward. Normal activation and rotation use existing repository governance, and no emergency credential or separate gate is introduced.
...
- repository reviews/checks/merge-queue evidence, mutations, epochs, keys, attestations, promotions, and tombstone;
...
- repository-governed ceremony operations use the existing reviewed repository tooling and GitHub control surfaces; the service exposes no separate approval or arbitrary mutation endpoint and records the observed operation plus its repository-governance evidence;
...
4. Repository-governance evidence tests for missing, stale, wrong-head, dismissed-review, unresolved-thread, failed-check, wrong-artifact, and non-merge-queue changes.
...
7. Key drills: engine-result and publisher/audit key rotation, revocation, compromise, loss, wrong-purpose payload, arbitrary-sign request, unattested/general-launcher request, candidate access/export attempt, workload-identity movement, verification-only historical overlap, and installation identity change. Evidence must prove only the attested active engine/launcher invocation can obtain the one exact purpose-bound signature and that loss or mismatch disables publication.
