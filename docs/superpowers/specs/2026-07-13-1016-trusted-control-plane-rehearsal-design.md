...
An independent existing GitHub owner-user session removes the target from the selected-repository authority installation with `DELETE /user/installations/{installation_id}/repositories/{target_repository_id}`. Preflight must prove the principal, exact installation, target and anchor identities and live provider support. The live call must return `204`; `422`, ambiguity, or any other result is `NO_GO`. The independent owner-user `GET` of the installation's selected repositories is the membership authority for this endpoint and must return exactly the anchor, with the target absent. Separately, an authority-App Checks create on the target must be provider-rejected and recorded, while the anchor remains inert and unchanged. No separate target-installation lookup is required. This stop-only control cannot create success, approve, restore, rotate, publish to the anchor, or provide another publisher; no credential or approval gate is added to the service.
