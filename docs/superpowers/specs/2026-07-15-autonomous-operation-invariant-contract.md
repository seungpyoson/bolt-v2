...
its fixed reserve. Before `C` is released or ballast retouched, residue transfers
to the corresponding `K_host` row; a pre-dial projection that would exceed any
row, port, or kernel subtotal blocks the dial. Stable reset mutates one fixed row
and never discards retained ownership early.
