...
`StrategyRegistrationContext::new` returns `Result` and resolves the execution
venue for every strategy through one shared helper. The context stores that
venue privately. If the strategy declares the settlement capability, its
settlement account and currency are derived from the same stored venue.
...
`assemble_strategy_build_context` reads the private stored venue. The late
`execution_venue_for_context` lookup and the settlement/non-settlement venue
selection branch are removed. There is no default, retry, conditional fallback,
or second venue-resolution path.
...
| A strategy resolves a different venue later | One resolver and one private stored venue | Structural fence pins the single caller and rejects alternate lookups |
...
| Preflight executes user binding behavior | Binding kind is inert string metadata; registration function is not called during preparation | Panic/counting callback regression remains unreachable on preflight failure |
