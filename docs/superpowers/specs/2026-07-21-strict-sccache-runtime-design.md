...
6. Each builder verifies the expected ELF architecture, absence of a runtime interpreter and shared-library dependencies, and host execution outside the build container. Its missing-S3 negative control runs with an isolated empty environment so runner residue cannot satisfy the control.
...
10. The publisher downloads only the verified bundle from its own workflow run, reruns the same manifest and byte verifier, and requires byte-identical verifier output before refusing any pre-existing tag, release, or draft. The release tag is derived from the full SHA-256 of the canonical governed build-input identity, including source, patch, executor, toolchain, build settings, verification threshold, and targets.
