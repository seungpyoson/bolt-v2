...
- `ci/sccache-strict.toml`: build-provenance inputs only: source URL and digest, upstream commit, patch identity, Rust toolchain, build-executor image digest, feature set, targets, replica topology, normalized build settings, the test-only cache mode, strict timeout, and frame limit used while verifying the derivative, and the bounded release-attestation retry policy. It does not own consumer release, asset, cache-location, runtime-policy, or runner values.
...
11. The token-bearing release step refuses any pre-existing tag, release, or draft, confirms observed-current `main` and the protected environment, generates a run/attempt/head ownership marker, arms exact draft/tag cleanup immediately, creates the draft, validates the returned release identity, and uploads the agreed binaries and provenance. It revalidates current `main` and the environment immediately before publishing. If the create response is lost, cleanup may recover a unique mutable draft by listing releases, but only when its tag, target, and exact ownership marker match; deletion always uses the recovered exact release ID. A tag is deleted only while it still directly targets the exact dispatched commit. If GitHub reports `immutable: false`, it deletes only that owned mutable release and unchanged generated tag, then fails.
...
- storage tests proving read errors, complete hit-materialization timeouts, late corruption without earlier-output commit, commit-failure rollback, quiescent final-output mutation before failure returns, corrupt stdout/stderr/required/optional objects (including temp-file failure), and read-write write failures return nonzero;
...
- listener lifecycle tests proving the exclusive ownership lock rejects concurrent starters, live Unix sockets are preserved, unchanged stale sockets are reclaimed, clean shutdown removes only the owned inode, replacement inodes survive cleanup, and occupied TCP ports remain fatal;
- publisher negative controls for cross-run or mismatched artifacts, stale or non-main heads, pre-existing tags/releases, absent environment approval, lost-create-response recovery by exact ownership marker, replacement-tag preservation, and exact-ID cleanup after a mutable publication result;
