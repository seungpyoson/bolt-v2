...
The governed Linux derivative accepts only a Linux abstract Unix-domain socket supplied through the adoption configuration. It rejects pathname Unix sockets and TCP at strict startup. Abstract sockets have no filesystem inode or cleanup lifecycle: the kernel releases the name when the last reference closes. This removes pathname ownership, stale-file recovery, lock-file replacement, quarantine, and shutdown-unlink behavior from the governed contract.
...
| Output targets `/dev/fd/*`, `/proc/*/fd/*`, a FIFO, device, socket, symlink, multiply linked regular file, duplicate path, or colliding path | Return typed `CannotCache`, record the reason, perform no storage operation or server compiler execution, and authorize one intentional client execution. |
...
| Pathname Unix socket or TCP selected for the strict derivative | Fail before server startup. The governed targets use only a configured Linux abstract Unix socket. |
...
Cache-hit output handling has two phases. First, every member is decoded and validated into a temporary file created through a pinned descriptor for the destination's own parent directory; no destination is mutated during this phase. The invocation is classified `CannotCache` before storage access when an output cannot support that contract. Second, validated regular outputs are committed synchronously with descriptor-relative same-directory renames. Existing regular outputs receive same-directory backups for rollback. Parent and destination identity are revalidated before commit. The implementation has no direct-copy fallback and no worker may continue mutation after failure returns. POSIX does not provide a multi-path atomic transaction: a late commit or rollback failure is fatal, and any partial local bytes have no valid build-evidence authority.

The server starts only when the effective backend and mode exactly match the governed S3 configuration. Read-only mode skips `put` entirely. Read-write mode withholds success until `put` completes. The patched binary requires typed startup, IPC, cache-read, required-write, abstract-socket, and maximum-frame inputs but does not own their consumer values; #1494 supplies them from `ci/sccache-location.toml`. `ci/sccache-strict.toml` owns `max_runtime_timeout_ms = 3600000`, which is compiled into the derivative and its identity. All four runtime timeouts are parsed once and must be positive and no greater than that limit. One checked deadline constructor owns all `Instant` arithmetic, including retry/backoff multiplication. The strict path has no built-in operational defaults.

## Repository artifacts

The prerequisite PR creates these bounded units:

- `ci/sccache-strict.toml`: build-provenance and derivative-contract inputs only: source URL and digest, upstream commit, patch identity, Rust toolchain, build-executor image digest, feature set, targets, replica topology, normalized build settings, the compiled-in runtime-timeout ceiling, the test-only cache mode, strict timeout, and frame limit used while verifying the derivative, and the bounded release-attestation retry policy. It does not own consumer release, asset, cache-location, actual runtime timeout, or runner values.
...
6. Each builder verifies the expected ELF architecture, absence of a runtime interpreter and shared-library dependencies, and host execution outside the build container. Separate scrubbed-environment controls prove that missing S3 configuration, an injected disk backend, pathname sockets, TCP, and oversized runtime timeouts fail startup with their distinct diagnoses. The positive strict-server control uses a unique Linux abstract socket and leaves no filesystem socket entry.
...
11. The token-bearing release step confirms observed-current `main` and the protected environment, then atomically creates the exact tag with `POST /git/refs`; only a direct `201` is success. A collision, timeout, lost response, or any other ambiguous result fails and preserves the tag state for operator review. It generates a run/attempt/head ownership marker, creates a draft bound to the already-created tag, validates the returned exact release ID and mutable draft state, and uploads the agreed binaries and provenance. If the draft-create response is lost, cleanup may recover a unique mutable draft by listing releases, but only when its tag, target, and exact ownership marker match. Cleanup deletes only an exact owned draft whose `immutable` value is exactly `false`; it never automatically deletes a Git tag. Failed or ambiguous release deletion stops all later cleanup mutations.
12. Every byte, manifest, release identity, current-`main` observation, environment predicate, and downloaded-asset comparison that can be checked before publication is checked while the release is still a draft. The publisher then revalidates current `main` and the environment immediately before publishing. Publication is a one-way transition: GitHub automatically creates the release attestation for an immutable release, and the workflow must read back `immutable: true`, recheck `main`, and use bounded TOML-governed polling for `gh release verify` and `gh release verify-asset`. If `main` moves or any post-publication predicate fails, the immutable candidate is retained without adoption authority and the workflow fails. It is never deleted automatically; a corrected derivative uses a new content-derived tag. A following tokenless step evaluates the exact release/tag/asset predicate. The later consumer still verifies its independently reviewed SHA-256 pin.

The repository owner must enable release immutability and create the main-restricted publisher environment before the first publication dispatch. The workflow uses only `GITHUB_TOKEN`: it verifies the environment before publication and verifies the immutable release result immediately after publication. A disabled immutability setting may leave the atomically reserved tag and exact mutable draft for operator review but cannot silently produce an adoptable asset. These operator prerequisites do not belong to the prerequisite PR's diff. GitHub documents that immutable release assets and their tag cannot be changed after publication, that immutable publication automatically generates the release attestation, and recommends draft, upload, then publish: [Immutable releases](https://docs.github.com/en/code-security/concepts/supply-chain-security/immutable-releases) and [Verifying release integrity](https://docs.github.com/en/code-security/how-tos/secure-your-supply-chain/secure-your-dependencies/verify-release-integrity).
...
- output-classification tests proving `/dev/fd/*` with different client/server file-descriptor tables, `/proc/*/fd/*`, FIFOs, devices, sockets, symlinks, multiply linked files, duplicate paths, and colliding paths become typed `CannotCache` before storage or server compilation;
- storage tests proving read errors, complete hit-materialization timeouts, late corruption without earlier-output commit, missing stdout or stderr is fatal, pinned-parent staging failure before destination mutation, parent or destination replacement is fatal, commit-failure rollback, fatal rollback failure, quiescent final-output mutation before failure returns, corrupt stdout/stderr/required/optional objects, and read-write write failures return nonzero;
...
- transport tests proving abstract-socket concurrent bind is refused, clean stop and crash release the abstract name, restart succeeds without filesystem cleanup, pathname sockets and TCP are rejected, and an upstream or stale daemon identity remains fatal;
- timeout tests proving zero, malformed, maximum-plus-one, and multiplication-overflow values fail at startup on both governed Linux targets while the exact compiled maximum is accepted;
- publisher negative controls for cross-run or mismatched artifacts, stale or non-main heads, atomic tag collision and lost response, absent environment approval, lost-draft-response recovery by exact ownership marker, ambiguous listings, strict `immutable: false` cleanup selection, failed draft deletion with no later mutation, permanent tag preservation, and bounded automatic-attestation polling;
...
Any verification, build, reproduction, environment, release, download, or digest failure stops the affected workflow. An exact owned mutable draft may be removed by numeric release ID; Git tags are never removed automatically. An immutable result is retained for review even if later evidence retrieval fails. A fix produces a new content-derived release identity. There is no fallback publication or installation channel.
