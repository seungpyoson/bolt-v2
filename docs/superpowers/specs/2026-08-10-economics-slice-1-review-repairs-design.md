...
The cancellation module owns the complete tracked-maker aggregate, not merely its inner state. One opaque `TrackedMakerOrders` owns the lock, map, `TrackedMakerOrderRecord`, optional resting economics, query seed, optional cancellation intent, and registration rollback guard. `BoltV3OrderEconomicsHandle` may store and call this owner but cannot construct, clone, replace, destructure, or borrow any record or cancellation field. The parent module never receives `&mut TrackedMakerOrderRecord`, a mutable callback over the registry, or a constructor for a partially initialized record. Its only interfaces are semantic operations: register a submitted resting order, refresh economics from an authoritative cache observation, request one cancellation, request an instrument/side scope, reconcile one NT callback, drive due recovery, inspect read-only IDs/health, and test whether draining is complete. Registration, refresh, intent merging, terminal removal, and rollback execute inside the owner. This makes generation, deadline, backoff, query identity, and record lifetime compiler-owned as one aggregate rather than replaceable at a parent call site.
...
| Opaque aggregate ownership | Resting registration, economics refresh, quote-lifecycle cancellation, scoped cancel-all, strategy drain, pending-cancel adoption, fill-void recovery, terminal removal, and rollback are exercised only through `TrackedMakerOrders` semantic operations. Production compilation and direct API/diff review prove that no parent caller can construct, clone, replace, destructure, or mutably borrow a tracked record or cancellation field; no source-scanning test is added |
