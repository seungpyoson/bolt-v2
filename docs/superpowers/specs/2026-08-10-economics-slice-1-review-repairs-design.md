...
Exit evidence is a typed phase machine, not a `submitted` boolean. `ExitIntentDecisionFact` records the requested strategy decision before fallible preparation and carries no claim that an order was prepared or submitted. The existing `ExitSubmissionDecisionFact` is emitted only after successful preparation and therefore always carries the compiler's actual final quantity/linkage. `ExitEvaluationFact` records the exhaustive terminal attempt outcome as `Held`, `Blocked`, `PreparationRejected { stage, reason }`, `AdmissionRejected { final_submission, reason }`, `PolicySkipped { final_submission }`, `SinkRejected { final_submission, reason }`, or `Submitted { final_submission }`. Illegal combinations are unrepresentable, and no separate preparation fact duplicates the evaluation. Facts, codecs, generated contract, fixtures, and round trips change atomically.

Venue routing remains after valid intent evidence, a prepared final basis, and admission. Residual remanagement is explicit over both reduction layers. If the venue only partially fills an IOC and then cancels/expires the remainder, the existing terminal event re-reads the canonical NT position and remanages it. If a reduced-size IOC fills its entire submitted quantity (`leaves_qty == 0`) while the position remains open, the pending exit enters a typed `TerminalFillAwaitingPosition` state. The fill callback resolves immediately when the canonical position cache is current; otherwise the next position event or strategy timer reconciliation transitions it to `Managed` with `ResidualRemanaged` evidence (or to flat when no position remains). The next evaluation can then route another reduction. Event-order permutations are explicit tests, so no fully filled reduced IOC can leave the residual permanently `ExitPending` merely because a position event arrived before or after the fill callback.
