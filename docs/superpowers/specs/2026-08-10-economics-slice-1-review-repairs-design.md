...
| L11: Bolt truncates fees toward zero on exact decimals while pinned NT's Rust `compute_commission` (`adapters/polymarket/src/execution/parse.rs`) applies `Decimal::round_dp(5)` — banker's rounding on an exact decimal, not float rounding | Pinned-NT-aligned point accounting with a conservative debit bound | Task 14 | Numeric fixtures match the pinned Rust `compute_commission` across rounding-boundary prices; the bound remains at or above the point value |
