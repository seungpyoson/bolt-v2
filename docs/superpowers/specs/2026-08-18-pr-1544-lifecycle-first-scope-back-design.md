...
- `ManagedPositionOrigin`, the `origin` field, and the public `is_recovering()` projection; every routeable managed position is current-process state. The private owner's exhaustive entry decision still emits `EntryBlockReason::RecoveryMode` for `EntryReconcilePending`, `UnsupportedObserved`, and `BlindRecovery`, so the existing versioned evidence reason remains producible without a second public state projection;
...
`Refused` is deliberately terminal for strategy-local routing: the edge taker cannot enter, exit, modify, or issue another cancel for that claim until exact terminal or zero-leaves evidence retires it. A persistent GTC/GTD remainder can therefore hold the strategy indefinitely. The unhealthy outcome is surfaced to the existing shared kill-switch cancellation/manual-intervention control plane over NT order identity, which remains responsible for proving whether a supported route exists. This slice neither claims that escalation canceled the order, invokes that control plane automatically, nor adds a strategy retry path.

The reducer installs `Pending` before the cancel call. A synchronous terminal callback consumes the exact entry identity and cannot be overwritten by the return path. Route completion reduces only while that identity and pending phase still match. Each exact entry identity can make at most one live cancellation call through the edge-taker remainder reducer: only policy skip restores `Working`, because it proves NT was not called. Cancellation uses the existing shared order-execution policy and event/cache truth; this design adds no cancellation map, timer, backoff, attempt counter, or second retry algorithm. Existing system-level kill-switch handling remains a separate operator control, not a retry owned by this reducer.
...
The existing reservation index becomes one order-ID-keyed typed reservation record containing the immutable committed attribution, exact ledger reservation ID, lifecycle phase, and that phase's owner-local state revision. It does not own a second mutable liability number. Its phase is `Reserved`, `SinkInvoked`, or `ObservedOpen`; there is no parallel unknown-reservation map. `Reserved` is refundable before the call boundary. `SinkInvoked` is a carried commitment whose authority is the completed admission plus the Bolt-to-NT call boundary. `ObservedOpen` retains the exact fresh NT open-order identity and diagnostic observation timestamp that advanced it. An exact open observation may advance `SinkInvoked` to `ObservedOpen`; projection omission cannot do so.

The current `capital_admission_nt_projection_epoch` is renamed and broadened into one `capital_admission_state_revision`; no second lifecycle counter is added. Under the same admission-state lock, every reservation or phase mutation, lifecycle/cache observation that may advance or retire a record, reconciliation invalidation, and accepted candidate swap advances this revision. Each record stores the revision at which its current phase was installed. Rebuild work captures an expected revision before reading NT state and may swap only if that exact revision still holds. Every mutation computes its checked successor before changing state; exhaustion preserves the prior state, marks admission unhealthy, and returns a typed failure rather than panicking. Submit preflight reserves that successor before the point of no return, and aggregate commitment only installs the prepared value, so the infallible boundary performs no checked arithmetic. Unix time, actor time, and NT `ts_event` remain freshness or diagnostic metadata and never establish lifecycle causality.
...
Reconciliation status gates new admission and replacement of the whole ledger; it does not make an exact existing reservation immutable. While reconciliation is invalid, two identity-preserving mutations remain available: exact pre-sink rollback of a `Reserved` generation, and exact terminal/zero-leaves retirement of a `SinkInvoked` or `ObservedOpen` generation. Each validates the typed record and ledger entry together. The admission owner exposes `retire_terminal_reservation`: it checks exact client order, reservation generation, lifecycle phase, and phase revision against the typed record, then cross-checks that record's pool, collateral group, and reservation ID against the ledger. The generic ledger exposes only `remove_existing(pool_id, collateral_group_id, reservation_id)` to its gate owner; it learns no client-order, lifecycle, venue, or strategy type, requires no reconciled status, and reapplies no pool freshness, available-balance, or minimum-balance admission policy. After authoritative exact cache/lifecycle proof, the terminal reducer prepares the next owner-local state revision under the same lock, so an equal or older Unix/event timestamp cannot defeat retirement. The admission-state lock serializes the existing fail-closed `CapitalAdmissionRebuildV1` append for that exact observation and the exact removal; no new release-evidence identity is added. Evidence failure preserves both, while success removes the ledger entry and typed record together and installs the prepared revision. Rollback and retirement return typed decisions, and the index record is removed only after the matching ledger removal succeeds. Missing, stale-revision, or mismatched identity preserves both and reports an invariant failure. Partial-fill revaluation remains blocked while unreconciled, so the full prior liability stays carried until a complete rebuild or terminal retirement.

Rebuild is candidate-first and atomic. It builds a candidate ledger and candidate typed index without clearing the live state. `refresh_capital_admission_state_from_components` may update NT-derived components and call non-destructive invalidation, but it never replaces the gate. `CapitalAdmissionGate::rebuild_open_order_reservations` constructs and returns a candidate without resetting the live ledger; every rejected decision reports the preserved live liability. The candidate carries its expected admission-state revision. Only a complete, valid candidate whose rebuild-evidence write succeeds and whose revision still matches replaces both ledger and index; the final swap rechecks the revision so a callback during evidence writing cannot be overwritten. Incomplete projection, rejected reservation evidence, fill-evidence-integrity failure, missing capital state, duplicate attribution, stale revision, or rebuild-evidence write failure preserves the existing records and numerical ledger, calls the gate's non-destructive reconciliation invalidation, and blocks new admission. Every current-process invalidation uses that non-destructive operation; an empty `CapitalAdmissionGate::unreconciled()` is used only for initial construction or restart before any current-process reservation exists. No error path clears a nonterminal record. The merge and commit live in shared admission state, not `LiveNode`.

A complete open-order projection merges every retained nonterminal record absent from the projection as `RetainedLifecycleReservation`, whether its phase is `SinkInvoked` or `ObservedOpen`. If the exact client order is present, its fresh attribution-checked observation advances `SinkInvoked` to `ObservedOpen` or refreshes the existing `ObservedOpen` evidence; it is not inserted a second time, and an identity mismatch rejects the candidate. The shared capital-admission event feed triggers an exact cache observation and atomically removes the record and ledger reservation only when the same client order is authoritatively terminal or has zero leaves. `OrderFilled` alone is not terminal proof because it may be partial. Exact generation and admission-state revision ordering ensure that a synchronous terminal callback or later local observation may retire the record before submit/rebuild completion and that stale completion cannot reinsert it. Cache absence, projection omission, elapsed time, an older/equal event timestamp, or another rebuild failure cannot retire either nonterminal phase or block otherwise exact terminal retirement.

`SinkInvokedUnknown` creation emits an error-level diagnostic and typed outcome evidence. Existing admission rebuild evidence exposes `unresolved_sink_invoked_reservation_count` and `unresolved_observed_open_reservation_count` while retained records in either phase are absent from a complete projection; their checked sum is the derived `unresolved_lifecycle_reservation_count`, and logging occurs when that pair changes, not on every projection. This also covers a successful NT return whose order has not yet appeared and a previously observed order later omitted without terminal proof. It is observability over the one typed record, not another lifecycle authority or release path. The current-only cutover extends the existing `CapitalAdmissionRebuildV1` wire with the two scalar counts, bumps its configured schema version, regenerates the contract and positive/rejection corpora, and updates codec/domain/round-trip evidence atomically. It does not add a compatibility decoder or parallel evidence identity. Deployment of that schema remains governed by the existing current-evidence hard-cutover runbook and is outside this PR's authorization.
...
- `Refused` also blocks strategy-local exit indefinitely until exact order retirement. Operator recovery is the existing shared kill-switch/manual-intervention control plane, not an automatic strategy effect.
...
3. Entry arming succeeds only from `Flat` and installs an exact owner-local generation before shared routing. Policy skip or pre-sink failure sends an exact abort through the sole release reducer; a synchronous terminal callback wins over route completion; `Submitted` and `SinkInvokedUnknown` retain it.
...
11. A `SinkInvoked` record absent from a complete projection remains reserved and exposes a nonzero sink-invoked count. After one exact open observation advances it to `ObservedOpen`, later projection omission carries the same ledger reservation, moves the unresolved count between the two phase fields, and remains unresolved rather than releasing it.
12. Incomplete projection, rejected rebuild input, fill-evidence-integrity failure, missing capital state, duplicate attribution, component refresh, and evidence-write failure preserve the record and numerical liability while invalidating admission reconciliation. Rejection evidence reports the preserved live liability. While invalid and with a stale pool snapshot, exact `Reserved` rollback and purpose-built, evidence-recorded exact terminal retirement remove ledger and record together; stale-revision or mismatched removal and terminal-evidence write failure preserve both. Exact terminal cache proof still retires when its Unix/event timestamp is equal to or older than the retained diagnostic timestamp.
13. A terminal callback that retires a record before a stale candidate rebuild completes cannot be reinserted because the admission-state revision changed.
...
- replaces `capital_admission_nt_projection_epoch` with the single admission-state revision and adds exact unreconciled rollback and terminal-retirement ledger operations without a second counter or liability authority;
...
- threads the tracked coordinator's existing intent/scope selections and lifecycle dispositions through `route_tracked_cancel`/`route_tracked_cancel_all` instead of discarding them, and renames maker results to `CancelIntentHandled`/`CancelScopeHandled` so clearing a planning binding cannot be read as terminal or NT-route evidence;
...
- updates the current `CapitalAdmissionRebuildV1` schema, generated contract, codecs, and corpora for the two phase-specific unresolved-reservation counts;
...
- behavior evidence that unreconciled-but-populated ledgers still permit exact `Reserved` rollback and state-revision-gated terminal retirement without pool-freshness checks while rejecting new admission, and that both omitted nonterminal phases remain numerically carried;
- current-evidence contract regeneration, codec/domain/round-trip checks, and frozen-corpus updates for the phase-specific unresolved-reservation counts, with no new release identity;
