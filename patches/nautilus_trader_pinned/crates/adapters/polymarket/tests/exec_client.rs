...
#[rstest]
#[tokio::test]
async fn test_submit_market_order_buy_honors_caller_price_cap() {
    let state = TestServerState::default();
    // Book crosses at 0.50, but the caller supplies a looser 0.60 protection price.
    *state.book_response.lock().await = Some(json!({
        "bids": [{"price": "0.48", "size": "100.00"}],
        "asks": [{"price": "0.50", "size": "100.00"}]
    }));
    ...
    let mut order = make_market_order("O-MKT-CAP", instrument_id, OrderSide::Buy, true);
    match &mut order {
        OrderAny::Market(market) => {
            market.protection_price = Some(Price::from("0.60"));
        }
    ...
    let body = state
        ...
        .await
        .clone()
        .expect("posted order body should be captured");
    assert_eq!(body["order"]["makerAmount"], "10000000");
    assert_eq!(body["order"]["takerAmount"], "16666666");
}
