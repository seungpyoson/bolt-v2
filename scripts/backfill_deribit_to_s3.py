...
# Deribit lists SOL/XRP/BNB (and other) linear futures+options under the
# settlement currency (USDC), not under the base ticker, so a base-only
# get_instruments sweep silently misses every altcoin contract. These host
# currencies are queried in addition to the requested bases; output is still
# filtered to the requested base_currency set.
LINEAR_HOST_CURRENCIES = ("USDC",)
...
def collect_universe(
    ...
    query_currencies = list(dict.fromkeys((*currencies, *LINEAR_HOST_CURRENCIES)))
...
class RateLimiter:
    """Thread-safe minimum-interval gate shared across the trade-history walkers
    so the combined get_last_trades_by_instrument request rate stays under the
    Deribit public tier ceiling regardless of how many instruments walk at once."""
    ...
    def acquire(self) -> None:
        ...
        with self._lock:
            ...
            start = now if wait <= 0 else self._next_allowed
...
def collect_trade_history(
    ...
    *,
    start_ms: int,
    end_ms: int,
    max_trade_batches_per_instrument: int,
    trade_request_min_interval: float,
    trade_walk_workers: int,
) -> list[dict[str, Any]]:
    """Walk each selected instrument's full trade history backward by trade_seq
    via public/get_last_trades_by_instrument (the only Deribit REST path that
    actually pages historical trades; the *_and_time variant returns empty for
    historical windows). Each raw 1000-trade batch is staged locally with a
    dt=<batch data date> attribute and pushed to S3 by the run's end-of-run
    recursive uploader (one aws cp instead of one subprocess per batch, which
    would otherwise bottleneck high-volume perpetuals far below the API rate).
    The walk stops once a batch's newest trade predates window_start, so it
    never pulls all-time history. Instruments walk concurrently behind a shared
    rate limiter; the per-instrument seq cursor stays strictly sequential."""
    jobs = [
        (product_family, str(row["instrument_name"]))
        for product_family, rows in selected.items()
        for row in rows
        if product_family in ("perpetual", "future", "option")
    ]
    limiter = RateLimiter(trade_request_min_interval)
    progress = {"done": 0}
    progress_lock = threading.Lock()

    def walk_one(job: tuple[str, str]) -> list[dict[str, Any]]:
        product_family, instrument = job
        out: list[dict[str, Any]] = []
        end_seq: int | None = None
        batches = 0
        try:
            while batches < max_trade_batches_per_instrument:
                params: dict[str, Any] = {
                    "instrument_name": instrument,
                    "count": 1000,
                    "include_old": "true",
                ...
                limiter.acquire()
                url, status, headers, payload, parsed = request_json("get_last_trades_by_instrument", params)
                result = parsed.get("result") or {}
                rows = result.get("trades") if isinstance(result, dict) else None
                rows = rows if isinstance(rows, list) else []
                if not rows:
                    break
                times = row_times(rows)
                in_window_times = [t for t in times if start_ms <= t <= end_ms]
                batch_date = ms_to_utc(max(in_window_times) if in_window_times else max(times)).split("T", 1)[0]
                seqs = [int(row["trade_seq"]) for row in rows if isinstance(row, dict) and row.get("trade_seq") is not None]
                out.append(
                    upload_payload(
                        scratch_root=scratch_root,
                        s3_prefix=s3_prefix,
                        run_id=run_id,
                        family="trades_seq_history",
                        attrs={
                            "product_family": product_family,
                            "instrument": instrument,
                            "dt": batch_date,
                        ...
                        extra={
                            ...
                            "schema_sample": schema_sample(rows),
                            "coverage": coverage_from_rows(rows),
                            "trade_seq_min": min(seqs) if seqs else None,
                            "trade_seq_max": max(seqs) if seqs else None,
                            "in_window_trade_count": len(in_window_times),
                        },
                    )
                )
                batches += 1
                oldest = min(times) if times else start_ms
                if oldest < start_ms or not result.get("has_more") or not seqs:
                    break
                end_seq = min(seqs) - 1
        except Exception as exc:  # noqa: BLE001 - one instrument failing must not abort the venue walk.
            out.append(
                error_record(
                    "trades_seq_history",
                    {"product_family": product_family, "instrument": instrument, "batches_completed": str(batches)},
                    exc,
                )
            )
        if PROGRESS_EVERY > 0:
            ...
            if done % max(1, PROGRESS_EVERY // 25) == 0 or done == len(jobs):
                print(f"trade-history: {done}/{len(jobs)} instruments walked", file=sys.stderr, flush=True)
        return out

    records: list[dict[str, Any]] = []
    if not jobs:
        return records
    workers = max(1, trade_walk_workers)
    if workers <= 1:
        for job in jobs:
            records.extend(walk_one(job))
        return records
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        for batch_records in executor.map(walk_one, jobs):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--max-trade-batches-per-instrument",
        type=int,
        default=20000,
        help="Per-instrument cap on 1000-trade seq-walk batches; the walk also stops at window_start. 0 disables the seq-walk trade-history family.",
    )
    parser.add_argument(
        "--trade-request-rps",
        type=float,
        default=18.0,
        help="Upper bound on get_last_trades_by_instrument requests/sec for the seq-walk (Deribit public tier is 20 RPS).",
    )
    parser.add_argument(
        "--trade-walk-workers",
        type=int,
        default=8,
        help="Instruments walked concurrently in the seq-walk; the shared rate limiter still caps the combined request rate at --trade-request-rps.",
    )
...
def main() -> int:
    ...
    run_seed = stable_json(
        {
            ...
            "max_trade_batches_per_instrument": args.max_trade_batches_per_instrument,
            "trade_request_rps": args.trade_request_rps,
            "trade_walk_workers": args.trade_walk_workers,
    ...
    if args.max_trade_batches_per_instrument > 0:
        all_records.extend(
            collect_trade_history(
                ...
                end_ms=end_ms,
                max_trade_batches_per_instrument=args.max_trade_batches_per_instrument,
                trade_request_min_interval=(1.0 / args.trade_request_rps) if args.trade_request_rps > 0 else 0.0,
                trade_walk_workers=args.trade_walk_workers,
            )
        )
    ...
    manifest: dict[str, Any] = {
        ...
        "bounds": {
            ...
            "trade_request_rps": args.trade_request_rps,
            "trade_walk_workers": args.trade_walk_workers,
            "linear_host_currencies": list(LINEAR_HOST_CURRENCIES),
            "request_workers": args.request_workers,
            "http_retries": args.http_retries,
            "request_delay_seconds": args.request_delay_seconds,
            "progress_every": args.progress_every,
        ...
        "known_gaps": [
            "trades_seq_history walks get_last_trades_by_instrument backward by trade_seq to window_start (the only REST path that pages historical trades); per-instrument batches are capped, so very high-volume instruments that hit the cap before window_start cover only the most recent in-window days. The legacy trades/trades_recent_probe/get_last_trades_by_instrument_and_time families are retained for provenance but return empty for historical windows.",
