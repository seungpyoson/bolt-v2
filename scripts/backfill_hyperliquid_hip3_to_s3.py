...
def source_family_slug(body: dict[str, Any]) -> str:
    ...
    if request_type == "candleSnapshot":
        req = body.get("req", {})
        return (
            f"info.candleSnapshot.coin={partition_value(str(req.get('coin')))}"
            f".interval={partition_value(str(req.get('interval')))}"
        )
...
def candle_rows_from_payload(
    ...
    if not isinstance(payload, list):
        raise ValueError(f"candleSnapshot payload for {instrument_name} {interval} is not a list")
    rows = []
    for item in payload:
        if not isinstance(item, dict):
            continue
        ...
        rows.append(
            {
                ...
                "bar_source_type": "provider_supplied",
                "interval": interval,
                "requested_start_utc": requested_start,
                "requested_end_utc": requested_end,
                "open_time": open_time,
                "open_time_utc": utc_from_millis(open_time) if isinstance(open_time, int) else None,
                "close_time": close_time,
                "close_time_utc": utc_from_millis(close_time) if isinstance(close_time, int) else None,
                "symbol": item.get("s"),
                "open": item.get("o"),
                "high": item.get("h"),
                "low": item.get("l"),
                "close": item.get("c"),
                "base_volume": item.get("v"),
...
def candle_coverage_record(
    ...
    rows = payload if isinstance(payload, list) else []
    times = [item.get("t") for item in rows if isinstance(item, dict) and isinstance(item.get("t"), int)]
    first_time = min(times) if times else None
    last_time = max(times) if times else None
    return {
        "venue": VENUE,
        ...
        "first_open_time_utc": utc_from_millis(first_time),
        "last_open_time": last_time,
        "last_open_time_utc": utc_from_millis(last_time),
        "raw_payload_hash": raw_record["payload_hash"],
        "raw_bytes": raw_record["bytes"],
        "coverage_statement": (
            "bounded_response_rows_only"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--candle-intervals",
        help=(
            "Comma-separated candleSnapshot intervals to pull over the full requested window, "
            "for example 1h,4h,1d. Sub-hourly intervals are capped by the source to the most "
            "recent rows and cannot cover the full window. Empty disables candle pulls."
        ),
    )
    parser.add_argument("--candle-start-utc")
    parser.add_argument("--candle-end-utc")
...
def main() -> int:
    ...
    candle_intervals = [item.strip() for item in (args.candle_intervals or "").split(",") if item.strip()]
    candle_start_ms = millis(args.candle_start_utc) if args.candle_start_utc else window_start_ms
    candle_end_ms = millis(args.candle_end_utc) if args.candle_end_utc else window_end_ms
    candle_start_utc = args.candle_start_utc or args.window_start_utc
    candle_end_utc = args.candle_end_utc or args.window_end_utc
    if candle_intervals and not (window_start_ms <= candle_start_ms < candle_end_ms <= window_end_ms):
        raise ValueError("candle window must be within the requested UTC window")
    ...
    candle_coverage: list[dict[str, Any]] = []
    for row in instrument_rows:
        for interval in candle_intervals:
            ...
            try:
                ...
                candle_coverage.append(
                    candle_coverage_record(
                        str(row["dex_name"]),
                        str(row["instrument_name"]),
                        interval,
                        candle_start_utc,
    ...
    candle_artifact = write_jsonl(
        local_path(scratch_root, "staged", "v1", "table=bars", f"run={run_id}", "part-000000.jsonl"),
        candle_rows,
    )
    candle_coverage_artifact = write_json(
        local_path(scratch_root, "source-proof", "v1", f"run={run_id}", "candle-coverage.json"),
        candle_coverage,
    )

    candle_families_uploaded = (
        [f"info.candleSnapshot.interval={interval}" for interval in candle_intervals]
        if candle_intervals
        else []
    )
    source_proof = {
        ...
        "candle_tranche": {
            ...
            "start_utc": candle_start_utc,
            "end_utc": candle_end_utc,
        },
        "candle_instrument_request_count": len(instrument_rows) if candle_intervals else 0,
        "candle_row_count": len(candle_rows),
        ...
        "gaps_and_unproven_families": [
            ...
            "candles are provider_supplied bars and do not satisfy native trades or order books",
            "candleSnapshot sub-hourly intervals are source-capped to recent rows and cannot cover the full window; only single-request-complete intervals such as 1h, 4h, 1d cover the requested window",
            "delisted HIP-3 instruments return zero candle rows over the requested window",
        ],
    }
    ...
    local_artifacts.extend(
        [
            ...
            (
                pathlib.Path(candle_artifact["path"]),
                s3_uri(s3_prefix, "staged", "v1", "table=bars", f"run={run_id}", "part-000000.jsonl"),
            ),
            (
                pathlib.Path(candle_coverage_artifact["path"]),
                s3_uri(s3_prefix, "source-proof", "v1", f"run={run_id}", "candle-coverage.json"),
            ),
            (
    ...
    manifest = {
        ...
        "counts": {
            ...
            "candle_instruments_requested": len(instrument_rows) if candle_intervals else 0,
            "candle_intervals_requested": len(candle_intervals),
        ...
        "bytes": {
            ...
            "staged_candle_bytes": candle_artifact["bytes"],
            "source_proof_bytes": source_proof_artifact["bytes"],
            "funding_coverage_bytes": coverage_artifact["bytes"],
            "candle_coverage_bytes": candle_coverage_artifact["bytes"],
        ...
        "candle_coverage_s3_uri": s3_uri(s3_prefix, "source-proof", "v1", f"run={run_id}", "candle-coverage.json"),
        ...
        "staged_candle_s3_uri": s3_uri(s3_prefix, "staged", "v1", "table=bars", f"run={run_id}", "part-000000.jsonl"),
