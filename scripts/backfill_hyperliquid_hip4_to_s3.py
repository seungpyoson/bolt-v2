...
def candle_rows(
    ...
    coverage = {
        ...
        "raw_payload_hash": payload_record["payload_hash"],
        "raw_bytes": payload_record["bytes"],
        "coverage_statement": "bounded_api_response_rows_only_max_5000_candles_per_official_docs",
    }
    return rows, coverage
...
def main() -> int:
    ...
    if args.candle_interval not in CANDLE_INTERVALS:
        raise ValueError(f"unsupported candle interval: {args.candle_interval}")

    generated_at = utc_now()
    run_seed = f"{generated_at}|{s3_prefix}|{args.window_start_utc}|{args.window_end_utc}|{args.candle_interval}"
    ...
    for side in side_rows:
        ...
        try:
            ...
            errors.append({"source_family": "info.candleSnapshot", "trade_coin": side["trade_coin"], "error": repr(exc)})
        time.sleep(args.request_sleep_seconds)
    ...
    source_proof = {
        ...
        "gaps_and_unproven_families": [
            ...
            "recentTrades payloads are current recent responses only",
            "candleSnapshot payloads are bounded API responses and remain subject to the official 5000-candle limit",
    ...
    manifest = {
        ...
        "commands_or_checks_run": [
            ...
            f"official API POST /info type=candleSnapshot interval={args.candle_interval} for each active outcome side",
