...
def assert_publish_rejects_a_moved_pr_head() -> None:
    ...
    def opener(request: object, **_kwargs: object) -> FakeResponse:
        ...
        if method == "GET":
            return FakeResponse({"head": {"sha": "2" * 40}})
...
def assert_publish_checks_head_then_posts() -> None:
    ...
    def opener(request: object, **_kwargs: object) -> FakeResponse:
        ...
        observed.append(method)
        if method == "GET":
            return FakeResponse({"head": {"sha": expected}})
        if method == "POST":
...
def assert_every_provider_is_text_only() -> None:
    ...
    for entry in providers:
        ...
        payload = module.chat_payload(provider, "untrusted diff")
        if "tools" in payload or payload["messages"] != [{"role": "user", "content": "untrusted diff"}]:
