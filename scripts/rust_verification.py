...
def managed_remote_fast_linker_env(policy: dict[str, Any]) -> dict[str, str]:
    ...
    if selected not in programs:
        return {}
    if not fast_linker_program_available(selected):
        return {}
    return {"RUSTFLAGS": f"-C link-arg=-fuse-ld={selected}"}


def fast_linker_program_available(program: str) -> bool:
    return shutil.which(program) is not None or shutil.which(f"ld.{program}") is not None
