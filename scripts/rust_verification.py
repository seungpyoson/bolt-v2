...
Advisory CI is pushed-head evidence, not merge authority.
"""
...
def validate_policy_data(data: dict[str, Any]) -> None:
    ...
    unexpected_test_keys = set(test_command) - {"cargo_args"}
    if unexpected_test_keys:
        raise PolicyError(f"commands.test contains unexpected keys: {sorted(unexpected_test_keys)!r}")
...
def validate_remote_fast_linker_policy(data: dict[str, Any]) -> None:
    ...
    if programs != ["mold"]:
        raise PolicyError("remote_fast_linker.programs must select only mold")
...
def managed_env(repo: pathlib.Path, policy: dict[str, Any] | None = None) -> dict[str, str]:
    ...
    env.update(managed_remote_fast_linker_env(repo, data))
    return env
...
def managed_remote_fast_linker_env(repo: pathlib.Path, policy: dict[str, Any]) -> dict[str, str]:
    ...
    if selected not in programs:
        raise PolicyError(f"remote Rust commands require {linker_policy['linker_env']}={programs[0]}")
    ...
    if shutil.which(selected, path=filtered_path) is None:
        raise PolicyError(f"remote Rust commands require configured linker {selected}")
    real_cc = shutil.which("cc", path=filtered_path)
    if real_cc is None:
        raise PolicyError("remote Rust commands require cc for the configured linker wrapper")
    ...
    if real_cc_key == wrapper_cc_key:
        raise PolicyError("remote Rust linker wrapper resolved recursively")
    ...
    wrapper.write_text(
        "\n".join(
            [
                "#!/usr/bin/env bash",
                "set -euo pipefail",
                f"real_cc={shlex.quote(real_cc)}",
                f"fast_linker={shlex.quote(selected)}",
                "skip_next=0",
                'for arg in "$@"; do',
                '  if [[ "$skip_next" == "1" ]]; then',
                "    skip_next=0",
                ...
                "      ;;",
                "    -fuse-ld=*)",
                '      echo "refusing alternate linker: $arg; required: -fuse-ld=$fast_linker" >&2',
                "      exit 2",
                "      ;;",
                "    -c|-S|-E|-M|-MM|--help|--help=*|--version|-###|-dumpmachine|-dumpversion|-dumpfullversion|-dumpspecs|-print-*)",
                '      exec "$real_cc" "$@"',
                "      ;;",
                "  esac",
                "done",
                'exec "$real_cc" "-fuse-ld=$fast_linker" "$@"',
                "",
            ]
        ),
        encoding="utf-8",
    ...
    *,
    ...
    if any(path.startswith("crates/") for path in normalized):
        return [
            ...
            "Use advisory CI for complete pushed-head Rust evidence.",
        ]
    return [
...
def evaluate_rust_probe_run(
    ...
    *,
    ...
    if state == "pass":
        ...
        print("DIAGNOSTIC ONLY -- Rust Probe is debugging feedback, not merge authority")
        return 0
    conclusion = str(run.get("conclusion") or "")
    ...
    print("DIAGNOSTIC ONLY -- Rust Probe is debugging feedback, not merge authority", file=sys.stderr)
    return 1
    ...
    if args.command == "test":
        try:
            with cache_lock(policy, exclusive=cargo_args_need_exclusive_cache_lock(command_cargo_args)):
                env = managed_env(repo, policy)
                return run_process(["cargo", *command_cargo_args], repo=repo, env=env)
        except CacheLockTimeoutError as exc:
            return print_refusal(refusal_payload(code="cache_lock_timeout", reason=str(exc), dry_run=False))
