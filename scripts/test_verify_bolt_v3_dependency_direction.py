...
def run_default_cli(root: Path, allowances, baseline) -> tuple[int, str, str]:
    """Run the normal CLI path with an injected mainline baseline."""

    original_root = VERIFIER.REPO_ROOT
    original_allow = VERIFIER.FINDING_ALLOWANCES
    original_read = VERIFIER._read_baseline_source
    ...
    try:
        VERIFIER.REPO_ROOT = root
        VERIFIER.FINDING_ALLOWANCES = allowances
        VERIFIER._read_baseline_source = lambda: baseline
        ...
        VERIFIER.REPO_ROOT = original_root
        VERIFIER.FINDING_ALLOWANCES = original_allow
        VERIFIER._read_baseline_source = original_read
...
def test_real_committed_allowlist_is_empty() -> None:
    if VERIFIER.FINDING_ALLOWANCES:
        raise AssertionError(
            f"dependency decomposition must leave no allowances: {VERIFIER.FINDING_ALLOWANCES!r}"
        )
...
def test_default_cli_mechanically_enforces_shrink_only() -> None:
    current = (allowance("src/bolt_v3_a.rs", "strategies::x::A"),)
    baseline = baseline_source({("src/bolt_v3_a.rs", "strategies::x::A")})
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        write_file(root, "src/bolt_v3_a.rs", "use crate::strategies::x::A;\n")
        code, out, err = run_default_cli(root, current, baseline)
    if code != 0 or err:
        raise AssertionError(f"expected normal CLI PASS, got {code}: {err!r}")
    for expected in (
        "dependency-direction verifier passed",
        "allowlist is a subset of the mainline baseline",
    ):
        if expected not in out:
            raise AssertionError(f"normal CLI did not enforce {expected}: {out!r}")
...
def main() -> int:
    tests = [
        ...
        test_real_committed_allowlist_is_empty,
        ...
        test_default_cli_mechanically_enforces_shrink_only,
