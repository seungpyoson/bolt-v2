...
RUNTIME_CONTRACT_PIN_SECTIONS = (
    ...
    (
        ...
        re.compile(
            r"^Current status: this branch pins the official NautilusTrader repository at\s+merge commit `([0-9a-f]{40})` for upstream PR #4474\.",
            re.MULTILINE,
        ),
    ),
)
...
def scan_forbidden_nt_sources(root: Path, findings: list[str]) -> None:
    result = subprocess.run(
        ["git", "-C", str(root), "ls-files", "--cached", "-z"],
        check=False,
        stdout=subprocess.PIPE,
    ...
    if result.returncode != 0:
        detail = result.stderr.decode("utf-8", errors="replace").strip()
        findings.append(
            "NautilusTrader source guard could not discover tracked files: "
            f"{detail or f'git ls-files exited {result.returncode}'}"
        )
        return

    needle = FORBIDDEN_NT_REPOSITORY_SLUG.encode()
    for raw_path in result.stdout.split(b"\0"):
        if not raw_path:
            continue
        relative = Path(os.fsdecode(raw_path))
        if relative.is_absolute() or ".." in relative.parts:
            findings.append(f"NautilusTrader source guard rejected unsafe tracked path: {relative}")
            continue
        try:
            content = (root / relative).read_bytes()
        except FileNotFoundError:
            # A tracked path deleted or renamed in the worktree is not an active
            # dependency/source surface. Its replacement is scanned once staged.
            continue
        except OSError as error:
            findings.append(
                f"NautilusTrader source guard could not read tracked file {relative}: {error}"
            )
            continue
        if needle in content:
            findings.append(
                f"{relative}: NautilusTrader source guard forbids the personal repository"
            )
