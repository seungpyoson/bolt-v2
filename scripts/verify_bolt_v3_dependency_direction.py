...
- ADDING an allowance is rejected mechanically: normal source-fence execution
  also runs the `--check-shrink-only-vs-main` check and fails unless the in-tree
  allowlist is a subset of the one on `origin/main`. A new back-reference is a
  bug to fix, not to allow. (This is a no-op on the PR that first introduces the
  fence — there is no mainline baseline yet — and active on every PR thereafter.)
...
# The dependency-direction decomposition is complete. Keep this tuple empty:
# a new back-reference is a bug to fix, not an allowance to add.
...
def main(argv: list[str] | None = None) -> int:
    enforce_shrink_only = argv is None
