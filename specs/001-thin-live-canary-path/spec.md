...
4. **Given** an entry, risk-reducing exit, or replace-submit candidate, **When** submit admission runs, **Then** the candidate carries an explicit submit intent kind before admission evaluates count and notional policy.
...
- **FR-009**: Submit admission MUST classify every live submit candidate with an explicit intent kind before NT submit. Entry and replace-submit candidates MUST consume `BoltV3LiveCanaryGateReport.max_live_order_count` and `max_notional_per_order`. Risk-reducing exit candidates MUST consume `max_notional_per_order` and MAY exceed `max_live_order_count` only when the strategy config-derived lifecycle policy explicitly permits one risk-reducing exit after a prior admitted exposure-opening submit. Plain cancel requests are not live submit candidates.
