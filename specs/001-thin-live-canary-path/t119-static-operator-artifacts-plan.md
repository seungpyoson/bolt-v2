...
- `ssm-manifest.json`: deterministic redacted inventory of configured SSM references. It should include non-secret config identity, client key, provider key, field name, AWS region, and `ssm_path_sha256`. It should not include raw secret values. External review must decide whether raw SSM paths are acceptable; default plan is path hashes only.
...
- `approval-nonce.json`: one-shot nonce evidence with schema/version/record kind and a generated nonce hash. Raw nonce material must not be printed.
...
- `static-artifacts-manifest.json`: helper-owned index of generated paths and SHA-256s so the operator can copy values into `[live_canary.operator_evidence]` without manual hashing.
...
4. RED: helper generates approval nonce with create-new semantics and refuses overwrite; stdout omits nonce material.
   GREEN: add nonce file schema and writer.
...
- Is path-hash-only redaction acceptable for the SSM manifest, or should raw SSM paths be included in the artifact while still not printing secrets?
