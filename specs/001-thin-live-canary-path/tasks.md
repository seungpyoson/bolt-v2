...
- [ ] T118 [US5] Define and enforce a non-circular approval-envelope schema. The envelope file must not need to contain its own final SHA or any root TOML hash that depends on the envelope SHA stored in root TOML; tests/docs must show exact hash ownership.
- [ ] T119 [US5] Add production helper coverage for static operator artifacts that can be derived without live submit: redacted SSM manifest, financial envelope from loaded TOML, approval nonce, and abort plan. No helper may print secret values or require non-SSM secret sources.
- [ ] T120 [US5] Add production strategy-input evidence generation or block T046 explicitly. The generator must use source-bound market selection, implemented price-to-beat input, and strategy decision inputs; a no-submit transport path that clears strategies cannot be used as proof of strategy-input generation.
- [ ] T121 [US5] Add production pre-run state evidence generation or block T046 explicitly. Evidence must cover host clock, venue account state, market/window approval, funding/margin, single-runner lock, egress identity, CLOB V2 checks, and release manifest.
