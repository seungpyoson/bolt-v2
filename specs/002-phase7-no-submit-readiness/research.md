...
**Implementation constraints**: Cache inspection uses the configured live-node timeout as a bounded recheck window, then fails closed on missing required instruments. No new cache-poll interval is introduced. Stop must run and be reported after every start attempt, including reference-cache failure or partial startup failure. Strategy `on_start()` side effects remain NT-owned startup behavior; Phase 7 must verify no bolt-v3 submit path is reachable because submit admission is unarmed, while feed freshness and strategy-input values remain Phase 8 safety-audit scope.
