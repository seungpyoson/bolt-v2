...
- [x] T039 Move `src/bolt_v3_tiny_canary_evidence.rs` one-live-order cap out of code ownership; behavior lock: focused Phase 8 evidence tests plus literal verifier classification. Current disposition: local implementation now compares proof/preflight to `[live_canary].max_live_order_count` from TOML; targeted tests and runtime literal verifier pass; Gemini/Claude/DeepSeek/GLM post-implementation review at `b897dd6` approved with no blockers.
