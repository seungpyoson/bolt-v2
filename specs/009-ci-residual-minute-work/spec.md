...
As maintainer, I can later require the current CI gate contexts without docs-only PRs becoming permanently pending when CI is skipped by `paths-ignore`.
...
- **FR-008**: The workflow verifier/self-tests MUST cover pass-stub required job names, changed-file classifier wiring, safe-pattern drift, required docs rows, and non-weakening of real CI gates.
...
- **PassStubGate**: `build`, `clippy`, `test`, and `gate` jobs that run the changed-file classifier for docs-only required-check compatibility.
