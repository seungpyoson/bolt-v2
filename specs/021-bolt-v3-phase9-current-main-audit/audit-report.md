...
| P9-BLOCKER-003 | blocker | Active config | This worktree has an ignored local operator config, but its approved no-submit attempt did not pass readiness. | `ls -l config/live.local.toml config/root.toml config/strategies/binary_oracle.example.toml` shows ignored `config/live.local.toml` plus tracked examples; `git check-ignore -v config/live.local.toml` shows it is ignored; the readiness report failed `controlled_connect` and skipped `reference_readiness`. | No live/no-submit operator claim from this checkout until the ignored operator config produces a satisfied approved report and checksum-bound evidence. |
