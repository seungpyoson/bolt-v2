...
- T064 GREEN: the same test passed after adding StopLimit NT-model validation and constructing the order through `OrderFactory::stop_limit`. The regression proves the compiled NT `StopLimit` preserves `price`, `trigger_price`, TIF, post-only/reduce-only/quote flags, and submit admission notional from `Order::price()`.
...
- Exact-head gate state: this StopLimit slice is local and unpushed when this evidence was recorded. Post-commit/push GitHub checks, no-mistakes state, and external post-implementation review are still required before claiming StopLimit support on the PR head.
