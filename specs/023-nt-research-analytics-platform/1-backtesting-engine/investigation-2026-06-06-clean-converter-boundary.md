...
- Fresh read-only recheck on 2026-06-07 returned no non-secret SSM parameter names containing `artifact`, `s3`, `backtest`, or `nt`, and `aws s3 ls s3://bolt-parquet/nt-research-analytics/ --summarize --recursive` still reported `Total Objects: 0`, `Total Size: 0`
