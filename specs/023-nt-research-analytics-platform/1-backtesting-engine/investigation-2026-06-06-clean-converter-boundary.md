...
- Current BTE implementation audit after selecting that policy: the source-proof
  ...
  lacks a `nautilus-polymarket` dependency and `catalog_projection.rs` still
  writes `CurrencyPair` plus `TradeTick` only. The next implementation gate is
  therefore PMXT binary-option L2 catalog projection/read-back using NT classes,
  not a venue-specific branch.
...
- BTE-022 now has an explicit open mapping-status artifact:
  ...
  `OrderBookDelta` for typed NT data-config construction, while catalog
  projection/read-back proof remains limited to the accepted `TradeTick` path.
  Other data classes must continue to fail closed until typed projection and
