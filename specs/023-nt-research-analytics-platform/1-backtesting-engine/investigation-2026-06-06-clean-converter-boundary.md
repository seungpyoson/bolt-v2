...
| --- | --- | --- |
| Main reconciliation | This branch is divergent from current `main`; read-only merge simulation shows no textual conflict, but final implementation evidence is not authoritative until it is reconciled with `main` | rebase or fresh-port the accepted branch commits onto current `main`, then rerun the focused BTE checks and any source-fence/repo guard before PR/completion claims |
