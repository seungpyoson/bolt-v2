...
| --- | --- | --- | --- |
| converter raw archive -> NT data | No for this project's raw-source proof and CSV archive normalization; yes once rows are NT typed data | NT `ParquetDataCatalog::write_to_parquet<T>` requires typed records implementing NT persistence traits, not project source-proof acceptance or CSV column mapping (`crates/persistence/src/backend/catalog.rs:505-575`). BTE owns the generic CSV native-trade mapping and registered converter (`crates/backtesting-vertical-slice/src/canonical_trades.rs:67-104`, `574-586`). | Bolt owns only the raw-source-to-canonical adapter and source-proof gate; venue/source values stay in TOML/source proof data. |
