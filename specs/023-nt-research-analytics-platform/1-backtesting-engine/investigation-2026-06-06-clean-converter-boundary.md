...
- BTE-022 status is now recorded in `reference/source-proof-nt-catalog-mapping-status.backtesting-engine-022.2026-06-08.json`: pinned NT provides the required `ParquetDataCatalog`, `BacktestNode`, `OrderBookDelta`/`OrderBookDepth10`/`TradeTick`, and Polymarket instrument/parser surfaces; the isolated BTE slice now has TDD-proven manifest/data-config mapping for `OrderBookDelta` and configured `instrument_ids`, NT-native `BinaryOption`/`OrderBookDelta`/`TradeTick` catalog write/read/logical-hash coverage, a generic TOML/ledger-driven first-proof selector over configured event-family roles and row budgets, and L2 result-contract validation for `event_count_ledger_hash` plus `selected_asset_ids_hash`. It still lacks machine-checked Polymarket adapter reuse, PMXT source-row projection, concrete PMXT selector-to-run wiring, and BacktestNode proof over a selected PMXT catalog.
...
- Current BTE implementation audit after selecting that policy: the source-proof
  ...
  for `BinaryOption`, `OrderBookDelta`, and `TradeTick` fixture data. The new
  ...
  `selected_asset_ids_hash`. The next implementation gate is therefore
  concrete selector-to-run wiring and PMXT source-backed binary-option L2 catalog
  projection using NT classes, not a venue-specific branch.
