...
- `s3://bolt-parquet/nt-research-analytics/` now contains SSM-backed raw/evidence staging proof objects only: one 1,066,394-byte Binance daily raw ZIP under `raw/` plus three source-proof evidence objects totaling 1,005 bytes under `source-proofs/`; no clean backtest catalog/result output artifacts have been published there yet
...
- no real S3 write has been performed through the main `--publish-output` catalog/result path in this branch yet; only the bounded raw-object and source-proof-evidence staging CLIs have written through the Rust SSM/object-store path
...
| Clean production output proof under `nt-research-analytics/` | The prefix now has raw/evidence staging objects, but still no clean catalog/result output artifacts from the main operator publish path | run the accepted BNBUSDC object through the operator into the configured prefix with `[manifest.artifact_store.ssm_parameters]`, upload checkpoint/manifest/metadata/catalog/contract, then verify S3 listing and hashes |
| Artifact-store SSM parameter paths | The paths now exist and have been proven by real S3 raw/evidence staging writes, but the main catalog/result publish path still needs to consume them | keep only the SSM parameter names in TOML (`/bolt/artifact-store/s3/access-key-id` and `/bolt/artifact-store/s3/secret-access-key` in `us-east-1`); never put secret values in TOML or logs; use those paths for the next `--publish-output --prove-published-catalog` run |
...
| Direct S3 catalog execution proof | The proof path is implemented to pass resolved object-store options into NT, and the same SSM/object-store path has now written raw/evidence staging objects to real S3, but the main operator has not yet published and read back a real S3 catalog/result set | run `--publish-output --prove-published-catalog` with the configured SSM paths, verify S3 artifact hashes, and require the published-catalog proof to stamp `direct_s3_catalog_access_proven = true` |
