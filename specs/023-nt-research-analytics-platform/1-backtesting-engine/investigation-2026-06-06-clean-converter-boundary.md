...
| Broader NT data-query surface | The current manifest maps NT `BacktestDataConfig` catalog path/storage, `TradeTick`, and one `instrument_id`, but the pinned NT struct also supports `instrument_ids`, data-config `start_time`/`end_time`, `filter_expr`, `client_id`, metadata, `bar_spec`, `bar_types`, and `optimize_file_loading` | keep the slice single-instrument trade replay until each additional NT query knob has a typed manifest field, source-proof/fidelity implication, resolved-surface record, and result-contract claim limit; do not add ad hoc query strings or hidden defaults |
