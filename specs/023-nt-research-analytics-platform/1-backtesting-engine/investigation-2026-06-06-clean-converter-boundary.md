...
- `s3://bolt-parquet/nt-research-analytics/` now contains SSM-backed staging proof objects plus one clean reference backtest publication: the Binance daily raw ZIP and three source-proof evidence objects are staged under `raw/`/`source-proofs/`, and the accepted BNBUSDC reference tranche has 10 catalog/result artifacts under `backtests/backtesting-vertical-slice-bnbusdc-2026-03-01/`
...
- one real S3 write has now been performed through the main `--publish-output --prove-published-catalog` catalog/result path for the accepted BNBUSDC reference tranche; this proves the clean S3 path for that slice only, not broad historical backfill
...
- the S3 publish/proof run stamps `direct_s3_catalog_access_proven = true` in `published-catalog-proof.json` with `nt_iterations = 937` and `expected_iterations = 937`; earlier local-only proofs remain forensic evidence only
- the accepted-tranche operator proof now proves deterministic local conversion/catalog/backtest behavior plus direct production S3 catalog readback for one 8.5 KB reference object; it does not prove broad historical backfill, multi-day/multi-instrument coverage, or Binance production acceptance
...
- Official Binance public-data documentation checked on 2026-06-07 confirms the archive is public market data exposed through Binance Data Collection, split into `daily` and `monthly` files, with new daily data available the next day, spot timestamps in microseconds from 2025-01-01 onward, `trades` files sourced from `/api/v3/historicalTrades`, and `.CHECKSUM` sidecars for each ZIP (`https://github.com/binance/binance-public-data`). The same README lists `Licence: MIT`, but no separate data-license or terms artifact was found for `data.binance.vision`; this is evidence for source legitimacy and checksum semantics, not enough by itself to pass the source-proof `license` check. User confirmed on 2026-06-07 that Binance BD relationship approval is available; that can satisfy the generic `license` check only after the written approval/attestation is stored as a durable evidence pointer and hash in the source proof.
...
- Fresh recheck on 2026-06-07 kept that conclusion: `curl -sS -I https://data.binance.vision/data/spot/daily/trades/BNBUSDC/BNBUSDC-trades-2026-03-01.zip` returned HTTP 200, `content-length = 1066394`, and `last-modified = Mon, 02 Mar 2026 01:44:18 GMT`; the `.CHECKSUM` sidecar returned `433d32b8d828abee5e1937e01372d16f7edadc14c41fe736b0b9577541fa5e81  BNBUSDC-trades-2026-03-01.zip`. The expected staged daily key under `s3://bolt-parquet/backfill-staging/2026-06-01/binance/raw/v1/source=data.binance.vision/product=spot/frequency=daily/family=trades/symbol=BNBUSDC/dt=2026-03-01/object=433d32b8d828abee5e1937e01372d16f7edadc14c41fe736b0b9577541fa5e81.zip` returned S3 `HeadObject` 404. Therefore the next small Binance source proof is blocked on staging that exact daily object and checksum evidence, while the existing staged monthly object would require a distinct monthly source-binding/proof decision and a larger conversion budget.
- Bounded local inspection of the daily Binance ZIP downloaded only the 1,066,394-byte public object to `/private/tmp/bte-binance-bnbusdc-2026-03-01.zip`; SHA256 matched the official checksum, `unzip -l` showed one member `BNBUSDC-trades-2026-03-01.csv` with decoded length 5,287,070 bytes, `wc -l` returned 71,431 rows, the first/last trade timestamps were `1772323201711256` and `1772409599584254` microseconds (`2026-03-01T00:00:01Z` through `2026-03-01T23:59:59Z`), and the first-row SHA256 was `337ce74e0eba11abdb66d987f7687d117cd1c2ec17cda39e3ee7ba2675c9ca64`. Fresh focused verification `python3 scripts/rust_verification.py cargo --repo crates/backtesting-vertical-slice -- test run_from_run_spec_uses_configured_single_csv_zip_payload -- --nocapture` exited 0; the existing test proves the operator can process a headerless single-member Binance ZIP with microsecond timestamps through NT for the `binance-spot-native-trades` binding. Therefore no venue-specific converter code is needed for this candidate; the remaining blocker is accepted-proof staging/governance.
...
- Binance instrument-universe solution path is now explicit: the preferred proof is a Binance/BD-provided dated `exchangeInfo` or equivalent symbol-metadata snapshot covering BNBUSDC on 2026-03-01, stored and hashed as the proof's `instrument_universe` evidence. If Binance cannot provide dated filter metadata, the current contract can now encode a narrower single-symbol `TRADE_REPLAY` policy by marking `instrument_universe` as `not_applicable` only when a matching blocking claim-limit row uses the same `evidence_ref` and forbids historical venue-rule, fillability, rounding, sizing, or execution-quality claims. That narrower path still requires an explicit accepted proof update; it must not be silently inferred from current `exchangeInfo`.
...
- Fresh S3 listing after those staging proofs returned `4` objects and `1,067,399` total bytes under `s3://bolt-parquet/nt-research-analytics/`: the Binance daily raw ZIP plus `schema-sample.json`, `license-attestation-placeholder.txt`, and `instrument-universe-current-summary.json`. This is staging/provenance evidence only; it is not yet a clean backtest catalog/result publication.
...
| Broader production output coverage under `nt-research-analytics/` | One clean reference backtest catalog/result path is now published and direct-S3-proven, but broad historical backfill output is not | use the same SSM-backed publish path for each newly accepted tranche; verify S3 listing, artifact hashes, and direct catalog readback per tranche before treating outputs as research/analytics inputs |
| Artifact-store SSM parameter durability | The paths now exist and have been proven by real S3 raw/evidence staging writes and one main catalog/result publish proof, but they were created during this investigation and should be treated as operational credentials | keep only the SSM parameter names in TOML (`/bolt/artifact-store/s3/access-key-id` and `/bolt/artifact-store/s3/secret-access-key` in `us-east-1`); never put secret values in TOML or logs; confirm rotation/ownership policy before long-running production backfill |
| Broader source proof coverage | This slice proves one accepted Bybit spot trade-replay object only, as a sample source; registry coverage now has Bybit and Binance native-trades candidates, and the converter can process headerless Binance-style ZIP CSV. A bounded local scratch proof processed the real Binance BNBUSDC daily ZIP through canonical parquet, NT catalog readback, and NT `BacktestNode` with 71,431 iterations, and the exact Binance daily raw ZIP plus schema/license/current-instrument evidence have now been staged to S3 through the generic SSM-backed staging gates. Binance is still not production accepted input because the staged license file is a placeholder attestation and the staged instrument evidence is current `exchangeInfo`, not dated historical universe evidence or an accepted claim-limited `not_applicable` policy record. The existing Binance staging manifest has monthly BNBUSDC trades, not the configured daily BNBUSDC object, and same-host monthly paths are rejected under the daily binding. Generic one-object raw staging and source-proof evidence staging now exist, so the missing piece is no longer converter code, manual broad backfill, or S3/SSM wiring | accept additional source proofs only after raw/schema samples are staged under S3 artifact storage, checksums/hashes are bound, license/retention evidence is recorded, and instrument-universe treatment is explicit. The preferred Binance path is a dated historical metadata snapshot; the narrower fallback is a `not_applicable` `instrument_universe` check with a matching blocking claim limit that forbids historical venue-rule/fillability/rounding/sizing/execution-quality claims. Binance BD approval can satisfy the license check once captured as a durable written approval/attestation pointer; for compatible CSV native-trade sources, add proof/run-spec `[converter.csv]` mapping without changing operator/runner/NT code; if accepting monthly Binance files, add a monthly source binding/proof path rather than reusing the daily binding; for non-CSV or non-trade data, add a new registered adapter and bind its converter config hash |
...
| Direct S3 catalog execution breadth | Direct S3 catalog execution is proven for the accepted BNBUSDC reference tranche only: `published-catalog-proof.json` stamps `direct_s3_catalog_access_proven = true` and `nt_iterations = 937/937` | require the same proof artifact for every accepted tranche before promoting it into research analytics; do not infer direct S3 behavior from local-only runs |
| Artifact Index commit proof | The pure record and committed-discovery contracts are now validated locally, including pointer/snapshot hash checks, stale pointer rejection, staged/orphan rejection, active hot metadata, manifest-lineage parent resolution, immutable event idempotency/overwrite rejection, and first-write/update precondition modeling. The pinned object_store API and AWS backend support the needed create-only/update-if-etag primitives when `conditional_put = "etag"` is configured. Real S3 create-only object writes for raw/evidence staging now work through the SSM path, but actual conditional latest-pointer swaps, persisted snapshot serialization, conditional-failure retry/rebase against storage state, audit epoch object creation, and producer IAM scopes are not proven | keep events/snapshots proof-gated; run a small S3 Artifact Index commit/readback proof using object_store `PutMode::Create` for event/snapshot objects and `PutMode::Update(UpdateVersion)` for latest pointers; if real S3 rejects those semantics, select an approved coordinator/table format before relying on Artifact Index for committed discovery |
...
- `s3://bolt-parquet/nt-research-analytics/` is no longer empty after the
  bounded S3 proofs. The latest recursive listing returned 14 objects totaling
  1,177,847 bytes: one staged Binance daily raw ZIP, three staged source-proof
  evidence objects, and ten clean accepted-reference backtest artifacts under
  `backtests/backtesting-vertical-slice-bnbusdc-2026-03-01/`. This proves the
  SSM-backed staging/publish path for the bounded slices only; it is not broad
