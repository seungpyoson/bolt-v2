...
2. Backfill foundation: not complete. There is substantial staged raw data and
   ...
   writer, plus a batch manifest-summary ingestion boundary, but no generated
   accepted coverage ledger artifact from the S3 manifests/inventory, no
   accepted normalized row tables, no accepted instrument/gap policy ledger,
   and no NT catalog export from that data.
