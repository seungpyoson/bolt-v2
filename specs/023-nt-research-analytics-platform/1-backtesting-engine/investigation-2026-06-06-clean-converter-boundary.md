...
| Direct S3 catalog execution breadth | Direct S3 catalog execution is proven for two accepted tranches: the Bybit sample/reference proof stamps `nt_iterations = 937/937`, and the Binance daily proof stamps `nt_iterations = 71431/71431` | require the same proof artifact for every accepted tranche before promoting it into research analytics; do not infer direct S3 behavior from local-only runs |
