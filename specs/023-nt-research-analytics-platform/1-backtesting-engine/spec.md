...
- Treat strategy escalation decisions as Research Analytics scope: the GO/NO-GO
  verdict and its promotion-config field/URI on the `experiment-results` artifact
  (per `../reference/data-model.md`; no standalone promotion package), not
  Backtesting Engine scope.
...
- Typed promotion-config generated by Research Analytics — the field/URI on the
  `experiment-results` artifact (per `../reference/data-model.md`), not a
  standalone promotion package.
