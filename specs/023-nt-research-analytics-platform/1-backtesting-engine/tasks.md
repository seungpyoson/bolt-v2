...
- [ ] BACKTESTING_ENGINE-004 Define one S3 `artifact_root` config with typed subpaths `raw/`, `nt-catalog/`, `source-proofs/`, `backtests/`, `artifact-index/`, and RA-reserved `research-analytics/`; define short config-selected path keys, `nt-catalog/` projection-root behavior, and reject separate canonical root knobs.
- [ ] BACKTESTING_ENGINE-005 Define the thin per-kind Artifact Index Contract for artifact-local manifests, immutable index events, committed snapshots, generated `artifact-index/v1/pointers/kind=<artifact_kind>/latest.json` pointers, producer-owned write authority, read-only consumers, staged/orphan state, `sha256` content hashes, cross-kind lineage ids, audit epoch records, and no recursive S3 discovery on the normal read path; treat event/snapshot serialization as proof-gated.
- [ ] BACKTESTING_ENGINE-006 Prove the configured artifact store and index writer support create-only writes and conditional latest-pointer updates required by the Artifact Index commit path, or select an approved commit coordinator/table format before relying on the index; prove producer IAM scopes pointer/event/snapshot writes per artifact kind.
...
- [ ] BACKTESTING_ENGINE-027 Select source only after source proof, license, sample, NT mapping, fidelity, and cost proof are recorded.
- [ ] BACKTESTING_ENGINE-028 Present cost-cut levers only after fidelity and source-contract claims are explicit.
