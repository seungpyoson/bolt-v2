...
|---|---|---|
| L0 data | `ParquetDataCatalog` in S3 = single canonical input (`from_uri` S3, Tardis/CSV streamers + `EncodeToRecordBatch`). NT's own `write_to_parquet` is non-atomic `head()`+`PutMode::Overwrite` — `grep PutMode` over NT crates returns zero hits. | Make the proven converters write to S3 DURABLY + IMMUTABLY (today reproduce-on-demand local only) via the conditional create-only writer over `object_store`'s `PutMode::Create` (S3 If-None-Match) per `reference/normalization-catalog-plan.v3.md`, plus config-driven venue dispatch. |
