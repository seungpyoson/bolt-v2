...
- `2-research-analytics`: owns research datasets, notebooks, point-in-time
  feature joins, experiment metadata, leakage checks, and the GO/NO-GO verdict
  with its promotion-config field/URI on the `experiment-results` artifact (no
  standing promotion-packages family; see `reference/data-model.md`).
