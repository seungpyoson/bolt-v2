...
3. Consult `shared/` only for original evidence rows, issue-audit details,
   staged issue payloads, cost/fidelity archives, or review/checklist history.
4. For Issues B-D, open or update only the selected project payload; handle
   shared/process payloads A and E only with explicit user approval.
