...
For historical dataset input, discovery metadata is non-authoritative. Each
run pins an explicit manifest URI and SHA-256, verifies the exact versioned
object set, and consumes those bytes through a sealed local catalog view that
NT reads. The generated latest pointer remains available for general artifact
discovery, but it cannot select, substitute, or advance a backtest's input.
