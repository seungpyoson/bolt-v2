...
- Metadata-derived probe schema is also supported for config-filtered metadata universes: `[clients.<id>.readiness_probe] quote_target_source = "metadata_response"` with TOML-owned `max_metadata_quote_targets`. This mode derives quote subscriptions from the same NT `InstrumentsResponse` observed in the no-submit run; it does not accept copied static instrument IDs.
- Metadata-derived probe target selection now fails closed when the same-run metadata universe exceeds `max_metadata_quote_targets`; it cannot silently truncate a broad metadata universe into a first-N sample, and readiness requires quote evidence for every same-run metadata target. Bounded TOML-owned metadata filters remain usable, while broad metadata responses cannot become arbitrary production-readiness evidence.
...
Current local evidence from the metadata-response probe against ignored operational `config/live.local.toml`, refreshed after metadata-response target truncation was made fail-closed and all same-run metadata targets became required:
...
Additional read-only Binance COIN-M probe evidence: `/private/tmp/bolt-v2-live-binance-coinm-metadata-response.toml` (sha256 `efc8a6d89254988112e91e0aaab5d8a0ec4c7cc280408f12aaa69ffb3dcc35d2`) used `clients.binance_coinm_data.readiness_probe.quote_target_source = "metadata_response"` with no copied instrument IDs and `max_metadata_quote_targets = 20`, matching the source-owned COIN-M metadata universe. `operator-artifacts collect-data-client-behavior-probe-events-source --config /private/tmp/bolt-v2-live-binance-coinm-metadata-response.toml --client-key binance_coinm_data --output /private/tmp/bolt-v2-t043a-binance-coinm-probe/binance-coinm-behavior-probe-events.jsonl` built a scoped no-submit LiveNode, registered only `binance_coinm_data`, loaded `20` COIN-M perpetual instruments, subscribed all `20` same-run metadata targets, stopped cleanly, and failed closed with `NoSubmitDataClientProbeFailed { reason: "reference quote probe did not observe all configured client readiness_probe.quote_targets quotes within [live_canary].reference_quote_wait_timeout_seconds=20" }`. No behavior artifact was written. This narrows the Binance COIN-M blocker to quote-observation behavior under the current configured timeout, not credential resolution, LiveNode registration, metadata reachability, or static target selection.

The same metadata-response approach is not proven for broad exchange metadata universes. A Bybit attempt with the temporary ignored `bybit_data.readiness_probe` metadata-response block reached NT metadata and quote subscription through a scoped no-submit `LiveNode`, but failed closed with `NoSubmitDataClientProbeFailed { reason: "reference quote probe did not observe all configured client readiness_probe.quote_targets quotes within [live_canary].reference_quote_wait_timeout_seconds=20" }`. NT emitted repeated Bybit quote parse errors for missing bid/ask levels before any behavior artifact was written. The runtime and artifact converter now reject metadata-response target truncation, so broad exchange behavior probes must first narrow their TOML-owned metadata universe or use another source-owned target-binding proof before those rows can become production-usable.
