...
- Metadata-derived probe schema is also supported for config-filtered metadata universes: `[clients.<id>.readiness_probe] quote_target_source = "metadata_response"` with TOML-owned `max_metadata_quote_targets` and `min_metadata_quote_targets`. This mode derives quote subscriptions from the same NT `InstrumentsResponse` observed in the no-submit run; it does not accept copied static instrument IDs.
- Metadata-derived probe target selection now fails closed when the same-run metadata universe exceeds `max_metadata_quote_targets` or is below `min_metadata_quote_targets`; it cannot silently truncate a broad metadata universe into a first-N sample. Bounded TOML-owned metadata filters remain usable, while broad metadata responses cannot become arbitrary production-readiness evidence.
...
Current local evidence from the metadata-response probe against ignored operational `config/live.local.toml`, refreshed after metadata-response target truncation was made fail-closed:

- `config/live.local.toml` uses `clients.polymarket_main.readiness_probe.quote_target_source = "metadata_response"` with TOML-owned `max_metadata_quote_targets = 20` and `min_metadata_quote_targets = 4`. No Polymarket instrument IDs are copied into the probe config.
- `operator-artifacts collect-data-client-behavior-probe-events-source --config config/live.local.toml --client-key polymarket_main --output /private/tmp/bolt-v2-t043a-metadata-response-v3/polymarket-behavior-probe-events.jsonl` passed read-only/no-submit and wrote sha256 `65ffd59a23623e6f8d91d232fad4cccf28a30ca313cd54f6de06431fd4da1256`. The JSONL has 9 events: 1 metadata event and 8 quote events.
- Current-config Polymarket policy source: `/private/tmp/bolt-v2-t043a-metadata-response-v3/polymarket-policy-source-current.json`, sha256 `40aa5d78caa5359a74983ae9203f2e984292410bda91526a47b65e455f25bcad`.
- Current-config Polymarket behavior observation source: `/private/tmp/bolt-v2-t043a-metadata-response-v3/polymarket-behavior-observation-source.json`, sha256 `b1a6644d91390af3fdbe07a3efac70cda55dd2e8a552af15f0f5da4327c73dab`.
- Current-config Polymarket behavior observation: `/private/tmp/bolt-v2-t043a-metadata-response-v3/polymarket-behavior-observation.json`, sha256 `3be175810d6f36a42790d76f3d11d5ce5b01cb0730f34633d584d1b24ed9da5a`; `behavior_observation_complete = true`, `missing_behavior_proofs = []`, `metadata_behavior.sample_count = 1`, `quote_behavior.sample_count = 8`, and `freshness.max_observed_age_millis = 9080`.
- Current-config matrix: `/private/tmp/bolt-v2-t043a-metadata-response-v3/data-client-production-readiness-matrix.json`, sha256 `b19b5b461a370568e3f7d6ad1b35c8efc4a0ebea3b507318105cca65fb427a23`. It records 11 clients, 1 production-usable row, and 10 non-production-usable rows still missing `behavior_observation`.
