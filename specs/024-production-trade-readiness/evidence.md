...
- The post-fix output directory remained empty, so no partial source artifact was written. Given the code path above, this is evidence that the configured account still has at least one NT-active position after zero/dust filtering; the command still did not print positions, credentials, keys, account addresses, or SSM values.
