...
- **Evidence:** `external-review/P2-adjudication.md`. Two external review rounds:
  round-1 (6 models) = 4 CLOSE-CONFIRMED + 2 STILL-OPEN (GPT/Grok, on schema-doc drift +
  cosmetic nits); round-2 after the schema-doc fix (`633ea9f0`) = **6/6 CLOSE-CONFIRMED**.
  One real fail-closed-at-load gap (F2 — non-positive / mis-ordered strategy sizing) FIXED
  + tests; F1 already-fixed (`f54181f0`); F3/F5 disproven; F4/F6 nits declined. No
  live-money hazard. (DeepSeek's `edge_threshold_basis_points` flag scoped to P4 as A-EDGE.)
