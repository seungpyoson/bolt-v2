...
7. **Shortest honest path:** fix code-side CI (clippy, 3 tests) → reconcile docs → freeze head → regenerate packet/no-submit at exact head → re-sync allowlist last → run one gated selected-path canary (with explicit live-submit authorization) → T045 → T046 → final CI/review.
...
**R13 — final CI + MANDATORY exact-head external review, once** at the end. The external review (**FR-008 / SC-004**) is a hard gate, not optional: send the exact `FINAL_HEAD` review to **Claude, Gemini, DeepSeek, GLM, and Grok** (Kimi waived per operator). Record verdicts in `external-final-review.md`. Green requires this review to pass at `FINAL_HEAD` (see §5).
