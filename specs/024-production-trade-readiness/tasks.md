...
- [x] T036A [US5] Add a source-input collector that writes replayable `entry-decision-source.json` and `instrument-source.json` from bounded source proofs, requiring source-bound `price_to_beat`, fee-rate proof, and two-sided selected-market books before T036 assembly.
