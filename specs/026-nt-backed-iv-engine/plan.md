...
| Derived IV needs complete inputs | `derive.rs` consumes `IvDerivedInputSet`; `policy.rs` resolves price, underlying, strike, side, time-to-expiry, rate, carry, helper-specific fields such as `initial_vol` for refine helpers, source timestamps, and convention through `IvDerivedInputPolicy`. Missing/invalid inputs reject. |
