...
| Command | Outcome | Notes |
|---|---|---|
| `cargo test --locked --test bolt_v3_iv_query --test bolt_v3_iv_config --test bolt_v3_iv_live_integration --test bolt_v3_iv_source_fence` | RED | Failed with missing `bolt_v3_iv::query`, missing profile-owned strategy authorization, missing `strategy_authorizations()`, and missing live/strategy registration functions. |
...
| Single-strike smile interpolation failures cannot silently downgrade to all-strikes projection. | `cargo test --locked --test bolt_v3_iv_query projected_scalar_query_rejects_when_single_strike_interpolation_has_no_eligible_source` was RED before the fail-closed interpolation state fix and GREEN after. Existing fallback coverage remained GREEN. | Complete |
| Quorum rejection fails closed instead of silently degrading to fallback. | `cargo test --locked --test bolt_v3_iv_query projected_scalar_query_rejects_when_configured_quorum_rejects_even_with_fallback` was RED before the fail-closed quorum fix and GREEN after. | Complete |
| Projected scalar fallback provenance follows the accepted fallback candidate instead of the first matching input. | `cargo test --locked --test bolt_v3_iv_query projected_scalar_fallback_provenance_uses_accepted_candidate_source` was RED before selected-input provenance construction and GREEN after. | Complete |
...
| Selector-scoped reads do not stop at an unauthorized first matching source. | `cargo test --locked --test bolt_v3_iv_query selector_scoped_smile_query_skips_unauthorized_matching_source` and `cargo test --locked --test bolt_v3_iv_query selector_scoped_point_query_skips_unauthorized_matching_source` were RED before authorized-candidate fallback and GREEN after. | Complete |
| Current-generation products are selected over stale historical matches. | `cargo test --locked --test bolt_v3_iv_query strategy_query_skips_non_current_matching_product_for_current_generation` and `cargo test --locked --test bolt_v3_iv_query projected_scalar_point_query_skips_non_current_matching_product_for_current_generation` were RED before current-product filtering and GREEN after. | Complete |
| Helper policy output config fails closed before runtime derivation. | `cargo test --locked --test bolt_v3_iv_config helper_policy_allowed_outputs_must_include_engine_helper_output` was RED before config validation and GREEN after. | Complete |
| Policy and derived-input source references are Rust-validated against the owning profile. | `cargo test --locked --test bolt_v3_iv_config policy_source_references_must_point_to_configured_sources`, `derived_input_profile_source_ref_must_point_to_configured_source_selector_pair`, and `derived_input_policy_field_sources_must_not_duplicate_fields` were RED before validation and GREEN after. | Complete |
| Helper/derived-input policy references and field-source allowlists are internally consistent. | `cargo test --locked --test bolt_v3_iv_config helper_and_derived_input_policy_refs_must_be_reciprocal`, `derived_input_policy_field_sources_must_allow_at_least_one_source_kind`, `derived_input_profile_source_ref_requires_profile_source_ref_kind`, and `derived_input_operator_values_must_be_operator_configured_kind` were RED before validation and GREEN after. | Complete |
| Two registered strategy instances can use different configured selector scopes through the runtime registry. | `cargo test --locked --test bolt_v3_iv_live_integration runtime_registry_supports_two_configured_strategies_with_different_selectors -- --nocapture` was RED before per-strategy TOML authorization and GREEN after. | Complete |
