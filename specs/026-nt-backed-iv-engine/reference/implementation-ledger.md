...
| Quorum rejection uses configured fallback instead of bypassing it. | `cargo test --locked --test bolt_v3_iv_query projected_scalar_query_uses_fallback_when_configured_quorum_rejects` was RED before the fallback routing fix and GREEN after. | Complete |
