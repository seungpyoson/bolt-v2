...
- `<surface_id>` is opaque and globally unique.
...
- Missing optional horizons must not block surface readiness.
...
For shipped production surfaces, configure every available public venue/source that has a valid client and instrument mapping for the canonical asset. If only one source is available, the TOML must make that explicit in a non-runtime documentation field or spec note, not by hardcoding the limitation in Rust.
