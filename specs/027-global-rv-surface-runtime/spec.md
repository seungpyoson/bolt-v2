...
**Created**: 2026-06-08  
**Status**: Draft  
**Input**: User description: "Create a full-scope plan and tasks to make the realized-volatility engine globally usable outside taker, enable multiple venues using available configured data clients, and make realized volatility mathematically more robust. Do not narrow scope."
...
3. **Given** one ready venue diverges beyond configured dispersion/MAD policy, **When** aggregation runs, **Then** the snapshot fails closed or downweights/rejects the bad source according to TOML policy and records the reason.

---

### User Story 3 - Multi-Horizon Robust RV (Priority: P1)
...
**Why this priority**: Multi-horizon RV is the first mathematical upgrade because it is auditable, incremental, and useful before more complex estimators. It avoids both false calm and overreaction while preserving fixed-grid RV as a component.

**Independent Test**: Feed deterministic price paths where short-horizon RV is quiet/noisy and medium/long horizons disagree; verify per-horizon values, final blend, and regime diagnostics in the snapshot/evidence.
...
**Why this priority**: Quote midpoint RV can be inflated by high-frequency microstructure noise. This is the most relevant estimator upgrade after global runtime and multi-horizon support.
...
**Why this priority**: Multi-venue config without robust aggregation still leaves bad-source risk. PR #609 has upper-quantile and dispersion checks; this feature must extend that to median/MAD/trimmed/weighted policies as TOML-owned choices.

**Independent Test**: Configure three or more source RVs with one outlier, one stale source, and two agreeing sources; verify median/MAD or configured robust aggregation produces the expected ready/blocking outcome.
...
2. **Given** sources are too dispersed under configured policy, **When** a snapshot is computed, **Then** the surface fails closed with auditable dispersion/MAD blockers.

---

### User Story 7 - Optional Forecast-Oriented RV Layer (Priority: P3)
...
**Why this priority**: Forecasting is useful but carries higher model risk. It must be optional, auditable, TOML-owned, and simpler than GARCH/ML for the first pass.

**Independent Test**: Enable an EWMA/HAR-style forecast policy and prove evidence distinguishes measured RV from forecast RV and fails closed when forecast inputs are invalid.
...
- Short horizon is ready but long horizon is not; TOML policy determines whether final RV is ready, fallback, or blocked.
...
- **FR-004**: System MUST keep RV policy, source identity, horizons, estimator methods, aggregation, and forecast settings TOML-owned.
...
- **FR-009**: System MUST support multi-horizon per-source RV with TOML-owned horizon definitions and final blend/floor/regime policy.
- **FR-010**: System MUST record per-horizon source RV values, final source RV, final surface RV, and regime diagnostics in snapshots/evidence.
...
- **FR-013**: System MUST support robust cross-source aggregation beyond current upper-quantile, including median/MAD or equivalent robust dispersion policy.
- **FR-014**: System MUST keep zero RV valid when all configured readiness requirements are satisfied.
- **FR-015**: System MUST fail closed when required horizons, source quorum, aggregation, estimator, or forecast inputs are unavailable or invalid.
...
- **HorizonRvEstimate**: Per-source per-horizon RV output with readiness, coverage, gaps, and estimator diagnostics.
- **RobustRvEstimate**: Final per-source and per-surface RV with measured, continuous, jump, blended, and optional forecast components.
...
- **SC-004**: Multi-horizon tests prove short/medium/long estimates and final blended RV are recorded and used according to TOML policy.
