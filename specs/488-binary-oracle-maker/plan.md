...
**W1B — Market-family seam hardening and proof families.** Replace the former WG product-shaped "lean perp" wording with architecture checks. The seam must expose family-neutral quote targets; binary/updown naming must be inventoried, quarantined, or mechanically renamed out of new shared surfaces; bounded-scalar, continuous-mark, and finite multi-outcome proof families must compile through the same seam without changing `quote_lifecycle`, `RequoteBudget`, admission, execution, or strategy core. W1B must land the enforcement gates: compile-proof tests, quote-lifecycle proof-family smoke tests, an N>=3 settlement proof, and static checks for forbidden venue/family names in shared modules. These proof families are not live-tradeable products and carry no venue adapter, funding, margin, liquidation, or backtest gate.

**W2 — Quote Lifecycle / Execution Control (the missing core).** Multi-resting-quote state; requote loop using `supports_modify` (cancel+resubmit) and the rate-budget variable; order-accepted reconciliation + deferred-cancel handling; post-only-reject requote; family-neutral cancel scope; reconnect resync + cancel-all-on-kill. Carries its own no-submit/canary proof against a simulated venue at the real budget. *(Built as pure NT-free modules `src/strategies/quote_lifecycle.rs` + `requote_budget.rs`, slices 1–4 landed before W0; each open PR must now be audited against W0 before merge.)*

**W3 — Crypto binary maker model.** GM/CG half-spread with a defined parameter source + calibration (from the historical full-depth corpus); two-sided YES/NO joint quote + inventory; book-imbalance term; inventory→skew functional form; new kill predicates (σ-floor/basis-cap/τ-floor) as TOML thresholds, fail-closed; graduated governor states (cancel-only/reduce-only/hard-flat/reward-preserving soft-hold); offset-composition precedence + (ε,1−ε) clamp/prune. This is crypto-binary-specific logic and must stay inside the crypto binary family adapter, binary maker model, or strategy-local evidence/display fields; it must not leak into `quote_lifecycle`, `RequoteBudget`, admission, execution, or shared settlement orchestration.

**W4 — Settlement & resolution lifecycle.** The shared N-outcome settlement primitive (built here, also consumed by the backtest — one module, no fork), wired live: handle `InstrumentStatus::Close`, book the payout vector, close positions, redemption path, settlement P&L. Crypto binary 0/1 payout is the first N=2 case, not a separate settlement engine.
...
| Shared settlement primitive (W4, also used by the backtest) | The backtest's net edge depends on terminal payout accounting; crypto binary is the first N=2 0/1 case and must match live accounting | A throwaway backtest settlement path is a dual-path violation and a false pass |
...
| Shared N-outcome settlement/accounting module | **coordinate** | one module, consumed by both; crypto binary uses the N=2 0/1 case — no fork (Codex #1) |
