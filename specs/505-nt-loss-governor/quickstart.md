...
Passing tests in PR #507 prove pure policy behavior, config binding, NT-derived sizing-state validation, worst-case binary liability sizing, capital-reservation reserve/release/rebuild/revalue behavior, pure already-attributed lifecycle update dispatch, configured submit-admission loss rejection before NT submit, and configured NT event-feed snapshot derivation. They do not prove positional-sizer live-path enforcement, NT order-event parsing, cancel/flatten behavior, or explicit NT RiskEngine trading-state transitions.
