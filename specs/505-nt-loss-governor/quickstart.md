...
Passing tests in PR #507 prove pure policy behavior, config binding, NT-derived sizing-state validation, worst-case binary liability sizing, capital-reservation reserve/release/rebuild/revalue behavior, pure already-attributed lifecycle update dispatch, configured submit-admission loss rejection before NT submit, configured NT event-feed snapshot derivation, and configured NT `RiskEngine::set_trading_state` transitions for loss halts. They do not prove positional-sizer live-path enforcement, NT order-event parsing, active cancel/flatten behavior, flat-position behavior, or operator clear-to-Active recovery.
