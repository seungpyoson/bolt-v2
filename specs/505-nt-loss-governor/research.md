...
- `src/bolt_v3_loss_halt_actions.rs`: adds explicit loss-halt trading-state actions, market-exit action fields, manual recovery evidence structure, and next-action decision logic. Active market-exit dispatch remains disabled by validation.
...
- TDD red/green evidence:
  ...
  - `loss_breach_maps_to_market_exit_action`: proves threshold breaches map to configured market-exit action fields in pure policy.
  - `market_exit_latch_marks_success_once_and_clears_on_recovery`: covers reserved latch semantics for the future Bolt-owned exit path.
  - `rejects_loss_governor_market_exit_all_registered_strategies`: proves startup validation rejects direct active market exit because NT market-exit commands bypass Bolt submit/cancel chokepoints.
