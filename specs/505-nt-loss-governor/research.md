...
- `src/bolt_v3_config.rs`, `src/bolt_v3_validate.rs`, `tests/config_parsing.rs`: add and validate `[risk.loss_governor]` fields, including configured account id, freshness, rolling window, thresholds, explicit trading-state action, market-exit action, recovery mode, and loaded strategy execution-account compatibility.
...
- `src/bolt_v3_loss_runtime_feed.rs`: subscribes to NT portfolio and position event topics, filters to the configured account, derives daily/session PnL, rolling PnL from portfolio PnL deltas, per-trade PnL from position changed/closed events, current equity, and peak equity, publishes a conservative `LossSnapshot` to submit admission, and invokes the configured halt-action handler after successful snapshot publication.
- `src/bolt_v3_loss_halt_actions.rs`: adds explicit loss-halt trading-state actions, market-exit actions, manual recovery evidence structure, next-action decision logic, and successful-dispatch latch semantics.
- `src/bolt_v3_live_node.rs`: maps enabled `[risk.loss_governor]` TOML into the shared submit-admission state and configured NT runtime feed during live-node build, applies configured NT `RiskEngine::set_trading_state` actions, and dispatches configured NT `Trader::market_exit_strategy` actions for registered strategies.
...
- TDD red/green evidence:
  ...
  - `stale_peak_timestamp_does_not_make_fresh_portfolio_snapshot_stale`: failed while historical peak timestamps could stale a fresh account heartbeat, then passed after snapshot freshness moved to conservative NT-derived observation timestamps for the published facts.
  ...
  - `loss_breach_maps_to_market_exit_action`: proves threshold breaches map to configured NT market-exit action.
  - `market_exit_latch_marks_success_once_and_clears_on_recovery`: proves repeated breached snapshots do not relatch already-successful strategy dispatches and recovery clears the latch.
  - `rejects_loss_governor_market_exit_without_reducing_state`: proves startup validation rejects `all_registered_strategies` market exit unless the matching NT trading-state action is `Reducing`.
  - `rejects_loss_governor_market_exit_strategy_account_mismatch`: proves configured active market exit cannot target strategies on an execution account that differs from `[risk.loss_governor].account_id`.
  - `disabled_loss_governor_market_exit_does_not_require_loaded_strategies`: proves disabled loss-governor configs do not accidentally trigger active market-exit validation.
  - `nt_market_exit_strategy_reaches_running_stub_strategy_hook`: proves NT `Trader::market_exit_strategy` reaches a running registered strategy hook.

**Live protection boundary**: This PR proves submit-admission loss protection for configured policy plus configured NT-derived runtime snapshots. Enabled live builds now carry the policy and subscribed runtime feed, fail closed until enough configured-account NT facts are observed, apply configured NT trading-state actions, and can dispatch configured NT market exits for registered strategies. NT market-exit dispatch does not prove the account is flat, and the operator clear-to-Active live command surface remains later work.
