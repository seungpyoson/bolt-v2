...
- `src/bolt_v3_config.rs`, `src/bolt_v3_validate.rs`, `tests/config_parsing.rs`: add and validate `[risk.loss_governor]` fields, including configured account id, freshness, rolling window, thresholds, explicit trading-state action, recovery mode, and manual-recovery evidence path bound. Active market-exit policy/config/latch scaffolding is intentionally absent; any later active market exit must call NautilusTrader's owned `Trader::market_exit_strategy` primitive directly from a real live boundary.
...
- `src/bolt_v3_loss_runtime_feed.rs`: subscribes to NT portfolio, account-state, and position event topics, filters to the configured account, derives daily/session PnL, rolling PnL from portfolio PnL or AccountState equity deltas, per-trade PnL from position changed/closed events, current equity, and peak equity, publishes a conservative `LossSnapshot` to submit admission, and invokes the configured halt-action handler after successful snapshot publication.
- `src/bolt_v3_loss_halt_actions.rs`: adds explicit loss-halt trading-state actions, manual recovery evidence structure, and next-action decision logic. Active market exit is not part of this slice; any later active market-exit path must call NautilusTrader's owned `Trader::market_exit_strategy` primitive directly from a real live boundary.
- `src/bolt_v3_live_node.rs`: maps enabled `[risk.loss_governor]` TOML into the shared submit-admission state and configured NT runtime feed during live-node build, applies configured NT `RiskEngine::set_trading_state` actions after trusted loss snapshots, and exposes live manual recovery that can set NT risk state back to `Active` only after fresh accepted loss evidence plus bounded operator evidence.
- `src/bolt_v3_decision_evidence.rs`: records `rejected_loss_governor_halted` plus deterministic `loss_halt_reasons` in the current decision-evidence schema.
...
**Live protection boundary**: This PR proves submit-admission loss protection for configured policy plus configured NT-derived runtime snapshots. Enabled live builds now carry the policy and subscribed runtime feed, fail closed until enough configured-account NT facts are observed, apply configured NT trading-state actions after trusted snapshots, and expose live manual recovery for fresh accepted loss snapshots plus bounded operator evidence. Active market-exit execution, flat-position proof, and the external operator command surface remain later work. Any future active market-exit path must call NautilusTrader's owned `Trader::market_exit_strategy` primitive directly from a real live boundary rather than adding Bolt-owned market-exit policy or latch scaffolding.
