...
**Independent Test**: Submit-admission tests verify missing snapshots fail closed before NT submit, breached snapshots halt entry risk, risk-reducing exits remain possible within the existing operator count cap, and live-node construction passes the configured policy into submit admission.
...
- **SC-005**: `cargo test --locked --test bolt_v3_submit_admission` passes.
- **SC-006**: `cargo test --locked --test config_parsing` and `cargo test --locked --test bolt_v3_decision_evidence` pass.
- **SC-007**: Final report separates submit-admission protection from unimplemented cancel/flatten or NT trading-state side effects.
...
- This slice wires configured submit-admission protection. Cancel, flatten, and explicit NT `RiskEngine::set_trading_state` side effects remain later work.
