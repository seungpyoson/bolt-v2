...
Base refreshed for A10 local slice: `origin/main` `31a4342a`; A9 is present at
...
| `src/strategies/binary_oracle_edge_taker/tests/*.rs` | 11,100 after A10 local split | 236 |
...
| A10 | split 236 tests; mod.rs = struct + DataActor + glue | `…/tests/` | ✓ RED structure test failed before split; RED canonicalizer test failed before inner-cfg exclusion; restored GREEN structure test (`1 passed`), canonicalizer regression (`1 passed`), and strategy subset (`238 passed; 364 filtered out`) | ✓ pure test move: `mod.rs` keeps only `#[cfg(test)] mod tests;`; split files mirror ownership buckets and start with `#![cfg(test)]`; `shared_fixture.rs` holds test-only helpers; source canonicalizer excludes inner-cfg test-only split files from production text | local gates green: `cargo fmt --check`; `cargo test bolt_v3_source_integrity --lib` (`11 passed; 591 filtered out`); runtime-literal self-test and verifier passed | PR CI/review not requested from this dirty local branch | **LOCAL GREEN — PR/CI/review pending** |
