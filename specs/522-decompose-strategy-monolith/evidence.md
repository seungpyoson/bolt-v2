...
| A6 | exposure/recovery state machine | `…/exposure.rs` | ✓ wrong-control RED failed as expected; restored GREEN targeted test passed (`1 passed; 553 filtered out`) with operator-approved alternate root `<ALTERNATE_VERIFICATION_ROOT>` | local WIP pure move: `exposure.rs` added; `mod.rs` definitions removed; no `pub use`; source-integrity golden re-derived with dynamic file walk; `git diff --check`, `cargo fmt --check`, lib/bin clippy, and `just source-fence` green locally | PR #583 CI gate tracked by exact-head GitHub checks; re-run required after every push before review or merge | internal adversarial review complete locally; Gemini docs findings fixed/resolved; external review packet pending user relay | **IN PROGRESS — not resolved** (`mod.rs` 17,480→17,127 locally; `exposure.rs` 416 lines; external review/operator merge still pending) |
