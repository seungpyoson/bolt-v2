...
| A6 | exposure/recovery state machine | `…/exposure.rs` | ✓ wrong-control RED failed as expected; restored GREEN targeted test passed (`1 passed; 553 filtered out`) with operator-approved alternate root `/Volumes/SP (2TB)/bolt-v2` | local WIP pure move: `exposure.rs` added; `mod.rs` definitions removed; no `pub use`; source-integrity golden re-derived with dynamic file walk; `git diff --check`, `cargo fmt --check`, lib/bin clippy, and `just source-fence` green locally | local cargo/source gates green; CI not opened | internal adversarial review complete locally; external review pending until PR CI is green | **IN PROGRESS — not resolved** (`mod.rs` 17,480→17,127 locally; `exposure.rs` 416 lines; PR/CI/review/merge still pending) |
