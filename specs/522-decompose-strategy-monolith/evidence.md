...
| A9 | admission-request construction (kill dup :7546) — owns base | `bolt_v3_submit_admission.rs` | ✓ RED unresolved shared-builder imports; restored GREEN targeted integration test passed; review follow-up added a market-style fee-before-ceiling error-order regression (`2 passed; 56 filtered out`) | local WIP move: shared `BoltV3SubmitAdmissionRequestInput` + `build_submit_admission_request_from_order`; strategy wrapper delegates valuation/request construction; test helper delegates to shared builder; source-integrity goldens re-derived | local targeted gates green: `cargo test submit_admission --lib` (`19 passed; 582 filtered out`) and `cargo test bolt_v3_source_integrity --lib` (`11 passed; 590 filtered out`); full local/CI gates pending | external review pending at final PR head | **IN PROGRESS — not resolved** (#507/#510 rebase onto it after merge) |
