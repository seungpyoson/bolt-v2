...
- **FR-002 — Numeric prerequisite (PR-0).** Extend `src/bolt_v3_numeric.rs` with `TWO_F64`, `HALF_F64`, `sanitize_open_probability` (verified missing on `main`; the maker imports all three), `pub(crate)`, each with a unit test, before any module that uses them is ported.
...
- **FR-006 — Admission dual-path never lands.** The notional/fee math has exactly one home: `bolt_v3_submit_admission` already exposes it as the `Decimal`-authoritative pub fns `base_quantity_admission_notional` (`:627`) and `fee_inclusive_admission_notional` (`:747`). The ported reservation gate **calls** that single primitive (or, only if a hot-loop `f64` face is profiled as necessary, an `f64` adapter co-located with and explicitly derived from it — never an independent re-implementation). The slice that establishes this reuse is sequenced **before** the reservation gate is ported, so the existing `f64` re-implementation (`maker_reservation.rs` `BuyCommitment::notional` + `gross_up`) never lands on `main`. The BPS-vs-multiplier fee representation is reconciled at the boundary (BPS → multiplier is `1 + bps/10000`).
- **FR-007 — Archetype last.** `binary_oracle_maker.rs` is written only after the shared helpers exist, as a peer to `binary_oracle_edge_taker` under `bolt_v3_archetypes/`. It imports only `bolt_v3_*` (never `crate::strategies::maker_*`) and binds the family write-side via the canonical dispatchers.
...
- **FR-009 — Fences green every slice.** `verify_bolt_v3_{naming,dependency_direction,core_boundary}.py` and the strategy-policy fence pass on every slice, plus `cargo fmt --check`, `clippy -D warnings`, and the slice's targeted tests.
...
PR     — one slice = one PR = one declared scope (CLAUDE.md #9). No slice may land a
         known dual-path, even temporarily (governs the admission-primitive ordering).
```
