...
- **FR-010**: The RCK model MUST solve an explicit optimization, not merely satisfy a constraint. Over the registered modeled states S_model and the permitted target domain X, the target MUST be x* ∈ argmax_{x∈X} Σ_{s∈S_model} πₛ · ln(Rₛ(x)) subject to Σ_{s∈S_model} πₛ · Rₛ(x)^(−κ) ≤ 1, with κ = ln β / ln α and explicit 0 < α < 1, 0 < β < 1, where Rₛ(x) = W_s_post_target(x) / reference_growth_wealth and W_s_post_target(x) includes current marked wealth, the terminal value change of existing filled exposure, the terminal value and execution cost of the target adjustment, and any same-instrument pending exposure treated as still reachable. The zero-risk target (no risk increase) MUST be feasible; ties MUST resolve toward lower absolute stress loss. The constraint is evaluated on post-trade target wealth, never the order delta. The flat target x = 0 and the no-trade action are DISTINCT concepts: emitting no order MUST always be a valid engine result, and the specification MUST NOT assume that retaining a pre-existing position automatically satisfies the RCK constraint or the FR-014 cap. When current exposure already violates a binding constraint, the engine yields no risk-increasing target — only FR-014 reductions — never an assumption of feasibility.
...
- **FR-012**: S_model (the model's sizing states) MUST be DISTINCT from S_stress (the full terminal set — void/dispute/oracle-failure/haircut/default — used by the substrate for hard feasibility). The model MUST NOT substitute S_model for the substrate's feasibility check. A decision's ADVERSE band end MUST be side-aware: the lower band for a long, the upper band for a short. The RCK growth model MUST arm ONLY for decisions whose adverse end is the CERTIFIED band end of the loaded `BandCoverageAttestation` (#724 FR-011). Phase 1 certifies only the lower band (`p_lower`, #724 FR-010A), so Phase-1 RCK is LONG-ONLY: a genuine short-side RCK action (adverse end = upper band) has no certified band-validity and MUST be rejected to the model-selection rule FR-015 (default no-trade; the calibration-free fixed-fraction model stays side-agnostic) until the symmetric upper-band attestation (#724 FR-010B, deferred) exists.
- **FR-013**: Sizing at the worst-case band end MUST be treated as the conservatism budget (box-DRO at fraction 1); the engine MUST NOT additionally apply a fractional-Kelly multiplier for the same risk. Band MISSPECIFICATION MUST be guarded separately: the Kelly model MUST arm only behind a valid `BandCoverageAttestation` produced offline by the calibration scoreboard (#724 FR-011 — config bound into the substrate's prepared epoch, not a live dependency) AND the model-risk cap of FR-014. #712 MUST consume the CANONICAL `BandCoverageAttestation` type defined by #724 without redefining or truncating its fields. DECISION-scoped fields MUST exactly match the sizing decision: model + band-method versions; instrument family/segment; side + horizon; the attestation's `certified_bound_end` MUST equal the decision's adverse band end (FR-012), so an attestation certifying only the lower band cannot arm a short-side decision; outcome-space + outcome-definition versions. EVIDENCE-scoped fields MUST be authenticated and accepted by the active policy epoch (they are not matched to the decision): evaluation cutoff + dataset hash; the certified property; the per-cell observed statistic + simultaneous one-sided lower confidence bound; effective event count; eligibility-policy version; producer/approver + issuance/expiry/revocation; and all schema, evaluation, and inference-method versions. The entire canonical artifact and its authentication MUST verify, and the prepared epoch MUST bind its exact content digest (#973 FR-050). A stale, mismatched, ineligible, expired, revoked, or tampered attestation MUST be treated as absent; on absent/invalid attestation the result MUST follow the model-selection rule FR-015 (default no-trade).
- **FR-014** (model-risk cap): `ModelRiskCap` is a separately named, ABSOLUTE target-scope cap guarding band misspecification, distinct from and enforced independently of the band-coverage attestation. Let `cap_amount(W) = min(model_risk_cap_absolute, model_risk_cap_fraction · reference_growth_wealth)`, where `model_risk_cap_absolute` is denominated in the account settlement unit and `model_risk_cap_fraction` is dimensionless. The specification MUST declare the exact model-risk SCOPE whose stress is capped (instrument/outcome exposure, event/risk bucket, strategy allocation, or portfolio); launch caps the instrument/outcome exposure being sized, config-declarable to a wider scope. Let `current_scope_stress = equity_stress_loss(current portfolio restricted to that scope)` and `target_scope_stress(x) = equity_stress_loss(post-target portfolio restricted to that scope)`, both the scope-restricted equity-floor stress loss computed by the substrate's shared evaluator (#973 FR-003) and returned on the scoped `RiskAssessment` (#973 FR-025) for one `risk_state_version`, never a model-local figure. The declared scope MUST be one the substrate recognizes — the instrument/outcome being sized, a substrate-derived concentration bucket, or the whole portfolio — and an unrecognized scope MUST fail closed. If `current_scope_stress ≤ cap_amount(W)`, a target x is cap-eligible only if `target_scope_stress(x) ≤ cap_amount(W)`. If `current_scope_stress > cap_amount(W)`, NO risk-increasing target is eligible; only targets with `target_scope_stress(x) < current_scope_stress` are eligible, through the reduction path (FR-022 / #973 SafetyAction). The cap is an INDEPENDENT constraint on the RCK feasible target set, applied BEFORE the expected-log argmax of FR-010 — never a post-hoc multiplier on the chosen size. `cap_amount` MUST be recomputed from current reference growth wealth on every decision, so it tightens after a drawdown. Both cap parameters, the declared scope, their units, and their policy-envelope ranges MUST be runtime configuration (TOML, fail-closed when missing or out of envelope). The cap MUST be tested as binding (a within-cap case where it, not the RCK constraint, sets the size; and an over-cap case where only strict reductions remain).
...
- **ModelRiskCap**: the separately named ABSOLUTE target-scope cap of FR-014 — `cap_amount(W) = min(absolute settlement-unit limit, dimensionless fraction · reference_growth_wealth)` bounding `target_scope_stress(x)` (current and post-target scoped stress substrate-computed via the scoped preview, #973 FR-003/FR-025) over a substrate-recognized declared scope (over cap ⇒ strict reductions only), applied before the RCK argmax and recomputed each decision — guarding band misspecification, enforced independently of the attestation.
...
- **SC-010** (RCK optimization): The RCK target maximizes expected log growth subject to the drawdown constraint, the zero-risk target is feasible, and `ModelRiskCap` (FR-014) binds where its `target_scope_stress(x) ≤ cap_amount` limit is tighter than the RCK constraint — proven by tests (a within-cap case where the cap, not the constraint, sets the size; and an over-cap case where `current_scope_stress > cap_amount` so only strict reductions remain, with no-trade distinct from a flat target).
