...
- **FR-013**: Sizing at the worst-case band end MUST be treated as the conservatism budget (box-DRO at fraction 1); the engine MUST NOT additionally apply a fractional-Kelly multiplier for the same risk. Band MISSPECIFICATION MUST be guarded separately: the Kelly model MUST arm only behind a valid, config-loaded `BandCoverageAttestation` produced offline by the calibration scoreboard (#724 FR-011 — config bound into the substrate's prepared epoch, not a live dependency) AND the model-risk cap of FR-014. The attestation's bound fields — model + band-method versions, instrument family/segment, side, horizon, outcome-space id/version, outcome-definition id/version, `ForecastRecord` schema version, evaluation-implementation version, dependence/inference-method version, attestation schema version, eligibility pass, and validity interval (the full FR-011 binding set) — MUST match the sizing decision under the attestation's canonical authenticated encoding; a stale, mismatched, ineligible, expired, revoked, or tampered attestation MUST be treated as absent. On absent/invalid attestation the result MUST follow the model-selection rule FR-015 (default no-trade).
- **FR-014** (model-risk cap): `ModelRiskCap` is a separately named cap guarding band misspecification, distinct from and enforced independently of the band-coverage attestation, defined concretely as follows. Let `cap_amount(W) = min(model_risk_cap_absolute, model_risk_cap_fraction · reference_growth_wealth)`, both parameters expressed in the account settlement unit. For a candidate target x define `incremental_stress(x) = max(0, equity_stress_loss(x) − equity_stress_loss(current_position))`, computed by the substrate's shared evaluator (#973 FR-003), not a model-local figure. A target x is cap-eligible only if `incremental_stress(x) ≤ cap_amount(W)`. The cap is an INDEPENDENT constraint on the RCK feasible target set, applied BEFORE the expected-log argmax of FR-010 — never a post-hoc multiplier on the chosen size. `cap_amount` MUST be recomputed from current reference growth wealth on every decision, so its fractional component falls after a drawdown. If existing exposure already makes `incremental_stress` exceed `cap_amount`, no risk-increasing target is permitted and only strict reductions remain eligible. Both cap parameters, their unit, and their policy-envelope ranges MUST be runtime configuration (TOML, fail-closed when missing or out of envelope). The cap MUST be tested as a binding cap (a case where it, not the RCK constraint, sets the size).
...
- **ModelRiskCap**: the separately named cap of FR-014 — `cap_amount(W) = min(absolute, fraction · reference_growth_wealth)` bounding `incremental_stress(x)`, applied before the RCK argmax and recomputed each decision — guarding band misspecification, enforced independently of the attestation.
...
- **SC-010** (RCK optimization): The RCK target maximizes expected log growth subject to the drawdown constraint, the zero-risk target is feasible, and `ModelRiskCap` (FR-014) binds where its `incremental_stress ≤ cap_amount` limit is tighter than the RCK constraint — proven by tests (a case where the cap, not the constraint, sets the size; and a case where current exposure exceeds the cap so only reductions remain, with no-trade distinct from a flat target).
