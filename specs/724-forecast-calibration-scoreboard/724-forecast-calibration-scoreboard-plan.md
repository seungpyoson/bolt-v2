...
- **C5 — Band-validity estimand + attestation + config bridge.** Implement the predeclared `BandValidityPolicy` (FR-010A): per-cell residual `d_i = y_i − p_lower_i`, certified property `E[d_i | cell] ≥ 0`, a simultaneous one-sided lower confidence bound `LCB_c ≥ 0` plus `n_effective ≥ n_min` per cell, emit only if every cell passes; the prohibited 0/1-outcome-in-interval definition must be unable to gate emission. **Launch inference method — Launch Policy v1 (one exact procedure, not deferred to the implementer):** (1) Event selection — for each terminal event, select the single LAST valid forecast observed at least `T_select` before resolution, ties broken deterministically by `decision_id`, yielding one residual `d_i = y_i − p_lower_i ∈ [−1, 1]` per independent terminal event. (2) Per-cell bound — for each predeclared cell `c` with `n_c` selected events, compute the one-sided lower confidence bound on `E[d_i | c]` by the empirical-Bernstein bound for bounded variables (Maurer–Pontil 2009): `LCB_c = mean(d) − sqrt(2·V̂·ln(1/δ_c)/n_c) − 7·R·ln(1/δ_c)/(3·(n_c−1))`, with `V̂` the sample variance of `d_i` in `c`, residual range `R = 2`, and per-cell tail level `δ_c`. (3) Multiplicity — allocate the family-wise level `α` across the predeclared cells by Holm step-down, yielding each cell's `δ_c`. (4) Pass/emit — a cell passes iff `LCB_c ≥ 0` AND `n_effective,c ≥ n_min,c`; emit iff every predeclared cell passes. `T_select`, `α`, the cell map, and each `n_min,c` are policy config. Hoeffding, self-normalized, or betting-martingale bounds, any alternative event-selection or cluster estimator, and the symmetric upper-band test for short-side RCK (#724 FR-010B) enter ONLY through separately versioned policies. Emit the versioned, machine-verifiable `BandCoverageAttestation` (FR-011) ONLY when that policy passes (producer/approver separation, validity, revocation), binding the full field set including outcome-space and outcome-definition versions under a canonical authenticated encoding; prove it loads unchanged into the #712/#973 prepared epoch with its exact digest bound and that stale/mismatched/ineligible/expired/revoked/tampered attestations are rejected. *Evidence: SC-008 band-validity estimand test (`LCB_c ≥ 0` gate passes on known-conservative cells, fails on an anti-conservative cell; interval-membership definition rejected) + SC-006 cross-module load/validate + tamper/expiry test; eligibility-gate test (no attestation emitted on fail).*
