...
- **BandValidityPolicy (offline)**: the predeclared estimand + inference (FR-010A) — decision cells, a one-sided lower-bound test that `p_lower` is conservative per cell, clustering key, repeated-forecast/cluster estimator, confidence method/level, multiplicity correction, minimum effective-event count, failure behavior. It is the gate that decides whether an attestation may be emitted; it explicitly forbids a 0/1-outcome-in-interval definition.
...
- **C5 — Band-validity estimand + attestation + config bridge.** Implement the predeclared `BandValidityPolicy` (FR-010A): decision cells + a one-sided lower-bound test that `p_lower` is conservative per cell (clustering key, repeated-forecast/cluster estimator, confidence method/level, multiplicity correction, minimum effective-event count, failure behavior); the prohibited 0/1-outcome-in-interval definition must be unable to gate emission. Emit the versioned, machine-verifiable `BandCoverageAttestation` (FR-011) ONLY when that policy passes (producer/approver separation, validity, revocation), binding the full field set including outcome-space and outcome-definition versions under a canonical authenticated encoding; prove it loads unchanged into the #712/#973 prepared epoch and that stale/mismatched/ineligible/expired/revoked/tampered attestations are rejected. *Evidence: SC-008 band-validity estimand test (one-sided lower-bound gate passes on known cells; interval-membership definition rejected) + SC-006 cross-module load/validate + tamper/expiry test; eligibility-gate test (no attestation emitted on fail).*
...
| Predeclared one-sided lower-bound estimand (FR-010A), not interval-membership | A single 0/1 outcome cannot reveal whether the latent probability lay inside a band; the only validatable claim for lower-bound Kelly is that `p_lower` is conservative per predeclared cell | Defining "band coverage" as a realized outcome falling in `[p_lower,p_upper]` is statistically meaningless and would arm Kelly on a bound never actually validated — the single biggest residual production risk |
