...
This module is a **shared, generic helper**: its core grades forecasts against realized outcomes over a **declared outcome space** using **proper scoring rules**, and emits standard calibration metrics (reliability curve, Brier score, log loss, per-bin counts). It hard-codes **nothing** about any venue, asset, oracle, or market family — the binary up/down outcome space (a forecast probability + a realized binary outcome) is simply the **first** registered instance, and the only one launch implements; any forecasting family plugs in by declaring its outcome space and supplying a thin outcome-derivation adapter. The scoring math is the same for all. ("Agnostic" here means agnostic across families that declare an outcome space + proper scoring rule; a multi-outcome space is a future registered instance, not a core change.)
...
- **FR-001**: The scorer core MUST grade forecasts against realized outcomes over a **declared outcome space** using **proper scoring rules**, computing a reliability curve (binned predicted probability vs realized frequency), Brier score, log loss, and per-bin sample counts. The binary outcome space (a forecast probability + a realized binary outcome) is the first registered instance and the only one launch implements; the metrics MUST be proper-scoring-rule instances that generalize to other registered outcome spaces without a core change. These MUST be shared, generic helpers with unit tests pinned to known analytic values.
- **FR-002**: The scorer core MUST contain no venue, asset, oracle, cadence, or market-family literal, and MUST be parameterized by the declared outcome space + scoring rule rather than branching on a family (agnostic, per the shared-infra mandate). Verified by review/grep.
...
- **FR-011** (BandCoverageAttestation — the artifact #712 consumes): When, and only when, configured eligibility criteria pass, the module MUST emit a machine-verifiable, versioned `BandCoverageAttestation` binding: model + band-method versions; instrument family/segment, side, and horizon; evaluation cutoff + dataset hash; target one-sided coverage; observed coverage and its one-sided confidence lower bound; effective sample size; eligibility-policy version + pass/fail result; and producer, approver, issuance time, expiry, and revocation identity. Producer and approver MUST be distinct, independently authenticated roles. No attestation may be emitted when eligibility fails. This attestation is the SOLE artifact the sizing engine (#712 FR-013) loads to arm its growth model; it is offline-produced config, never a live call. It MUST load unchanged into the substrate's prepared policy epoch (#973 FR-050); a stale, mismatched, ineligible, expired, revoked, or tampered attestation MUST be rejected — failing closed to no growth-model arming.
...
- **OutcomeSpace / ScoringRule**: the seam that parameterizes the scorer core (FR-001); the binary outcome space + its proper scoring rules is the first registered instance.
