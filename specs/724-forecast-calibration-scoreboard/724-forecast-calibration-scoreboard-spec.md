...
- **FR-010A** (band-validity estimand and inference): The eligibility policy MUST define the EXACT statistical property certified for the forecast band ends `p_lower` / `p_upper`. It MUST NOT define "coverage" by comparing a single Bernoulli realization directly with a probability interval — a 0/1 outcome cannot reveal whether the latent true probability lay inside a band, so a "realized outcome ∈ [p_lower, p_upper]" rule is meaningless and is prohibited. For the Phase-1 lower-bound Kelly use case, the policy MUST define a predeclared segmentation/calibration map (decision cells) and a one-sided validation test for the claim that `p_lower` is conservative within every attested cell (realized frequency ≥ `p_lower` per cell). The policy MUST bind: the terminal-event clustering key; a deterministic repeated-forecast selection rule or a named cluster estimator; overlapping-horizon treatment; the confidence-bound method and confidence level; the multiplicity correction across cells; the minimum effective independent-event count; and the failure behavior. Point-only records (no band) MAY enter point-calibration reports but MUST NOT support a `BandCoverageAttestation`. A launch-safe default policy is: one deterministically selected forecast per unique terminal event, predeclared cells, a named one-sided confidence-bound method, and a predeclared multiplicity correction; a cluster method is acceptable only if fully specified, never left to the implementer.
- **FR-011** (BandCoverageAttestation — the artifact #712 consumes): When, and only when, configured eligibility criteria (FR-010A) pass, the module MUST emit a machine-verifiable, versioned `BandCoverageAttestation` binding: model + band-method versions; instrument family/segment, side, and horizon; evaluation cutoff + dataset hash; the certified one-sided band-validity property and its target coverage; observed coverage and its one-sided confidence lower bound; effective sample size; eligibility-policy version + pass/fail result; and producer, approver, issuance time, expiry, and revocation identity. It MUST ADDITIONALLY bind, under a canonical authenticated encoding: outcome-space id/version; outcome-definition id/version (the same identity `ForecastRecord` records, so an attestation cannot be reused after the economic outcome definition changes); `ForecastRecord` schema version; evaluation-implementation version; dependence/inference-method version; and attestation schema version. Producer and approver MUST be distinct, independently authenticated roles. No attestation may be emitted when eligibility fails. This attestation is the SOLE artifact the sizing engine (#712 FR-013) loads to arm its growth model; it is offline-produced config, never a live call. It MUST load unchanged into the substrate's prepared policy epoch (#973 FR-050); #712 and #973 MUST require exact matches for every bound field; a stale, mismatched, ineligible, expired, revoked, or tampered attestation MUST be rejected — failing closed to no growth-model arming.
...
- **BandValidityPolicy**: the predeclared eligibility policy of FR-010A — decision-cell map, one-sided lower-bound test for `p_lower`, clustering key, repeated-forecast/cluster estimator, horizon treatment, confidence method/level, multiplicity correction, minimum effective-event count, and failure behavior — that gates whether a `BandCoverageAttestation` may be emitted.
...
- **SC-008** (band-validity estimand): Emitting a `BandCoverageAttestation` requires the predeclared one-sided lower-bound test (realized frequency ≥ `p_lower` within every cell, on effective independent events, with the bound confidence method and multiplicity correction) to pass; a band-validity definition that tests a single 0/1 realization against the interval is rejected by construction — proven by a unit test over known cells and a negative test that the prohibited interval-membership definition cannot gate emission.
