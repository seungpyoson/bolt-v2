...
| F8 | `eval(`/`exec(`, shell-string execution (`os.system`, `os.popen`, `subprocess.getoutput`, `subprocess.getstatusoutput`, `pty.spawn`), and **Python-shaped** process replacement (`os.exec*`, `os.spawn*`, `os.posix_spawn*`) are tripwired; resolved non-Python process replacement is a documented Layer-3 boundary for the cargo shim | (grep at HEAD) |
