...
| F8 | `eval(`/`exec(` and the dynamic code-exec family (`os.system`, `os.popen`, `os.exec*`, `os.spawn*`, `os.posix_spawn*`, `subprocess.getoutput`, `subprocess.getstatusoutput`, `pty.spawn`) appear in **0** scanned scripts → an enumeration tripwire over them is pure drift-fence, breaks nothing today | (grep at HEAD) |
