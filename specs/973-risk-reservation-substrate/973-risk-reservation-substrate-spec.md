...
- **FR-003**: A single substrate-owned, pure `RiskEvaluator` MUST compute every authoritative risk quantity (collateral, incremental equity-floor stress loss, incremental governor realized loss, post-fill position/order usage, global and per-bucket usage, scope-restricted equity-floor stress loss for a caller-declared evaluation scope — both the current scoped stress and the post-candidate scoped stress — hard-limit results). The declared evaluation scope MUST be one the substrate recognizes — the candidate instrument/outcome, a substrate-derived concentration bucket (FR-004), or the whole portfolio — and an unrecognized scope MUST fail closed; scope restriction means the equity-floor stress loss computed as if only that scope's positions plus the candidate adjustment existed, on the one coherent `risk_state_version`. The SAME function MUST serve the advisory preview and the authoritative commit; it MUST perform no mutable reads and no I/O. Aggregate reserved/headroom quantities MUST be derived from the live reservation-record set and active descriptor at the evaluation point; mutable running totals or caches MUST NOT be authoritative for admission.
...
- **FR-050**: Policy, descriptor map, classifier version, fee model, quantity-policy versions, approvals, and attestations MUST first form an immutable `PreparedPolicyEpoch` whose internal references resolve without consulting a partially active epoch. Before cutover the substrate MUST validate the entire bundle, execute descriptor and classifier coverage checks, revalue all affected positions/orders/reservations under the prepared epoch (after draining queued venue events to establish accurate state, before any new admission), and determine the post-cutover admission state. Bundle validation MUST verify each `BandCoverageAttestation` (#724 FR-011) as a COMPLETE canonical artifact — its full field set plus authentication (signature/MAC) plus non-expiry/non-revocation — and MUST bind its exact content digest into the epoch; an entry that merely names an "attestation" without a verified canonical artifact and a bound digest MUST fail closed (consumed by #712 FR-013).
...
- **RiskPreviewInput / RiskAssessment**: the provenance-free preview input (carrying the declared model-risk evaluation scope) and its result (FR-025) — the result exposing the current and post-candidate scope-restricted equity-floor stress loss the model-risk cap consumes; the same pure kernel evaluates it and the committed candidate.
...
- The bankroll surface (`bolt_v3_capital_admission_state.rs::NtDerivedCapitalAdmissionState`: free collateral, equity, open orders, positions, loss snapshot) is the input to risk state; the substrate adds the atomic reservation and version that surface lacks today.
...
- Code anchors (at `main`): `bolt_v3_capital_reservation.rs::ReservationLedger`, `bolt_v3_capital_admission_state.rs::NtDerivedCapitalAdmissionState`, `bolt_v3_submit_admission.rs`, `bolt_v3_live_node.rs`, `bolt_v3_config.rs::CapitalPoolBlock`, `bolt_v3_capital_admission.rs::evaluate_capital_admission`. Reused single sources (FR-064): `bolt_v3_loss_governor.rs` + `bolt_v3_loss_halt_actions.rs` (realized-loss accumulator), `bolt_v3_kill_switch*.rs` (NT cancel/flatten/halt; #509/#673).
