...
- Multiple bot instances trading ONE capital pool MUST share the one serialization domain via the single transactional-store option (FR-006), not a per-process actor; the per-process single-writer-actor option applies only to a pool owned by one instance. The sizing engine (#712) is stateless and instance-safe; shared-capital coherence across instances is the substrate's job, not the sizer's.
