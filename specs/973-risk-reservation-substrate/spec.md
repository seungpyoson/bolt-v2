...
- **SC-008**: No dual paths — one serialization domain, one risk-state version, one cash-flow authority (the descriptor), one reservation ledger — verified by review.
...
- Code anchors (at `main`): `bolt_v3_capital_reservation.rs::ReservationLedger`, `bolt_v3_sizing_state.rs::NtDerivedSizingState`, `bolt_v3_submit_admission.rs`, `bolt_v3_live_node.rs`, `bolt_v3_config.rs::CapitalPoolBlock`, `bolt_v3_position_sizer.rs::evaluate_position_sizing` (renamed by #711).
- Dependency chain: #711 (rename → clean gate) → this substrate → #712 (sizing); #688 arms live.
