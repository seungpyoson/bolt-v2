...
impl LossSnapshotSource {
    #[must_use]
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::NtLossRuntimeFeed => "nt_loss_runtime_feed",
            Self::NtPortfolioSnapshot => "nt_portfolio_snapshot",
            Self::NtAccountSnapshot => "nt_account_snapshot",
            Self::NtAccountAndPositionSnapshot => "nt_account_and_position_snapshot",
            Self::NtPositionEvent => "nt_position_event",
            Self::NtPositionChanged => "nt_position_changed",
            Self::NtPositionClosed => "nt_position_closed",
            Self::NtPositionAdjusted => "nt_position_adjusted",
            Self::NtCapitalAdmissionState => "nt_capital_admission_state",
            Self::BoltLossSnapshot => "bolt_loss_snapshot",
            Self::LossGovernor => "loss_governor",
...
pub struct LossSnapshot {
    pub source: Option<LossSnapshotSource>,
...
fn loss_snapshot_diagnostics(
    ...
    let source_empty = snapshot.source.is_none();
...
mod tests {
    ...
    use super::{
        ...
        LossSnapshotStaleReason, LossSourceObservationTimestamps, evaluate_loss_admission,
        loss_snapshot_diagnostics, loss_snapshot_stale_reason, snapshot_is_stale,
    };
    ...
    fn stale_reason_refactor_matches_legacy_staleness_logic() {
        ...
        let fresh = LossSnapshot {
            source: Some(LossSnapshotSource::Other),
            observed_at_ns: 10_000,
    ...
    fn stale_missing_or_unattributed_snapshot_fails_closed() {
        ...
        stale.source = Some(LossSnapshotSource::NtPortfolioSnapshot);
