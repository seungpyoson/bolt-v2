...
pub struct LossGovernorPolicy {
    ...
    pub max_per_trade_loss: Decimal,
...
pub fn evaluate_loss_admission(
    ...
    if loss_breaches(daily_pnl, policy.max_daily_loss) {
        halt_reasons.push(LossHaltReason::DailyLossLimit);
    }
    if loss_breaches(rolling_pnl, policy.max_rolling_loss) {
        halt_reasons.push(LossHaltReason::RollingLossLimit);
    }
    if drawdown_breaches(current_equity, peak_equity, policy.max_drawdown) {
...
mod tests {
    ...
    fn policy() -> LossGovernorPolicy {
        LossGovernorPolicy {
            ...
            max_per_trade_loss: Decimal::new(10, 0),
            max_daily_loss: Decimal::new(25, 0),
            max_rolling_loss: Decimal::new(30, 0),
    ...
    fn snapshot() -> LossSnapshot {
        LossSnapshot {
            ...
            current_equity: Some(Decimal::new(1000, 0)),
            peak_equity: Some(Decimal::new(1000, 0)),
        }
    }

    #[test]
    fn policy_thresholds_are_required_values() {
        let policy = policy();
        let _: Decimal = policy.max_per_trade_loss;
        let _: Decimal = policy.max_daily_loss;
        let _: Decimal = policy.max_rolling_loss;
        let _: Decimal = policy.max_drawdown;
    }
    ...
    fn daily_loss_breach_rejects_admission() {
        ...
        snapshot.per_trade_pnl = Some(Decimal::ZERO);
    ...
    fn max_drawdown_breach_rejects_admission() {
        let mut snapshot = snapshot();
        snapshot.per_trade_pnl = Some(Decimal::ZERO);
        snapshot.current_equity = Some(Decimal::new(959, 0));
