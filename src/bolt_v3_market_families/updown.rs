...
/// and additionally rejects the static-market override fields — updown is a
/// rotating-cadence family, so a `static_condition_id` / `static_yes_outcome` /
/// `static_no_outcome` on an updown declaration is a misconfiguration that must
/// fail closed at load rather than be silently ignored.
