...
pub enum SafetyPolicyEnvelopeViolation {
    ...
    QuantityPolicyVersionNotPermitted {
        quantity_policy_version: String,
    },
...
fn validate_prepared_epoch(
    ...
    if !is_clean_runtime_value(&prepared_epoch.epoch_id)
        ...
        || prepared_epoch.quantity_policy_versions.is_empty()
        || prepared_epoch.approvals.is_empty()
    {
        return Err(PolicyEpochPrepareError::InvalidBundle);
    }

    for quantity_policy_version in &prepared_epoch.quantity_policy_versions {
        if !is_clean_runtime_value(quantity_policy_version) {
            return Err(PolicyEpochPrepareError::InvalidBundle);
        }
    }
...
fn validate_safety_envelope(
    ...
    if !is_clean_runtime_value(&envelope.envelope_id)
        ...
        || envelope.ranges.max_quantity_policy_versions == 0
        ...
        || envelope.permitted_quantity_policy_versions.is_empty()
        ...
        || envelope
            ...
            .chain(envelope.permitted_quantity_policy_versions.iter())
    ...
    if prepared_epoch.quantity_policy_versions.len() > envelope.ranges.max_quantity_policy_versions
    {
        return Err(PolicyEpochPrepareError::EnvelopeViolation(
            SafetyPolicyEnvelopeViolation::InvalidEnvelope,
        ));
    ...
    for quantity_policy_version in &prepared_epoch.quantity_policy_versions {
        if !envelope
            .permitted_quantity_policy_versions
            .contains(quantity_policy_version)
        {
            return Err(PolicyEpochPrepareError::EnvelopeViolation(
                SafetyPolicyEnvelopeViolation::QuantityPolicyVersionNotPermitted {
                    quantity_policy_version: quantity_policy_version.clone(),
                },
            ));
        }
    }
