...
pub fn assemble_strategy_build_context(
    ...
    .with_order_routing(context.order_routing.clone());
...
pub(crate) fn build_order_routing_handle(
    ...
    let execution = client
        .execution
        .as_ref()
        .ok_or_else(|| binding_error(strategy, "execution client has no execution block"))?;
    let binding = binding_for_provider_key(client.venue.as_str()).ok_or_else(|| {
        binding_error(
            strategy,
            "execution client has no provider registry binding",
        )
    })?;
    let load_economics = binding.execution_economics.ok_or_else(|| {
        binding_error(
            strategy,
            "execution client has no economics registry binding",
        )
    })?;
    ...
    BoltV3OrderRoutingHandle::new_with_product_surfaces(
        ...
        crate::bolt_v3_order_execution::BoltV3MultiSurfaceOrderRoutingConfig {
            ...
            reporting_unit: loaded.root.economics.reporting.pnl_currency.as_str(),
            routing_attachment_policy: economics.routing_attachment_policy,
        },
    )
    .map_err(|error| binding_error(strategy, format!("economics routing: {error:#}")))
}
