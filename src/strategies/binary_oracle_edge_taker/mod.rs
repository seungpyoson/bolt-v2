...
impl BinaryOracleEdgeTaker {
    ...
    fn apply_robust_sizing_and_probe(
        &self,
        ...
        let resized_notional = choose_robust_size(
            &self.robust_sizing_inputs(sized_expected_ev_per_notional, book_impact_cap_notional),
        );
        ...
        let resized_probe =
            match self.executable_entry_probe_for_side(selected_side, order_side, resized_notional)
            {
                Ok(probe) => probe,
                Err(reason) => {
        ...
        let final_supported_notional = choose_robust_size(
            &self.robust_sizing_inputs(final_expected_ev_per_notional, book_impact_cap_notional),
        );
        ...
        if resized_executable_edge.trade_allowed && resized_notional_supported {
            ...
            evaluation
                .pricing_blocked_by
                .push(EntryPricingBlockReason::SizedNotionalUnsupported(
                    selected_side,
                ));
            evaluation.selected_side = None;
            evaluation.sized_notional = None;
        ...
        if let Some(selected_side) = evaluation.selected_side {
                ...
                .filter(|value| is_positive_finite(*value))
            {
                ...
                if sized_executable_edge.trade_allowed {
                    ...
                    self.apply_robust_sizing_and_probe(
                        &mut evaluation,
                        selected_side,
                        fair_probability_up,
                        order_side,
