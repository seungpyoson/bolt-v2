...
//! handler feeds confirmed fills in here. The sign convention is owned by the
//! [`crate::strategies::maker_quote::MakerFamily`] implementation, so binary
//! outcome tokens and linear perps do not share a hardcoded exposure model. No
//! hardcoded literal (zero comes from [`crate::bolt_v3_numeric`]).

use crate::bolt_v3_numeric::{ZERO_F64, is_positive_finite};
use crate::strategies::maker_quote::{MakerFamily, QuoteSide};
...
impl MakerInventory {
    ...
    /// Fold one confirmed fill into the book using the family's directional
    /// exposure convention.
    ///
    ...
    pub fn apply_fill(
        ...
        self.net_position += family.signed_fill_qty(leg, side, qty);
        true
    }
}
...
mod tests {
    use super::*;
    use crate::strategies::maker_quote::{BinaryFamily, LinearPerpFamily};
    ...
    fn yes_buy_lengthens_and_no_buy_shortens_the_position() {
        ...
        assert!(book.apply_fill(&BinaryFamily, Leg::Yes, QuoteSide::Buy, 3.0));
        assert!((book.net_position() - 3.0).abs() < EPSILON);
        // A NO buy is the anti-YES outcome -> shortens the directional position.
        assert!(book.apply_fill(&BinaryFamily, Leg::No, QuoteSide::Buy, 2.0));
    ...
    fn sells_reverse_each_leg_sign() {
        let mut book = MakerInventory::flat();
        book.apply_fill(&BinaryFamily, Leg::Yes, QuoteSide::Buy, 5.0);
        // Selling YES (an exit) reduces the long-YES position.
        assert!(book.apply_fill(&BinaryFamily, Leg::Yes, QuoteSide::Sell, 2.0));
        assert!((book.net_position() - 3.0).abs() < EPSILON);
        // Selling NO lengthens the directional (YES-equivalent) position.
        assert!(book.apply_fill(&BinaryFamily, Leg::No, QuoteSide::Sell, 1.0));
        assert!((book.net_position() - 4.0).abs() < EPSILON);
    }

    #[test]
    fn linear_perp_sell_reduces_directional_inventory() {
        let mut book = MakerInventory::flat();
        assert!(book.apply_fill(&LinearPerpFamily, Leg::Yes, QuoteSide::Buy, 5.0));
        assert!(book.apply_fill(&LinearPerpFamily, Leg::No, QuoteSide::Sell, 2.0));
        assert!((book.net_position() - 3.0).abs() < EPSILON);
    }
    ...
    fn a_matched_pair_nets_to_flat() {
        ...
        let mut book = MakerInventory::flat();
        book.apply_fill(&BinaryFamily, Leg::Yes, QuoteSide::Buy, 4.0);
        book.apply_fill(&BinaryFamily, Leg::No, QuoteSide::Buy, 4.0);
    ...
    fn a_non_positive_or_non_finite_qty_is_rejected_and_leaves_the_book_unchanged() {
        let mut book = MakerInventory::flat();
        book.apply_fill(&BinaryFamily, Leg::Yes, QuoteSide::Buy, 2.0);
        assert!(!book.apply_fill(&BinaryFamily, Leg::Yes, QuoteSide::Buy, 0.0));
        assert!(!book.apply_fill(&BinaryFamily, Leg::Yes, QuoteSide::Buy, -1.0));
        assert!(!book.apply_fill(&BinaryFamily, Leg::No, QuoteSide::Buy, f64::NAN));
        assert!(!book.apply_fill(&BinaryFamily, Leg::No, QuoteSide::Buy, f64::INFINITY));
