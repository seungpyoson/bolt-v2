//! Pure, NautilusTrader-free requote-rate throttle for the binary-oracle maker
//! (W2 slice 4 — requote throttle reconciled to the venue budget).
//!
//! It keeps the maker's requote rate under the venue's published REST budget so
//! the strategy never out-runs the adapter's own rate limiter. The throttle is a
//! sliding-window token check plus a minimum inter-requote interval — pure
//! integer logic over a caller-supplied millisecond clock, with no NT type and no
//! wall-clock read, so it is exhaustively unit-testable.
//!
//! The window cap is resolved by the strategy layer from the venue capability
//! contract (`rate_budget.clob_per_minute`) times a configured
//! `requote_budget_fraction` (reserving headroom for non-requote REST calls); the
//! window length likewise comes from the minute definition in
//! [`crate::bolt_v3_numeric`]. This module receives the already-resolved integer
//! cap, interval, and window so it stays free of floats and hardcoded facts.
//! (The per-batch submit cap from `rate_budget.batch_submit_limit` is applied at
//! the order-submission site in the NT strategy shell, a later slice.)
//!
//! The budget is denominated in **REST calls, not requotes**: each `try_acquire`
//! is charged the requote's true REST-call weight, which the caller derives from
//! the venue capability — 1 on a modify-capable venue (a single in-place Modify)
//! and 2 on a no-modify venue (cancel + resubmit). Counting a flat one-per-requote
//! would let a no-modify venue issue twice the REST calls the cap intends and
//! out-run the adapter's limiter, so the weight is explicit at the call site
//! rather than buried in an unenforced headroom fraction.

use std::collections::VecDeque;

/// A sliding-window requote-rate throttle.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RequoteBudget {
    /// Length of the rate window, in milliseconds (e.g. one minute).
    window_ms: u64,
    /// Maximum REST-call cost permitted within any `window_ms` window. A requote
    /// is charged its true REST-call weight (see `try_acquire`), not a flat one.
    max_cost_per_window: u64,
    /// Minimum gap between two requote commands, in milliseconds.
    min_interval_ms: u64,
    /// `(timestamp_ms, cost)` of commands still inside the current window, oldest
    /// first.
    emits: VecDeque<(u64, u64)>,
    /// Running sum of `cost` over `emits` — the quantity checked against the cap.
    window_cost: u64,
    /// Timestamp (ms) of the most recent granted command, for the min-interval.
    last_emit_ms: Option<u64>,
}

impl RequoteBudget {
    /// A throttle permitting at most `max_cost_per_window` REST-call cost per
    /// `window_ms`, no closer together than `min_interval_ms`. Constructed
    /// explicitly — the bolt-v3 surface forbids `Default` — so the caller names
    /// the resolved budget it derived from the venue contract.
    pub fn new(max_cost_per_window: u64, window_ms: u64, min_interval_ms: u64) -> Self {
        Self {
            window_ms,
            max_cost_per_window,
            min_interval_ms,
            emits: VecDeque::new(),
            window_cost: 0,
            last_emit_ms: None,
        }
    }

    /// Try to acquire budget for a requote of REST-call `cost` as of `now_ms`.
    /// Returns `true` — recording the command — when the minimum interval since
    /// the last command and the remaining sliding-window budget both allow it;
    /// `false` (throttled, so the caller coalesces or drops the requote)
    /// otherwise.
    ///
    /// `cost` is the requote's true REST-call weight, derived by the caller from
    /// the venue capability: 1 for a modify-capable venue (a single in-place
    /// Modify) and 2 for a no-modify venue (cancel + resubmit).
    ///
    /// Fail-closed: a zero `cost` is a caller bug and is refused (a requote
    /// always costs at least one REST call); a `max_cost_per_window` of zero, or
    /// a single `cost` exceeding it, permits nothing.
    pub fn try_acquire(&mut self, now_ms: u64, cost: u64) -> bool {
        if cost == 0 {
            return false;
        }
        self.evict(now_ms);
        if let Some(last_ms) = self.last_emit_ms
            && now_ms.saturating_sub(last_ms) < self.min_interval_ms
        {
            return false;
        }
        if self.window_cost + cost > self.max_cost_per_window {
            return false;
        }
        self.emits.push_back((now_ms, cost));
        self.window_cost += cost;
        self.last_emit_ms = Some(now_ms);
        true
    }

    /// Number of commands currently counted within the window as of the last
    /// acquire/evict. Read seam for observability. (Differs from
    /// [`Self::cost_in_window`] when requotes cost more than one REST call.)
    pub fn in_window(&self) -> usize {
        self.emits.len()
    }

    /// Total REST-call cost currently counted within the window — the quantity
    /// actually compared against the budget cap.
    pub fn cost_in_window(&self) -> u64 {
        self.window_cost
    }

    fn evict(&mut self, now_ms: u64) {
        let cutoff_ms = now_ms.saturating_sub(self.window_ms);
        while self
            .emits
            .front()
            .is_some_and(|&(ts_ms, _)| ts_ms < cutoff_ms)
        {
            if let Some((_, cost)) = self.emits.pop_front() {
                self.window_cost -= cost;
            }
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    const ONE_MINUTE_MS: u64 = 60_000;

    #[test]
    fn burst_beyond_window_budget_is_throttled() {
        // Three REST calls per minute, no min-interval, unit-cost requotes: a
        // burst of five distinct ticks inside the window grants only three.
        let mut budget = RequoteBudget::new(3, ONE_MINUTE_MS, 0);
        let granted = (0..5_u64)
            .filter(|&i| budget.try_acquire(1_000 + i * 100, 1))
            .count();
        assert_eq!(granted, 3, "only the per-window budget is granted");
        assert_eq!(budget.in_window(), 3);
        assert_eq!(budget.cost_in_window(), 3);
    }

    #[test]
    fn min_interval_throttles_back_to_back_requotes() {
        // Large window budget, 500ms minimum interval.
        let mut budget = RequoteBudget::new(100, ONE_MINUTE_MS, 500);
        assert!(budget.try_acquire(1_000, 1));
        assert!(
            !budget.try_acquire(1_400, 1),
            "400ms < 500ms min interval must throttle"
        );
        assert!(
            budget.try_acquire(1_500, 1),
            "exactly the min interval is allowed"
        );
    }

    #[test]
    fn tokens_replenish_as_the_window_slides() {
        let mut budget = RequoteBudget::new(2, ONE_MINUTE_MS, 0);
        assert!(budget.try_acquire(1_000, 1));
        assert!(budget.try_acquire(2_000, 1));
        assert!(
            !budget.try_acquire(3_000, 1),
            "the two-per-window budget is exhausted"
        );
        // Advance past the window so the 1_000ms command ages out, freeing a slot.
        assert!(
            budget.try_acquire(1_000 + ONE_MINUTE_MS + 1, 1),
            "a slot frees once the oldest command leaves the window"
        );
    }

    #[test]
    fn zero_budget_permits_nothing() {
        let mut budget = RequoteBudget::new(0, ONE_MINUTE_MS, 0);
        assert!(!budget.try_acquire(1_000, 1));
        assert!(!budget.try_acquire(2_000, 1));
        assert_eq!(budget.in_window(), 0);
    }

    #[test]
    fn weighted_cost_counts_rest_calls_not_requotes() {
        // A no-modify venue requote is cancel + submit = 2 REST calls. With a
        // window cap of 4 REST calls only two such requotes fit (cost 4), not
        // four — the asymmetry a flat one-per-requote count would miss.
        let mut budget = RequoteBudget::new(4, ONE_MINUTE_MS, 0);
        assert!(budget.try_acquire(1_000, 2));
        assert!(budget.try_acquire(1_100, 2));
        assert!(
            !budget.try_acquire(1_200, 2),
            "two 2-cost requotes fill the 4-call budget"
        );
        assert_eq!(budget.cost_in_window(), 4);
        assert_eq!(budget.in_window(), 2, "two commands, four REST calls");
    }

    #[test]
    fn zero_cost_is_refused_fail_closed() {
        let mut budget = RequoteBudget::new(10, ONE_MINUTE_MS, 0);
        assert!(!budget.try_acquire(1_000, 0));
        assert_eq!(budget.cost_in_window(), 0);
    }

    #[test]
    fn a_single_requote_costlier_than_the_budget_never_grants() {
        let mut budget = RequoteBudget::new(1, ONE_MINUTE_MS, 0);
        assert!(
            !budget.try_acquire(1_000, 2),
            "a 2-call requote cannot fit a 1-call budget"
        );
    }
}
