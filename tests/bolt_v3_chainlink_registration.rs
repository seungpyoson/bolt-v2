...
//! `[secrets]` it consumes) registers as a data-only NT client through the
//! bolt-v3 LiveNode build path: secret resolution and adapter mapping both
//! succeed, the `ChainlinkStrikeSourceFactory` builds a `DataClient`, and the
//! NT data engine exposes the matching `ClientId` — with no execution client
//! created (the strike source declares no `[execution]` block).
