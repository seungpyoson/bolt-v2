...
fn live_node_build_path_registers_only_strategy_bound_signal_data_and_exec() {
    ...
    // feed at `okx_data`, plus a strategy-bound reference-current-price source
    // at `chainlink_reference`. `binance_reference` remains an extra configured
    // probe client that is unbound, and the scoped trade runner correctly
    // EXCLUDES it from both the registration summary and the NT engines.
    //
