...
fn live_node_module_runs_nt_through_bolt_v3_wrapper_without_live_canary_gate() {
    ...
    let position_sizer_rebuild_index = live_run_body
        ...
        .expect("run wrapper must reconcile NT cache state before runtime capture");
    assert!(
    ...
    assert!(
        ...
        "NT runtime capture must be wired before LiveNode::run"
    );
    assert!(
        position_sizer_rebuild_index < capture_index && capture_index < live_run_index,
        "startup reconciliation must run before NT runtime capture and LiveNode::run"
    );
    assert!(
