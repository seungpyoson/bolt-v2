...
fn strategy_input_snapshot_with_realized_volatility_snapshot() -> BoltV3StrategyInputEvidenceSnapshot
{
    BoltV3StrategyInputEvidenceSnapshot {
        ...
        realized_volatility_gate_result: BoltV3RvGateResult::Accepted,
...
fn sample_entry_skip_evidence() -> BoltV3EntrySkipEvidence {
    BoltV3EntrySkipEvidence {
        ...
        realized_vol_gate_result: BoltV3RvGateResult::Accepted,
...
fn entry_rv_receipt_fields_deserialize_from_legacy_evidence() {
    let mut skip = serde_json::to_value(sample_entry_skip_evidence()).expect("serialize skip");
    let skip_object = skip.as_object_mut().expect("skip must serialize as object");
    skip_object.remove("realized_vol_gate_result");
    skip_object.remove("realized_vol_receive_watermark_ms");
    let skip: BoltV3EntrySkipEvidence = serde_json::from_value(skip).expect("legacy skip");
    assert_eq!(
    ...
    let snapshot: BoltV3StrategyInputEvidenceSnapshot =
        serde_json::from_value(snapshot).expect("legacy strategy input");
    assert_eq!(
        snapshot.realized_volatility_gate_result,
        BoltV3RvGateResult::MissingSnapshot
    );
    assert_eq!(snapshot.realized_volatility_receive_watermark_ms, None);
}
