...
fn kill_switch_config_is_optional_and_parses_when_present() {
    ...
    assert_eq!(kill_switch.max_state_file_bytes, 65_536);
    assert_eq!(kill_switch.action_retry_interval_ms, 250);
    assert_eq!(kill_switch.action_retry_timeout_ms, 5_000);
    assert_eq!(kill_switch.mandatory_proof_max_age_ms, 1_000);
    assert_eq!(kill_switch.manual_reset_evidence_max_age_ms, 60_000);
    assert_eq!(
        kill_switch.authorized_operator_ids,
        vec!["operator-primary"]
    );
    assert_eq!(kill_switch.account_ids, vec!["POLYMARKET-001"]);
    assert_eq!(kill_switch.instrument_ids, vec!["BTC-USD.BINANCE"]);
