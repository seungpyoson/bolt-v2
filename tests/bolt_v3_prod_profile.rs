...
#[test]
fn verify_rejects_a_non_reviewed_but_internally_consistent_profile() {
    // The release anchor's reason to exist: a profile that is NOT the reviewed release, yet
    // is self-consistent with its own generated live.toml (so byte-compare, schema load, and
    // strategy-content checks would all pass), must STILL be rejected — otherwise a stale or
    // hand-edited on-box profile that an operator regenerated could pass verification.
    let dir = stage_runtime_dir("prod-profile-nonreviewed");
    // A loadable, self-consistent profile differing from the reviewed bytes by one harmless
    // trailing comment (an operator hand-edit not present in the release). Keep the basename
    // so the anchor matches it by name and fails on the bytes, not on an unknown name.
    let tweaked = format!(
        "{}\n# operator hand-edit not present in the reviewed release\n",
        support::repo_text(PROFILE)
    );
    let tweaked_profile = dir.path().join("prod-btc-5m.toml");
    write(&tweaked_profile, &tweaked);

    let generated =
        generate_live_config(&tweaked_profile).expect("a self-consistent profile still generates");
    let deployed = dir.path().join("live.toml");
    write(&deployed, &generated.text);

    let error = verify_live_config(&tweaked_profile, &deployed).expect_err(
        "a non-reviewed profile must be rejected by the release anchor even when it is \
         internally consistent with its generated live.toml",
    );
    assert!(
        matches!(error, ProfileError::ReleaseMismatch(_)),
        "a non-reviewed but self-consistent profile must fail at the release anchor, got: {error}"
    );
}

#[test]
fn verify_rejects_an_unrecognized_profile_name() {
    // A profile whose basename is not in the binary's release set must be rejected outright,
    // even if its bytes are a valid, loadable production config.
    let dir = stage_runtime_dir("prod-profile-unknown-name");
    let renamed_profile = dir.path().join("prod-eth-5m.toml");
    write(&renamed_profile, &support::repo_text(PROFILE));

    let generated =
        generate_live_config(&renamed_profile).expect("a valid profile still generates");
    let deployed = dir.path().join("live.toml");
    write(&deployed, &generated.text);

    let error = verify_live_config(&renamed_profile, &deployed)
        .expect_err("a profile name absent from the binary's release set must be rejected");
    match error {
        ProfileError::ReleaseMismatch(message) => assert!(
            message.contains("release-reviewed profile"),
            "error must explain the profile is not release-reviewed, got: {message}"
        ),
        other => panic!("expected a release-mismatch error, got: {other}"),
    }
}

#[test]
fn verify_of_the_tracked_profile_satisfies_the_release_anchor() {
    // Guards embed↔tracked drift through public behavior: verifying a freshly generated
    // deployment of the REAL tracked profile must pass the anchor. If config/prod-btc-5m.toml
    // or its strategy_files change without RELEASE_PROFILES being updated, the anchor's
    // strategy-set or byte check fails here (include_str! cannot drift on the profile bytes
    // themselves, but the embedded strategy list can).
    let dir = stage_runtime_dir("prod-profile-anchor-ok");
    let generated = generate_live_config(&support::repo_path(PROFILE)).expect("generate succeeds");
    let deployed = dir.path().join("live.toml");
    write(&deployed, &generated.text);

    verify_live_config(&support::repo_path(PROFILE), &deployed).expect(
        "the tracked profile must satisfy the release anchor — update RELEASE_PROFILES if its \
         strategy set changed",
    );
}
