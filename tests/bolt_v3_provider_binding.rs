...
#[test]
fn strategy_registration_resolves_capabilities_once_and_assembly_uses_cached_proof() {
    let source = support::repo_text("src/bolt_v3_strategy_registration.rs");
    let source_tokens = tokenize(&source);
    let context_tokens = item_body_tokens(
        &source_tokens,
    ...
    for cached_field in [
        ...
        vec!["order_routing", ":", "BoltV3OrderRoutingHandle"],
    ...
    let order_routing_position = token_sequence_position(
        &constructor_tokens,
        &[
            "let",
            "order_routing",
            "=",
            "build_order_routing_handle",
            "(",
        ],
    )
    .expect("constructor must prepare economics-backed order routing");
    assert!(
        settlement_position < order_routing_position,
        "settlement identity must fail closed before economics routing construction"
    );
    assert_eq!(
    ...
    for required_consumption in [
        ...
        vec!["context", ".", "order_routing", ".", "clone", "("],
