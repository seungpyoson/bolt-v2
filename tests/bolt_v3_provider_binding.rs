...
fn fixture_resolved_secrets() -> ResolvedBoltV3Secrets {
    ...
    clients.insert(
        ...
        // Provider mapping resolves these credentials through NT: `private_key`
        // must be a valid 32-byte secp256k1 scalar, and NT decodes `api_secret`
        // with the padded URL-safe base64 engine.
...
fn try_assembly_context<'a>(
    ...
    _resolved: &'a ResolvedBoltV3Secrets,
...
fn shared_strategy_assembly_installs_polymarket_rv_and_settlement_capabilities() {
    ...
    let _economics = assembled.order_economics();
...
#[test]
fn shared_strategy_assembly_allows_runtime_economics_authority_after_registration() {
    ...
    let economics_execution = toml::from_str::<toml::Value>(include_str!(
        "fixtures/economics/hyperliquid/execution.toml"
    ))
    .expect("offline Hyperliquid economics fixture should parse");
    let economics = economics_execution
        .as_table()
        .and_then(|execution| execution.get("economics"))
        .cloned()
        .expect("offline Hyperliquid fixture should contain economics TOML");
    loaded
        .root
        .clients
        .get_mut("hyperliquid_perps")
        .and_then(|client| client.execution.as_mut())
        .and_then(toml::Value::as_table_mut)
        .expect("inline Hyperliquid execution should be a table")
        .insert("economics".to_string(), economics);
    loaded.strategies[0].config.execution_client_id = "hyperliquid_perps".into();
    let resolved = fixture_resolved_secrets();
    let context = try_assembly_context(
        &loaded,
        &resolved,
        StrategyRuntimeCapabilities {
            realized_volatility: true,
    ...
    .expect("valid TOML authority should bind before the first runtime snapshot");
    let assembled = assemble_strategy_build_context(&context)
        .expect("strategy assembly should retain the dynamic economics handle");
