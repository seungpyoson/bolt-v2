...
fn strategy_with_reference_price(reference_price: &str) -> String {
...
fn parse_strategy(reference_price: &str) -> BoltV3StrategyConfig {
    toml::from_str(&strategy_with_reference_price(reference_price))
        .expect("strategy fixture should parse")
}

fn validate_reference_price(reference_price: &str) -> Vec<String> {
    validate_strategies(
        &root_fixture(),
        &loaded_strategy(parse_strategy(reference_price)),
    )
}

#[test]
fn strategy_scoped_reference_price_config_parses_ordered_sources_with_defaults() {
...
[reference_price.source.chainlink_primary]
provider = "chainlink_ws"
client_id = "chainlink_reference"
instrument_id = "BTC-USD.CHAINLINK"

[reference_price.source.polyresearch_backup]
...
"#,
    ...
    let reference_price = strategy
        .reference_price
        .expect("reference_price block should be present");
    assert_eq!(reference_price.asset, "BTC");
    assert_eq!(
        reference_price.source_order,
        ["chainlink_primary", "polyresearch_backup"]
    );
    assert_eq!(reference_price.min_valid_sources, 1);
    assert_eq!(
        reference_price.selection_policy,
        ReferencePriceSelectionPolicy::FirstValidPerInterval
    );
    assert_eq!(reference_price.max_source_age_ms, 2000);
    assert_eq!(reference_price.max_source_drift_bps, 25);
    assert_eq!(
        reference_price.drift_policy,
        ReferencePriceDriftPolicy::Observe
    );
    assert_eq!(
        reference_price.stale_policy,
        ReferencePriceStalePolicy::Block
    );

    let chainlink = reference_price
        .sources
    ...
    let polyresearch = reference_price
        .sources
...
#[test]
fn reference_price_validation_rejects_duplicate_ordered_source_keys() {
    let messages = validate_reference_price(
        r#"
[reference_price]
asset = "BTC"
...
"#,
    ...
    assert!(
        messages.iter().any(|message| {
            message.contains("reference_price.sources")
...
#[test]
fn reference_price_validation_rejects_empty_source_list() {
    let messages = validate_reference_price(
        r#"
[reference_price]
asset = "BTC"
...
"#,
    ...
    assert!(
        messages.iter().any(|message| {
            message.contains("reference_price.sources") && message.contains("non-empty")
        }),
...
#[test]
fn reference_price_validation_rejects_missing_source_section() {
    let messages = validate_reference_price(
        r#"
[reference_price]
asset = "BTC"
    ...
    assert!(
        messages.iter().any(|message| {
            message.contains("reference_price.sources")
                ...
                && message.contains("missing [reference_price.source.missing_backup]")
        }),
...
#[test]
fn reference_price_validation_rejects_unlisted_source_section() {
    let messages = validate_reference_price(
        r#"
[reference_price]
asset = "BTC"
...
[reference_price.source.unlisted_backup]
provider = "polyresearch_ws"
enabled = true
required = false
client_id = "polyresearch_reference"
...
"#,
    ...
    assert!(
        messages.iter().any(|message| {
            message.contains("[reference_price.source.unlisted_backup]")
...
#[test]
fn reference_price_validation_rejects_min_valid_sources_above_enabled_source_count() {
    let messages = validate_reference_price(
        r#"
[reference_price]
asset = "BTC"
...
"#,
    ...
    assert!(
        messages.iter().any(|message| {
            message.contains("reference_price.min_valid_sources")
                && message.contains("2")
...
#[test]
fn reference_price_validation_rejects_blank_asset() {
    let messages = validate_reference_price(
        r#"
[reference_price]
asset = ""
...
"#,
    ...
    assert!(
        messages.iter().any(|message| {
            message.contains("reference_price.asset") && message.contains("non-empty")
        }),
        "blank reference_price.asset should fail validation, got: {messages:#?}"
    );
}

#[test]
fn reference_price_validation_rejects_non_positive_thresholds() {
    let messages = validate_reference_price(
        r#"
[reference_price]
asset = "BTC"
...
"#,
    ...
    assert!(
        messages
            .iter()
            .any(|message| message.contains("reference_price.max_source_age_ms")),
        "non-positive max_source_age_ms should fail validation, got: {messages:#?}"
    );
    assert!(
        messages
            .iter()
            .any(|message| message.contains("reference_price.max_source_drift_bps")),
        "non-positive max_source_drift_bps should fail validation, got: {messages:#?}"
    );
}

#[test]
fn reference_price_validation_rejects_chainlink_source_without_instrument_id() {
    let messages = validate_reference_price(
        r#"
[reference_price]
asset = "BTC"
...
"#,
    ...
    assert!(
        messages.iter().any(|message| {
            message.contains("reference_price.source.chainlink_primary.instrument_id")
...
#[test]
fn reference_price_validation_rejects_source_identifier_for_wrong_asset() {
    let messages = validate_reference_price(
        r#"
[reference_price]
asset = "BTC"
...
[reference_price.source.polyresearch_primary]
provider = "polyresearch_ws"
enabled = true
required = true
client_id = "polyresearch_reference"
...
"#,
    ...
    assert!(
        messages.iter().any(|message| {
            message.contains("reference_price.source.polyresearch_primary")
