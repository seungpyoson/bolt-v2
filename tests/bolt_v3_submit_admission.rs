...
fn latched_kill_switch_states_block_entry_and_replace_before_nt_submit_without_consuming_count() {
    let gate_report = support::validated_bolt_v3_live_canary_gate_report(1, Decimal::new(1, 0));
    for state in latched_kill_switch_states() {
        let writer = Arc::new(support::RecordingDecisionEvidenceWriter::default());
        let admission = BoltV3SubmitAdmissionState::new_unarmed(writer.clone());
        admission
            .arm(gate_report.clone())
            .expect("valid gate report should arm admission");
        ...
        assert!(decisions.iter().all(|decision| {
            decision.outcome == BoltV3AdmissionOutcome::RejectedKillSwitchLatched
        }));
    }
}
