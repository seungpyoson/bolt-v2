...
fn rejects_absolute_decision_evidence_machine_relative_path() {
    ...
    let mutated = replace_in_fixture_root(
        "machine_relative_path = \"bolt-v3/decision-evidence/current/machine.jsonl\"",
        "machine_relative_path = \"/var/lib/bolt/decision-evidence/order-intents.jsonl\"",
    );
    let root: BoltV3RootConfig = toml::from_str(&mutated)
...
#[test]
fn rejects_empty_decision_evidence_retired_path_fence() {
    use bolt_v2::{bolt_v3_config::BoltV3RootConfig, bolt_v3_validate::validate_root_only};
    ...
    let root: BoltV3RootConfig =
        toml::from_str(&mutated).expect("empty retired-path fixture should parse");
    let messages = validate_root_only(&root);

    assert!(
        messages.iter().any(|message| {
            message.contains("persistence.decision_evidence.retired_relative_paths")
                && message.contains("must name at least one retired stream")
        }),
        "expected config rejection of an empty retired-path fence, got: {messages:#?}"
    );
}

#[test]
fn rejects_shared_decision_evidence_active_stream_path() {
    use bolt_v2::{bolt_v3_config::BoltV3RootConfig, bolt_v3_validate::validate_root_only};
    ...
    let root: BoltV3RootConfig =
        toml::from_str(&mutated).expect("shared active-path fixture should parse");
    let messages = validate_root_only(&root);

    assert!(
        messages
            .iter()
            .any(|message| message.contains("must resolve to different files")),
        "expected config rejection of a shared active stream, got: {messages:#?}"
    );
}

#[test]
fn rejects_retired_decision_evidence_path_reused_as_active_stream() {
    use bolt_v2::{bolt_v3_config::BoltV3RootConfig, bolt_v3_validate::validate_root_only};
    ...
    let root: BoltV3RootConfig =
        toml::from_str(&mutated).expect("active retired-path fixture should parse");
    let messages = validate_root_only(&root);

    assert!(
        messages
            .iter()
            .any(|message| message.contains("retired paths must differ from active stream paths")),
        "expected config rejection of an active path in the retired fence, got: {messages:#?}"
    );
}
